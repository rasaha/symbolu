...
the legacy ``ai_hiring.*`` paths resolve to the identical kernel objects.
"""
...
def test_service_classes_are_object_identical():
    from ai_hiring.services import (
...
def test_repository_classes_are_object_identical():
    from ai_hiring.repositories import (
...
def test_audit_identity_policy_are_object_identical():
    from ai_hiring.services.audit_service import AuditService as OldAudit
    from ai_hiring.domain.enums import AuditEventType as OldEv, ActorType as OldAct
    from ai_hiring.policies.evidence_access_policy import Permission as OldPerm
    from ai_hiring.policies.decision_boundary import IdentityProvider as OldIdp
    from decision_governance.audit import AuditService as NewAudit, AuditEventType as NewEv
    from decision_governance.identity import ActorType as NewAct, IdentityProvider as NewIdp
    from decision_governance.policy import Permission as NewPerm
...
def test_typed_errors_are_object_identical():
    ...
    from decision_governance.errors import (
...
def test_operational_kernel_imports_without_the_application():
    code = (
        "import decision_governance.services, decision_governance.repositories, "
        "decision_governance.audit, decision_governance.identity, "
        "decision_governance.policy, sys; "
        "bad=[m for m in sys.modules if m.startswith(('ai_hiring','domains','applications'))]; "
...
def test_full_governance_lifecycle_runs_without_the_hiring_domain():
    # Everything imported here is kernel-only — no ai_hiring, no domains.hiring.
    ...
    from decision_governance.policy import AccessGrant, EvidenceAccessPolicy, GrantStore, Permission
    from decision_governance.repositories import (
