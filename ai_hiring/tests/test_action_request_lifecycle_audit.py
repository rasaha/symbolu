...
from ai_hiring.action_requests import ActionRequestStatus, is_legal_transition
from ai_hiring.api.action_request_routes import (
...
def test_legal_transition_table_has_no_executed_state(action_platform):
    ...
    from ai_hiring.action_requests.lifecycle import ALLOWED_TRANSITIONS
...
def test_illegal_transition_is_rejected(action_platform):
    ...
    from ai_hiring.errors import ActionRequestNotReadyError
...
def test_conflicting_idempotency_key_is_rejected(action_platform):
    from ai_hiring.errors import DuplicateActionRequestError
    _, decision = decided_case(action_platform)
    published_mapping(action_platform)
...
def test_no_execution_record_type_exists():
    import ai_hiring.action_requests as ar
...
def test_domain_depends_on_port_not_concrete_gate():
    ...
    import ai_hiring.services.action_authorization_service as svc
    source = svc.__file__
