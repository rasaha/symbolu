...
2. The hiring domain audit trail is hiring-owned and additive — it never smuggles
   ...
   only the frozen ``ugence_decision_authority.api`` surface.
"""
...
from ugence_decision_authority.api.audit import AuditEventType
from ugence_ai_hiring.domain_audit.events import HiringDomainEventType
from ugence_ai_hiring.hiring_applications.status import (
...
H1_MODULES = [
    "ugence_ai_hiring/domain_audit/events.py", "ugence_ai_hiring/domain_audit/event.py",
    "ugence_ai_hiring/domain_audit/repository.py", "ugence_ai_hiring/domain_audit/service.py",
    "ugence_ai_hiring/requisitions/requisition.py", "ugence_ai_hiring/requisitions/job_definition.py",
    "ugence_ai_hiring/requisitions/status.py",
    "ugence_ai_hiring/candidates/candidate.py",
    "ugence_ai_hiring/hiring_applications/application.py", "ugence_ai_hiring/hiring_applications/status.py",
    "ugence_ai_hiring/hiring_applications/eligibility.py", "ugence_ai_hiring/hiring_applications/readiness.py",
    "ugence_ai_hiring/intake/intake.py",
    "ugence_ai_hiring/repositories/product_repositories.py",
    "ugence_ai_hiring/services/_hiring_context.py", "ugence_ai_hiring/services/requisition_service.py",
    "ugence_ai_hiring/services/candidate_service.py", "ugence_ai_hiring/services/application_service.py",
    "ugence_ai_hiring/services/evidence_intake_service.py",
    "ugence_ai_hiring/services/hiring_reconstruction_service.py",
    "ugence_ai_hiring/api/product_contracts.py",
]
...
def test_h1_active_code_consumes_only_public_api():
    """Every H1 module that imports the platform imports it via ugence_decision_authority.api."""
    violations = []
    for rel in H1_MODULES:
        ...
        for node in ast.walk(tree):
            ...
            for t in targets:
                if t.split(".")[0] == "ugence_decision_authority" and not t.startswith("ugence_decision_authority.api"):
                    violations.append(f"{rel}:{node.lineno} -> {t}")
    assert not violations, "H1 code must import only ugence_decision_authority.api:\n" + "\n".join(violations)
