...
(``ugence_decision_authority``) with the **hiring domain** (``ugence_ai_hiring``) into a
fully-wired, in-memory :class:`HiringPlatform`.

Direct kernel adoption (Phase 5C): every governance concept below is imported
**directly from ``ugence_decision_authority.*``** — services, repositories, audit,
identity, policy, the action control-plane port, and the external-execution
port. Hiring-domain concepts are imported from ``ugence_ai_hiring.*``. This module
...
Dependency direction: ``ugence_ai_hiring`` → {``ugence_ai_hiring``,
``ugence_decision_authority``}; ``ugence_ai_hiring`` → ``ugence_decision_authority``. The
reverse never holds.
"""
...
from ugence_decision_authority.api.identity import IdentityProvider, StaticIdentityProvider
from ugence_decision_authority.api.audit import AuditService, InMemoryAuditRepository
from ugence_decision_authority.api.policy import EvidenceAccessPolicy, GrantStore
from ugence_decision_authority.api.repositories import (
    InMemoryActionRequestRepository,
    InMemoryDecisionCaseRepository,
    InMemoryExecutionRepository,
)
from ugence_decision_authority.api.ports import (
...
# --- Hiring domain ----------------------------------------------------------
from ugence_ai_hiring.hiring.repositories import (
...
from ugence_ai_hiring.hiring.services import (
...
from ugence_ai_hiring.hiring.adapters import HiringAssessmentLinkedRecordAdapter
...
class HiringPlatform:
    ...
    def build_api(self):
        """Construct the callable :class:`~ugence_ai_hiring.api.HiringAPI` facade."""
        from ugence_ai_hiring.api.routes import HiringAPI
    ...
    def build_ontology_api(self):
        """Construct the callable :class:`~ugence_ai_hiring.api.OntologyAPI` facade."""
        from ugence_ai_hiring.api.ontology_routes import OntologyAPI

        return OntologyAPI(self.ontology_service, self.identity_provider)

    def build_rubric_api(self):
        """Construct the callable :class:`~ugence_ai_hiring.api.RubricAPI` facade."""
        from ugence_ai_hiring.api.rubric_routes import RubricAPI

        return RubricAPI(self.rubric_service, self.identity_provider)

    def build_assessment_api(self):
        """Construct the callable :class:`~ugence_ai_hiring.api.AssessmentAPI` facade."""
        from ugence_ai_hiring.api.assessment_routes import AssessmentAPI

        return AssessmentAPI(self.assessment_service, self.identity_provider)

    def build_decision_case_api(self):
        """Construct the callable :class:`~ugence_ai_hiring.api.DecisionCaseAPI` facade."""
        from ugence_ai_hiring.api.decision_case_routes import DecisionCaseAPI
    ...
    def build_action_request_api(self):
        """Construct the callable :class:`~ugence_ai_hiring.api.ActionRequestAPI` facade."""
        from ugence_ai_hiring.api.action_request_routes import ActionRequestAPI
    ...
    def build_execution_api(self):
        """Construct the callable :class:`~ugence_ai_hiring.api.ExecutionAPI` facade."""
        from ugence_ai_hiring.api.execution_routes import ExecutionAPI
