# S1 — enforcement obligations discharged; contracts specified and implemented
...
* the **exact C5 category for every classified field**, with **every** C5a and C5b entry
  mutation-pinned — not a chosen sample. The sweep is **55 patterned entries × 8
  weakening categories = 440 cases**, and every one is killed when the guard's verdict
  ...
  `test_swapping_a_patterned_entry_to_its_sibling_is_rejected_not_ignored`, 55 further
...
* **V13** makes `terminal_outcome = PROPOSAL` conditional on a selection and on
  readiness. C7 made readiness unconstructible in S1, so the fail-closed ceiling was
  `NEED_EVIDENCE`, `ABSTAIN`, `ESCALATE`; with C7 removed under OD-7, V13 enforces R-2
  as ratified — `ProposerProcessRecord` requires a selection for `PROPOSAL`, and
  `build_proposer_advisory` recomputes Equation 2 for the resolved candidate — and
  Equation 2's seventh term is what keeps a completed-but-unsatisfied candidate out;
...
| **R-3 process ordering** | **superseded — see the row below.** This row previously described a pre-implementation state (`vocabulary.py` not yet declaring `ProposerProcessState`, `TerminalOutcome` standing in as a placeholder, `test_process_ordering_obligation.py` carrying the obligation as a named skip) and an open cardinality/comparison-basis question for R-4. `ProposerProcessState` is declared (`vocabulary.py`), R-3 and R-4 are enforced model validators in `contracts.py`, and both are in the `ENFORCED` registry below. The cardinality and comparison-basis question this row once left open is resolved by OD-6(iii) (`docs/architecture/ADR_UGENCE_AGENTIC_PROPOSER_MVP_READINESS.md`): the enum is nine members, the four terminal members share `TerminalOutcome`'s wire values, and R-4's "equals" is value equality — ratified as specification text, not left to this document to settle |
| **Zero locally decidable violations the declared contracts accept** | **fully enforced** — `tests/s1_specification_mirror.py`'s `representative_shapes()` returns the declared `src/` contracts rather than temporary duplicates, and `tests/test_unenforced_local_rules.py`'s `ENFORCED` registry carries the discharge record for every rule that used to live in this row: L-1; the three `candidates` rules on both `ProposerAdvisory` (D7) and `AdvisoryCandidateSet` (D6); R-8's no-duplicates rule; the three Part D rejects-an-empty-list rules; S-1 and S-2 on `AdvisoryCandidateSet`; R-1b(v), (vi) and the local half of (vii); R-3 and R-4; C4 on all sixteen `datetime` fields; D8's `completed_at >= started_at`; D3's NFC requirement on `WorkMandate.purpose`; and, since OD-7, part 3's completion/outcome coupling and OD-8's selection-policy recomputation, which replaced C7's and C9's unconditional refusals in the same change set — each paired with a construction the real contract now rejects. `[V]` **`S-2 (via R-1b)` was the last remaining entry and is now enforced**: it landed on `ProposerAdvisory` only as a consequence of R-1b(iii)/(iv) holding against the *referenced* `AdvisoryCandidateSet`, which no single instance carries — but selection-policy v1 recomputes the qualifying pool from the advisory's **own** nested candidates, so an advisory selecting an ineligible one is refused locally. It moved to `ENFORCED` rather than being deleted. `CandidateAdvisory.claim_refs` is deliberately absent — R-8 does not name it and its Part D row states `each C5a` only, so a duplicate there is lawful. `tests/test_unenforced_local_rules.py` constructs a violating instance for every rule named here, so this row cannot claim a rule is unenforced once it is, nor quietly omit one that still is |
| **R-2, R-5, R-6, R-7, R-9 and R-10** | **named, not covered** — none is locally decidable from one contract instance, which is the scope this row's derivation is quantified over (`tests/test_unenforced_local_rules.py`'s own boundary); each needs a builder, a verifier or a second contract to check, so none can appear in that module's `UNENFORCED` or `ENFORCED` registries regardless of what exercises it elsewhere. Real coverage does exist elsewhere, and this row does not contradict it: R-5, R-6, R-7, R-9 and R-10 are exercised through `CrossContractViolationError` (OD-6(ii)) in `tests/test_s1_implementation_obligations.py`'s `OD-6(ii)` section, and R-2's locally decidable residue — that `PROPOSAL` requires a selection (V13, as reimplemented under OD-7) — is separately enforced and is in `ENFORCED`, not here; only R-2's general biconditional, which needs the identity/role/mandate/context objects a `ProposerProcessRecord` alone does not carry, remains outside what any locally scoped registry can claim. They are *named* in `tests/test_unenforced_local_rules.py`, which records why each falls outside the locally decidable set, so that a rule no test mentions cannot be confused with a rule nobody has considered — but a mention in a scope paragraph establishes nothing behavioural and must not be read as coverage. Both halves are derived, not hand-written, and **coverage is decided by a registry of constructed cases, never by a textual mention**: `test_every_ratified_rule_is_named_somewhere_under_tests` recomputes the unnamed set from the specification's rule statements and a scan of `tests/` and requires it to be empty, and `test_the_named_but_unexercised_row_is_derived_from_the_test_tree` recomputes this row's membership as the ratified rules absent from `UNENFORCED`, `ENFORCED` and the obligation module's `OBLIGATION_RULES`. That distinction is load-bearing: deriving membership from mentions classified **R-4** as covered by the very module that disclaims covering it. R-1b is not in this list because clauses (v), (vi) and (vii)'s local half carry constructed cases; its remaining cross-contract clauses are as uncovered as the rules above, and for the same reason. An earlier hand-written version of this row was wrong in both directions at once — it named R-4 and omitted R-7, which nothing mentioned at all |
