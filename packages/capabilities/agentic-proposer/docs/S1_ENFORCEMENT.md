...
| **One locally decidable violation the declared contracts accept** | **not enforced** — `tests/s1_specification_mirror.py`'s `representative_shapes()` now returns the declared `src/` contracts rather than temporary duplicates, and `tests/test_unenforced_local_rules.py`'s `ENFORCED` registry carries the discharge record for every rule that used to live in this row: L-1; the three `candidates` rules on both `ProposerAdvisory` (D7) and `AdvisoryCandidateSet` (D6); R-8's no-duplicates rule; the three Part D rejects-an-empty-list rules; S-1 and S-2 on `AdvisoryCandidateSet`; R-1b(v), (vi) and the local half of (vii); R-3 and R-4; C4 on all sixteen `datetime` fields; D8's `completed_at >= started_at`; and D3's NFC requirement on `WorkMandate.purpose` — each paired with a construction the real contract now rejects. `[I]` **`S-2 (via R-1b)`** is the one entry that remains: S-2 lands on `ProposerAdvisory` only as a consequence of R-1b(iii)/(iv) holding against the *referenced* `AdvisoryCandidateSet`, which no single `ProposerAdvisory` instance carries, so it stays a builder/verifier obligation (`build_proposer_advisory`, `verify_advisory_selection`) rather than a local one. `CandidateAdvisory.claim_refs` is deliberately absent — R-8 does not name it and its Part D row states `each C5a` only, so a duplicate there is lawful. `tests/test_unenforced_local_rules.py` constructs a violating instance for the one rule named here, so this row cannot claim a rule is unenforced once it is, nor quietly omit one that still is |
