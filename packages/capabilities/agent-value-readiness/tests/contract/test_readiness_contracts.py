...
These assert *structure* only. No test expects the package to compute a readiness
decision — that is GV-3R-b.
"""
...
def rdy_ref(tenant=""):
    scope = PolicyScope.TENANT if tenant else PolicyScope.GLOBAL
    return PolicyReference(policy_id="r", policy_family=PolicyFamily.READINESS, version="1", content_digest=D, scope=scope, tenant_id=tenant)


def claim(metric="accuracy", value="0.95", tenant=TEN, subject=SUB, basis=SourceBasis.OBSERVED):
    kw = {}
    if basis is SourceBasis.OBSERVED:
        kw["assessment_window"] = WIN
    return MetricClaim(
        claim_id=f"clm-{metric}", tenant_id=tenant, subject_id=subject, metric_id=metric, value=value,
...
def intel(**kw):
    base = dict(result_id="ir1", tenant_id=TEN, subject_id=SUB, context_id=CTX, task_or_outcome_ref="i",
                ...
                applicable_targets=[ReadinessTarget.PILOT, ReadinessTarget.PRODUCTION], status=GateStatus.PASS)
    base.update(kw)
...
def cap(**kw):
    base = dict(result_id="cr1", tenant_id=TEN, subject_id=SUB, context_id=CTX, task_or_outcome_ref="i",
                ...
                demonstration=CapabilityDemonstration.MET_THRESHOLD, evidence_sufficient=True)
    base.update(kw)
...
def gate(gid="g1", target=ReadinessTarget.PILOT, kind=RequirementClass.MANDATORY, applicable=True, status=GateStatus.PASS):
    return GateResult(gate_id=gid, readiness_policy_ref=rdy_ref(), gate_kind=kind, requested_target=target, applicable=applicable, status=status)


def active_condition(cid="cond1"):
    return ConditionSet(
        condition_id=cid, source_gate_or_finding_ref="g-cond", concern_requirement_class=RequirementClass.CONDITIONAL,
        ...
        evidence_refs=["ev1"], effective_from=T0, revocation_trigger="breach",
    )


def determination(**kw):
    base = dict(assessment_id="a1", tenant_id=TEN, subject_id=SUB, context=context(), readiness_policy_ref=rdy_ref(),
...
# --------------------------------------------------------------------------- #
# Distinct types + indicator class
# --------------------------------------------------------------------------- #
...
def test_all_five_classifications_representable():
    values = {c.value for c in ReadinessClassification}
    assert values == {"NOT_ASSESSABLE", "NOT_READY", "PILOT_READY", "READY_WITH_CONDITIONS", "DEPLOYMENT_READY"}
...
def test_capability_distinguishes_demonstration_states():
    ...
    assert missing.status is GateStatus.FAIL  # missing critical capability = mandatory blocking failure, representable
...
def test_adoption_preserves_source_basis_axis_without_elevation():
    # A modeled/synthetic-basis adoption claim keeps its axis; the readiness
    # result does not elevate it to observed/attributed/verified.
...
def test_indicator_evidence_refs_reject_blanks_and_dups():
    with pytest.raises(ReadinessContractError):
        intel(evidence_refs=["a", "a"])
    with pytest.raises(ReadinessContractError):
        intel(evidence_refs=["a", "  "])


# --------------------------------------------------------------------------- #
# GateResult: applicable vs diagnostic; non-waivable
# --------------------------------------------------------------------------- #
def test_gateresult_requires_readiness_family_ref():
    ...
    with pytest.raises(ReadinessContractError):
        GateResult(gate_id="g", readiness_policy_ref=bad, gate_kind=RequirementClass.MANDATORY, requested_target=ReadinessTarget.PILOT, applicable=True, status=GateStatus.PASS)


def test_diagnostic_gate_is_not_blocking_even_on_fail():
    # Production-only mandatory gate, evaluated diagnostically for a PILOT target.
    g = gate(gid="prod-only", target=ReadinessTarget.PILOT, kind=RequirementClass.MANDATORY, applicable=False, status=GateStatus.FAIL)
    assert g.is_diagnostic is True
    assert g.is_blocking is False


def test_applicable_mandatory_fail_is_blocking():
    g = gate(kind=RequirementClass.MANDATORY, applicable=True, status=GateStatus.FAIL)
    assert g.is_blocking is True


def test_applicable_advisory_fail_is_not_blocking():
    g = gate(kind=RequirementClass.ADVISORY, applicable=True, status=GateStatus.FAIL)
    assert g.is_blocking is False


# --------------------------------------------------------------------------- #
# ConditionSet: non-waivable mandatory; APPROVED_ACTIVE completeness
# --------------------------------------------------------------------------- #
...
def test_condition_rejects_advisory_concern():
    with pytest.raises(ReadinessContractError):
        ConditionSet(condition_id="c", source_gate_or_finding_ref="g", concern_requirement_class=RequirementClass.ADVISORY, current_status=ConditionStatus.PROPOSED)


def test_incomplete_approved_active_rejected():
    with pytest.raises(ReadinessContractError):
        ConditionSet(condition_id="c", source_gate_or_finding_ref="g", concern_requirement_class=RequirementClass.CONDITIONAL, current_status=ConditionStatus.APPROVED_ACTIVE)  # missing authority/owner/etc


def test_complete_approved_active_is_active():
    c = active_condition()
    assert c.is_active is True


def test_expired_condition_is_not_active():
...
def test_proposed_condition_is_not_active():
    c = ConditionSet(condition_id="c", source_gate_or_finding_ref="g", concern_requirement_class=RequirementClass.CONDITIONAL, current_status=ConditionStatus.PROPOSED)
    assert c.is_active is False


# --------------------------------------------------------------------------- #
# AdvisoryComposite: Decimal, advisory, no tier
# --------------------------------------------------------------------------- #
def test_composite_rejects_float():
    with pytest.raises(ReadinessContractError):
...
def test_composite_rejects_out_of_range_score():
    with pytest.raises(ReadinessContractError):
        AdvisoryComposite(method_id="m", method_version="1", score=Decimal("1.5"), scale_min=Decimal("0"), scale_max=Decimal("1"), component_result_refs=["r1"])


def test_composite_requires_method_version_and_components():
    with pytest.raises(ReadinessContractError):
        AdvisoryComposite(method_id="m", method_version="", score=Decimal("0.5"), scale_min=Decimal("0"), scale_max=Decimal("1"), component_result_refs=["r1"])
    with pytest.raises(ReadinessContractError):
        AdvisoryComposite(method_id="m", method_version="1", score=Decimal("0.5"), scale_min=Decimal("0"), scale_max=Decimal("1"), component_result_refs=[])


def test_composite_is_advisory_locked_true():
    with pytest.raises(ReadinessContractError):
        AdvisoryComposite(method_id="m", method_version="1", score=Decimal("0.5"), scale_min=Decimal("0"), scale_max=Decimal("1"), component_result_refs=["r1"], is_advisory=False)


def test_composite_has_no_tier_field():
    field_names = {f.name for f in dataclasses.fields(AdvisoryComposite)}
    assert "classification" not in field_names and "tier" not in field_names and "readiness" not in field_names


# --------------------------------------------------------------------------- #
# Determination: target/classification consistency
# --------------------------------------------------------------------------- #
def test_determination_happy_pilot_ready():
    det = determination(gate_results=[gate(status=GateStatus.PASS)])
...
def test_ready_with_conditions_requires_conditions():
    with pytest.raises(ReadinessContractError):
        determination(requested_target=ReadinessTarget.PRODUCTION, classification=ReadinessClassification.READY_WITH_CONDITIONS, conditions=[])


def test_ready_with_conditions_production_and_conditions_ok():
    det = determination(requested_target=ReadinessTarget.PRODUCTION, classification=ReadinessClassification.READY_WITH_CONDITIONS, conditions=[active_condition()])
    assert len(det.conditions) == 1


def test_ready_with_conditions_cannot_target_pilot():
    with pytest.raises(ReadinessContractError):
        determination(requested_target=ReadinessTarget.PILOT, classification=ReadinessClassification.READY_WITH_CONDITIONS, conditions=[active_condition()])


def test_not_ready_requires_blocking_reason():
    with pytest.raises(ReadinessContractError):
        determination(classification=ReadinessClassification.NOT_READY)


def test_not_ready_with_blocking_gate_ok():
    g = gate(gid="mfail", status=GateStatus.FAIL)
    det = determination(classification=ReadinessClassification.NOT_READY, gate_results=[g], blocking_gate_ids=["mfail"])
...
def test_not_assessable_with_indeterminate_gate_ok():
    g = gate(gid="mind", status=GateStatus.INDETERMINATE)
    det = determination(classification=ReadinessClassification.NOT_ASSESSABLE, gate_results=[g], indeterminate_gate_ids=["mind"])
    assert det.indeterminate_gate_ids == ("mind",)


def test_blocking_gate_ids_must_reference_applicable_mandatory_fail():
    # A diagnostic (non-applicable) FAIL gate cannot be listed as a blocker.
    g = gate(gid="diag", applicable=False, status=GateStatus.FAIL)
    with pytest.raises(ReadinessContractError):
        determination(classification=ReadinessClassification.NOT_READY, gate_results=[g], blocking_gate_ids=["diag"])


def test_ready_class_cannot_carry_blocking_gate():
    g = gate(gid="mfail", status=GateStatus.FAIL)
    with pytest.raises(ReadinessContractError):
        determination(classification=ReadinessClassification.PILOT_READY, gate_results=[g], blocking_gate_ids=["mfail"])


def test_determination_gate_target_must_match_requested():
    g = gate(gid="g1", target=ReadinessTarget.PRODUCTION, status=GateStatus.PASS)
    with pytest.raises(ReadinessContractError):
...
def test_determination_duplicate_result_ids_rejected():
    with pytest.raises(ReadinessContractError):
        determination(intelligence_results=[intel(result_id="dup"), intel(result_id="dup", dimension=IntelligenceDimension.RELIABILITY, claim=claim("rel", "0.9"))])


def test_determination_diagnostic_gate_ids_property():
    g_app = gate(gid="app", applicable=True, status=GateStatus.PASS)
    g_diag = gate(gid="diag", applicable=False, status=GateStatus.PASS)
