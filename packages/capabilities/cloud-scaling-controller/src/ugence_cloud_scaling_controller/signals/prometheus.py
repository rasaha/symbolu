...
# ``requests`` is an OPTIONAL dependency (the ``prometheus`` extra). Importing this
# module must not require it — only instantiating :class:`PrometheusClient` (a
# read-only signal adapter) does. This keeps the core package importable with numpy
# alone. Behavior is unchanged whenever ``requests`` is installed.
...
class PrometheusClient:
    ...
    def __init__(self, config: Optional[PrometheusConfig] = None):
        if requests is None:  # pragma: no cover - optional extra not installed
            raise ImportError(
                "PrometheusClient requires the 'requests' package. Install the "
                "optional extra: pip install ugence-cloud-scaling-controller[prometheus]"
            )
