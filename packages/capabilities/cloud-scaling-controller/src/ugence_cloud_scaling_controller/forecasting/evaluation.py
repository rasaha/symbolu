...
class ForecastEvaluationRecord:
    ...
    An ``EVALUATED`` record EMBEDS the canonical actual ``CanonicalCapacityState`` and
    derives — and re-validates at construction — every outcome field from it:
    ``actual_state_digest`` (= ``actual_state.digest()``), ``actual_value`` / ``unit`` (from
    ``extract_sample(actual_state, target)``), and the subject/tenant/scope binding
    (``actual_state.subject == subject``). The signed/absolute/squared errors and interval
    coverage/width are then recomputed from the derived actual value and the bound forecast
    interval. Because the actual state is embedded, a caller cannot present a forged
    ``actual_value`` with an unrelated ``actual_state_digest`` and self-consistent arithmetic:
    the digest is checked against the embedded state, and the value/unit are re-extracted from
    it. :meth:`from_dict` reconstructs the state and runs the same validation, so no
    deserialization path bypasses the relationship.

    The digest remains a canonical content IDENTITY/integrity value — not a signature and not
    proof of authenticity; authenticity comes from producing the record through the controlled
    :func:`evaluate_forecast` service bound to a real canonical state.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.subject, CapacitySubject):
            raise EvaluationError("subject must be a CapacitySubject")
        ...
        # --- structural actual-state binding ---------------------------------------
        if self.actual_state is not None:
            if not isinstance(self.actual_state, CanonicalCapacityState):
                raise EvaluationError("actual_state must be a CanonicalCapacityState or None")
            derived_digest = self.actual_state.digest()
            if self.actual_state_digest != derived_digest:
                raise EvaluationError(
                    "actual_state_digest must equal the embedded actual_state's digest "
                    "(a forged digest cannot be presented for an unrelated state)")
            if self.actual_event_time is not None and self.actual_event_time != self.actual_state.observed_at:
                raise EvaluationError("actual_event_time must equal actual_state.observed_at")
        else:
            if self.actual_state_digest is not None:
                raise EvaluationError("actual_state_digest requires the embedded actual_state")
        ...
        if self.status is EvaluationStatus.EVALUATED:
            if self.actual_state is None:
                raise EvaluationError("EVALUATED requires the embedded actual_state")
            # Subject/tenant/scope binding: the scored actual must be the SAME subject.
            if self.actual_state.subject != self.subject:
                raise EvaluationError("EVALUATED actual_state.subject must equal the forecast subject")
            # Value + unit are DERIVED from the state, never accepted independently.
            sample = extract_sample(self.actual_state, ForecastTarget(self.target))
            if sample is None:
                raise EvaluationError("EVALUATED actual_state must carry the forecast target")
            if self.unit != sample.unit:
                raise EvaluationError("unit must equal the target sample's unit derived from actual_state")
            if self.actual_value is None or abs(self.actual_value - sample.value) > _TOL:
                raise EvaluationError("actual_value must equal the value derived from actual_state")
            if self.point_forecast is None:
                raise EvaluationError("EVALUATED requires point_forecast")
        ...
        elif self.status is EvaluationStatus.SUBJECT_MISMATCH:
            if self.actual_state is None:
                raise EvaluationError("SUBJECT_MISMATCH requires the embedded actual_state")
            if self.actual_state.subject == self.subject:
                raise EvaluationError("SUBJECT_MISMATCH requires actual_state.subject != forecast subject")
            if self.actual_value is not None or any(v is not None for v in scored_fields) or any(v is not None for v in interval_fields):
                raise EvaluationError("SUBJECT_MISMATCH records must not carry scored/actual fields")
            if self.reason is None:
                raise EvaluationError("SUBJECT_MISMATCH requires a reason")
        elif self.status is EvaluationStatus.ABSTAINED:
            if self.actual_value is not None or any(v is not None for v in scored_fields) or any(v is not None for v in interval_fields):
                raise EvaluationError("ABSTAINED records must not carry scored/actual fields")
            if self.reason is None:
                raise EvaluationError("ABSTAINED requires a reason")
        else:  # UNMATCHED / AMBIGUOUS — recorded, never scored
            if self.actual_value is not None or any(v is not None for v in scored_fields) or any(v is not None for v in interval_fields):
                raise EvaluationError(f"{self.status.value} records must not carry scored/actual fields")
            if self.reason is None:
    ...
    def to_canonical_dict(self, *, include_digest: bool = True) -> Dict[str, Any]:
        data: Dict[str, Any] = {
            ...
            "actual_state": (self.actual_state.to_canonical_dict() if self.actual_state is not None else None),
    ...
    def digest(self) -> str:
        """Deterministic ``sha256:`` content identity/integrity over all record fields,
        including the embedded actual state. NOT a signature or authenticity proof."""
    ...
    @classmethod
    def from_dict(cls, data: Dict[str, Any]) -> "ForecastEvaluationRecord":
        """Reconstruct a record from its canonical dict, re-running full validation.

        Reconstruction rebuilds the embedded actual state and subject and constructs the
        frozen record, so ``__post_init__`` re-verifies the actual-state binding — a
        deserialization boundary cannot bypass the relationship."""
        if not isinstance(data, dict):
            raise EvaluationError("record data must be a mapping")
        actual_state = (CanonicalCapacityState.from_dict(data["actual_state"])
                        if data.get("actual_state") is not None else None)
        return cls(
            schema_version=data["schema_version"],
            forecast_evidence_digest=data["forecast_evidence_digest"],
            subject=CapacitySubject.from_dict(data["subject"]),
            actual_state=actual_state,
            actual_state_digest=data.get("actual_state_digest"),
            target=data["target"],
            unit=data.get("unit"),
            horizon_seconds=data["horizon_seconds"],
            forecast_cutoff=data["forecast_cutoff"],
            forecast_for=data["forecast_for"],
            actual_event_time=data.get("actual_event_time"),
            match_tolerance_seconds=data["match_tolerance_seconds"],
            match_delta_seconds=data.get("match_delta_seconds"),
            status=EvaluationStatus(data["status"]),
            reason=data.get("reason"),
            point_forecast=data.get("point_forecast"),
            actual_value=data.get("actual_value"),
            signed_error=data.get("signed_error"),
            absolute_error=data.get("absolute_error"),
            squared_error=data.get("squared_error"),
...
def _unscored(evidence, status, reason, *, match_tolerance_seconds, actual_state=None,
              match_delta_seconds=None) -> ForecastEvaluationRecord:
    """Internal helper: build a typed unscored record, embedding the bound actual state
    (its digest is re-derived and validated by ``__post_init__``)."""
...
def unscored_record(
    ...
    *,
    ...
    return _unscored(evidence, status, reason, match_tolerance_seconds=match_tolerance_seconds,
                     actual_state=actual_state, match_delta_seconds=match_delta_seconds)
...
def evaluate_forecast(
    ...
    *,
    ...
    An EVALUATED record EMBEDS the actual canonical state; the digest, actual value, unit,
    and subject binding are DERIVED from it (never a caller-supplied scalar) and re-verified
    at construction. ``None`` means no candidate actual was available.
    """
    ...
    # Abstention: recorded, not scored (bound to the forecast, not an actual).
    if fc.is_abstained:
        return ForecastEvaluationRecord(
            actual_state=None, actual_state_digest=None, unit=None,
            actual_event_time=None, match_delta_seconds=None,
    ...
    if actual_state is None:
        return _unscored(evidence, EvaluationStatus.UNMATCHED, REASON_NO_ACTUAL,
    ...
    if actual_state.subject != fc.subject:
        return _unscored(evidence, EvaluationStatus.SUBJECT_MISMATCH, REASON_ACTUAL_SUBJECT_DIFFERS,
                         match_tolerance_seconds=match_tolerance_seconds, actual_state=actual_state)

    delta = (_as_utc(actual_state.observed_at) - _as_utc(fc.forecast_for)).total_seconds()
    sample = extract_sample(actual_state, fc.target)
    if sample is None:
        return _unscored(evidence, EvaluationStatus.UNMATCHED, REASON_ACTUAL_MISSING_TARGET,
                         match_tolerance_seconds=match_tolerance_seconds,
                         actual_state=actual_state, match_delta_seconds=delta)
    # Unit must match the forecast's unit — never silently compare across units.
    if sample.unit != fc.unit:
        return _unscored(evidence, EvaluationStatus.UNMATCHED, REASON_ACTUAL_UNIT_MISMATCH,
                         match_tolerance_seconds=match_tolerance_seconds,
                         actual_state=actual_state, match_delta_seconds=delta)
    if abs(delta) > match_tolerance_seconds:
        return _unscored(evidence, EvaluationStatus.UNMATCHED, REASON_ACTUAL_OUTSIDE_TOLERANCE,
                         match_tolerance_seconds=match_tolerance_seconds,
                         actual_state=actual_state, match_delta_seconds=delta)

    point = float(fc.point_estimate)
    actual_value = float(sample.value)     # derived from the embedded state
