...
def test_holdout_is_disjoint_from_dev_corpus():
    from ugence_storygraph.evaluation import final_eval
    assert final_eval.dev_corpus_excludes_holdout() is True


def test_holdout_hash_computable_without_evaluation():
    from ugence_storygraph.evaluation import story_corpus_v2 as S
    h = S.holdout_hashes()
...
def test_official_final_refuses_development_mode(tmp_path):
    from ugence_storygraph.evaluation import freeze, final_eval
...
def test_all_three_runs_preserved():
    from ugence_storygraph.evaluation.prior_runs import RUN_1, RUN_2, RUN_3, PRIOR_RUNS
...
def test_holdout_final_gates_pass_recorded():
    from ugence_storygraph.evaluation.prior_runs import RUN_3
    m = RUN_3["metrics"]
