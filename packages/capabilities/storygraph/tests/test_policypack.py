...
from ugence_storygraph.policypack import (
    business_form as BF, compiler, event_mapping, lifecycle, providers_mapping,
    reference, replay, schema,
)
from ugence_storygraph.evaluation import evidence_chain as EC
from ugence_storygraph.evaluation import freeze

import ugence_storygraph.policypack as _pp

HERE = os.path.dirname(os.path.abspath(__file__))
_PP_DIR = os.path.dirname(_pp.__file__)
FIXTURE = os.path.join(_PP_DIR, "fixtures", "account_takeover_replay.json")
...
def test_evidence_commit_paths_verified():
    ok = EC.verify_evidence_commit_paths(
        ["packages/capabilities/storygraph/src/ugence_storygraph/evaluation/evidence/run4.json"])
    bad = EC.verify_evidence_commit_paths(
        ["packages/capabilities/storygraph/src/ugence_storygraph/storygraph.py"])
...
def test_json_schema_contract_matches_python_validator():
    path = os.path.join(_PP_DIR, "schemas", "storypolicypack.schema.json")
