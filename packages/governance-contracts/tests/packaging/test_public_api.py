"""C6/§15 — the documented public API agrees with the actual package surface.

Rebuilds the public-API description straight from the installed
``ugence_governance_contracts.api`` module and asserts it is byte-equal (after
normalization) to the committed ``public_api.json``. This catches an accidental
export addition/removal, an enum-value drift, or a dataclass-field change that
was not reflected in the machine-readable contract snapshot.

Also asserts the PEP 561 ``py.typed`` marker is present in the package (the
package advertises typing support), so it will ship in the wheel.
"""

from __future__ import annotations

import dataclasses
import enum
import json
import pathlib

import ugence_governance_contracts as g
from ugence_governance_contracts import api

_PKG_ROOT = pathlib.Path(g.__file__).resolve().parent
# tests/packaging/ -> tests/ -> packages/governance-contracts/
_DIST_ROOT = pathlib.Path(__file__).resolve().parents[2]
_PUBLIC_API_JSON = _DIST_ROOT / "public_api.json"


def _kind(obj) -> str:
    if isinstance(obj, type):
        if issubclass(obj, enum.Enum):
            return "enum"
        if issubclass(obj, Exception):
            return "exception"
        if dataclasses.is_dataclass(obj):
            return "dataclass"
        if getattr(obj, "_is_protocol", False):
            return "protocol"
        return "class"
    return type(obj).__name__


def _actual_surface() -> dict:
    symbols: dict[str, dict] = {}
    for name in sorted(api.__all__):
        obj = getattr(api, name)
        entry: dict = {"kind": _kind(obj)}
        if isinstance(obj, type) and issubclass(obj, enum.Enum):
            entry["values"] = [m.value for m in obj]
        elif isinstance(obj, type) and dataclasses.is_dataclass(obj):
            entry["fields"] = [f.name for f in dataclasses.fields(obj)]
        elif isinstance(obj, type) and issubclass(obj, Exception):
            fc = getattr(obj, "failure_class", None)
            entry["failure_class"] = fc.value if fc is not None else None
        symbols[name] = entry
    return {
        "distribution": "ugence-governance-contracts",
        "namespace": "ugence_governance_contracts",
        "package_version": g.__version__,
        "contract_version": g.CONTRACT_VERSION,
        "curated_api_module": "ugence_governance_contracts.api",
        "symbols": symbols,
    }


def test_documented_public_api_matches_actual():
    documented = json.loads(_PUBLIC_API_JSON.read_text())
    documented.pop("note", None)
    actual = _actual_surface()
    # Compare metadata fields.
    for key in ("distribution", "namespace", "package_version",
                "contract_version", "curated_api_module"):
        assert documented[key] == actual[key], key
    # Compare the full symbol table (names, kinds, enum values, dataclass fields,
    # error failure-classes) exactly.
    assert documented["symbols"] == actual["symbols"]


def test_curated_api_names_match_module_all():
    documented = json.loads(_PUBLIC_API_JSON.read_text())
    assert set(documented["symbols"]) == set(api.__all__)


def test_py_typed_marker_present():
    assert (_PKG_ROOT / "py.typed").is_file(), "PEP 561 py.typed marker missing"
