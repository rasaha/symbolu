...
def test_the_distribution_is_named_and_versioned_exactly():
    ...
    assert 'name = "ugence-cloud-scaling-credential-broker"' in text
    assert PKG_DIR.name == "ugence_cloud_scaling_credential_broker" and pkg.__version__ == "0.1.0"


def test_the_public_api_manifest_equals_the_live_package_surface():
    manifest = json.loads(PUBLIC_API.read_text(encoding="utf-8"))
    assert manifest["distribution"] == "ugence-cloud-scaling-credential-broker"
...
def test_py_typed_and_src_layout():
    ...
    assert 'ugence_cloud_scaling_credential_broker = ["py.typed"]' in text
...
def test_the_readme_and_changelog_state_the_posture():
    ...
    assert "not execution" in readme and "5x" in readme and "fifteen minutes" in readme
