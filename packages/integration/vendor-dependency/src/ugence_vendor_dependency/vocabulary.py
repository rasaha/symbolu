...
**Structured, not opaque — and this package is where that distinction was decided.**
``PUB-2`` authorized reusing **this package's own** ``policy_ref`` for the vocabulary
reference *only if* it normatively identified the exact vocabulary, and determined that
it does not: ``VR-4`` rules ``policy_ref`` an opaque string this package "must not
resolve, verify, interpret or fetch", and a string forbidden to be interpreted cannot
normatively identify anything. It references *some* policy, with no guarantee which. So
the binding is a separate field beside it rather than a second meaning loaded onto it —
``policy_ref`` is the shape to copy, not the carrier.
A binding here is the opposite: three named parts — identifier, exact version and the
specification's digest — so it names one immutable published specification and no
other. **A bare version string is insufficient** (``PUB-2``), which is why the digest
is required rather than encouraged.

**The same type, spelled the same way, in each package that needs one.** It is copied
rather than shared, exactly as ``_canon.py`` is: ``VV-A`` put the binding on the record
so that no new neutral type enters ``governance-contracts`` and ``LP-5`` is not
pre-empted by accident. Four identical copies is the price of that ruling, and it is
the price the ruling chose.
...
does not say what a member means, and ``VR-3`` still forbids this package from having an
opinion: no grade, enum, taxonomy, ordering, severity, score, dominance **or implied
eligibility**. The last of those is why the vocabulary this package cites is called
``vendor-dependency-assessment-state`` and not ``…-permission``: ``PUB-1a`` renamed it
because a name is what a reader sees first, and ``LV-C`` refused a permission ladder for
exactly that reason. Citing it confers nothing, and the published specification records
``eligibility`` as explicitly absent rather than leaving it to be inferred.
...
regular expression. Both come from ``data-use-admission``, whose payload guard forbids
the identifier ``content`` and the ``re`` import outright; the name is kept identical
across the four packages so a reader meets one spelling rather than four.
"""
