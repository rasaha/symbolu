...
def test_the_distribution_is_named_and_versioned_exactly():
    ...
    assert PKG_DIR.name == "ugence_vendor_dependency" and pkg.__version__ == "0.3.0"
    assert pkg.CONTRACT_VERSION == "vendor_dependency.v2"
    # The historical shape is named, not deleted: records written under it stay readable
    # and keep the digest and derived id they were stored with (VV-B).
    assert pkg.LEGACY_CONTRACT_VERSION == "vendor_dependency.v1"


def test_the_public_api_manifest_equals_the_live_package_surface():
    manifest = json.loads(PUBLIC_API.read_text(encoding="utf-8"))
    assert manifest["distribution"] == "ugence-vendor-dependency"
    assert manifest["namespace"] == "ugence_vendor_dependency"
...
def test_py_typed_and_src_layout():
    ...
    assert 'ugence_vendor_dependency = ["py.typed"]' in text
...
def test_the_readme_and_changelog_state_the_posture():
    ...
    assert "maturity ceiling" in readme and "not a permission" in readme
