...
def test_phase_5a_is_at_the_version_this_package_was_pinned_against():
    ...
    ``0.2.0`` since 5B-1, which added the required policy coordinate to the candidate;
    ``0.3.0`` since 5B-2 (R-9) and ``0.4.0`` since R-12b, neither of which moves a digest or a
    schema identifier. This package's source is unchanged by all of them and its own version
    does not move; what moves is the fixture chain it verifies against. The assertion stays
    exact rather than becoming a range: a version that drifted without this suite being
    re-measured is the thing worth catching.
    """
