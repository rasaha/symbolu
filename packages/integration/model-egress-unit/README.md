...
| Custody port (0.2.0) | `ModelCredentialCustodyPort` mirroring the credential broker's port; a `CredentialLease` whose secret is reachable only through `use(consumer, now=…)` — a slotted plain class with no instance `__dict__`, so `vars()`, `asdict()`/`astuple()`, `repr`/`str`/format, logging, JSON defaults, copying and pickling cannot reach the secret, and equality and hashing never read it; every materialization audited as identifiers and digests; the only adapter shipped is the inert `ReferenceCustodyAdapter`, never production-authoritative, refused in production |
...
| Execution posture (0.5.1) | LP-8 (`ADR_UGENCE_LIVE_MODEL_PROVIDER_COMMISSIONING.md` §0.7): `infrastructure.ExecutionPosture` and `check_execution_posture` accept only `DEPLOYED_MEU_INSTANCE` under the instance's non-human workload identity through `PRODUCTION_FORM_SECRET_MANAGER_ADAPTER` in a non-production environment; each of `FORBIDDEN_CREDENTIAL_HOLDERS` (developer machine, CI runner, browser, shared hosting environment, production business workflow) is refused by name, and any `CI_ENVIRONMENT_MARKERS` variable refuses the process first whatever it claims; `sequence_complete` is equality with the authorized `max_calls`, never fewer; `NON_PRODUCTION_VALIDATION_OUTCOME_SCOPE` states that success authorizes no production commissioning |
| Identity mechanisms (0.6.0) | `custody.PRODUCTION_AUTHORITATIVE_IDENTITY_KINDS` and `CustodyIdentity.production_authority_refusal()`: `native_gcp_workload_identity` joins `workload_identity_federation` as production-authoritative (commissioning ADR §0.8), bound to the exact designated service-account resource under an attested designation digest; `application_default_credentials` is a discovery order and is refused by name, as are a human or default-compute identity, a fake or emulator identity and (at construction) a downloaded service-account key; `is_service_account_resource` is the shared shape check |
