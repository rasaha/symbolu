...
from _fixtures import OBSERVER, TENANT, observation, observer_signer
...
def _attestation():
    return ea.mint_effect_attestation(
        observation(), signer=observer_signer(), tenant_id=TENANT, attested_at=ATTESTED_AT
    )


def _snapshot_document():
    """A signed snapshot carrying the observer's anchor at its effect coordinate."""

    anchor = observer_signer().trust_anchor(
...
def _resolver(document=None, **kw):
    """A signed-snapshot resolver over the observer's anchor unless given a document."""
...
def test_the_set_reason_map_is_one_to_one_and_omits_instant_required():
    assert ea.TRUST_ANCHOR_SET_REASONS == {
    ...
    values = list(ea.TRUST_ANCHOR_SET_REASONS.values())
    assert len(set(values)) == len(values)
    assert (TrustedEvidenceRefusalReason.TRUSTED_EVIDENCE_TRUST_ANCHOR_SET_INSTANT_REQUIRED
            not in ea.TRUST_ANCHOR_SET_REASONS)


def test_no_cached_answer_follows_a_stale_one():
    resolver = _resolver()
    assert _verify(resolver, as_of=FRESH).outcome is ea.EffectAttestationVerificationOutcome.VERIFIED
...
def test_a_bad_instant_raises_at_the_caller_seam_rather_than_minting_a_reason():
    verifier = ea.Ed25519EffectAttestationVerifier(trust_anchor_resolver=_resolver(), production_mode=True)
    for bad in (None, datetime(2026, 9, 6, 12, 0), "2026-09-06T12:00:00Z", 1_757_160_000):
        with pytest.raises(ea.EffectAttestationContractError):
            verifier.verify(attestation=_attestation(), expected_role=OBSERVER,
                            expected_tenant_id=TENANT, expected_observation=observation(), as_of=bad)
...
def test_a_resolver_whose_snapshot_failed_to_load_is_admitted_and_refuses_everything():
    ...
    verifier = ea.Ed25519EffectAttestationVerifier(trust_anchor_resolver=broken, production_mode=True)
...
def test_a_resolver_declaring_no_posture_is_still_refused_at_composition():
    ...
    for resolver in (_Silent(), _Truthy()):
        with pytest.raises(ea.EffectAttestationConfigurationError, match="exact bool"):
            ea.Ed25519EffectAttestationVerifier(trust_anchor_resolver=resolver, production_mode=True)
    assert ea.declares_production_posture(_Silent()) is False
    assert ea.declares_production_posture(_Truthy()) is False
    assert ea.declares_production_posture(rc.resolver(None)) is True
    assert ea.resolver_serves_production(rc.resolver(None)) is False
    assert ea.resolver_serves_production(_resolver()) is True


def test_the_reference_directory_is_still_refused_in_production():
    static = ea.StaticTrustAnchorDirectory(
        [observer_signer().trust_anchor(trust_anchor_set_id="s", trust_anchor_set_version="1")],
        trust_anchor_set_id="s", trust_anchor_set_version="1",
    )
    with pytest.raises(ea.EffectAttestationConfigurationError, match="REFERENCE"):
        ea.Ed25519EffectAttestationVerifier(trust_anchor_resolver=static, production_mode=True)

    class _Claiming(ea.StaticTrustAnchorDirectory):
        is_production_authoritative = True

    claiming = _Claiming([], trust_anchor_set_id="s", trust_anchor_set_version="1")
    with pytest.raises(ea.EffectAttestationConfigurationError, match="REFERENCE"):
        ea.Ed25519EffectAttestationVerifier(trust_anchor_resolver=claiming, production_mode=True)
...
def test_deny_all_keeps_its_ratified_reason_and_is_exempt_from_the_gate():
    ...
    deny = ea.DenyAllTrustAnchorDirectory()
    assert ea.declares_production_posture(deny) is False
...
def test_the_reference_grade_path_is_unchanged_by_the_gate():
    ...
    static_ok = ea.Ed25519EffectAttestationVerifier(
        trust_anchor_resolver=ea.StaticTrustAnchorDirectory(
            [observer_signer().trust_anchor(trust_anchor_set_id="s", trust_anchor_set_version="1")],
            trust_anchor_set_id="s", trust_anchor_set_version="1"),
...
# --------------------------------------------------------------------------- #
def test_the_two_members_are_appended_and_nothing_earlier_moved():
    ...
    assert names[:11] == [
        ...
        "UNSUPPORTED_ENCODING", "ROLE_MISMATCH", "WRONG_TENANT", "OBSERVATION_MISMATCH",
        "ANCHOR_UNKNOWN",
    ]
...
def test_no_verified_result_is_reachable_through_a_trust_state_reason():
    for reason in (R.ANCHOR_SET_STALE, R.ANCHOR_SET_UNAVAILABLE):
        result = ea.EffectAttestationVerificationResult(
            outcome=ea.EffectAttestationVerificationOutcome.REFUSED,
            refusal_reason=reason,
            attester_role=OBSERVER,
            attester_identity="observer-omega",
            attester_key_id="observer-key-1",
            tenant_id=TENANT,
            observation_digest=ea.observation_digest(observation()),
            signing_payload_digest=None,
            anchor_coordinate_digest=ea.anchor_coordinate_digest(
                ea.effect_attester_coordinate(role=OBSERVER, attester_identity="observer-omega",
                                              attester_key_id="observer-key-1")),
