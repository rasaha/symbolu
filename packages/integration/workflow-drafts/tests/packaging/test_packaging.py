...
def test_the_distribution_is_named_and_versioned_exactly():
    ...
    assert 'name = "ugence-workflow-drafts"' in text
    assert PKG_DIR.name == "ugence_workflow_drafts" and pkg.__version__ == "0.1.0"


def test_the_public_api_manifest_equals_the_live_package_surface():
    manifest = json.loads(PUBLIC_API.read_text(encoding="utf-8"))
    assert manifest["distribution"] == "ugence-workflow-drafts"
...
def test_py_typed_and_src_layout():
    ...
    assert 'ugence_workflow_drafts = ["py.typed"]' in text
...
def test_the_readme_and_changelog_state_the_posture():
    ...
    assert "never" in lowered and "presented_unproven" in lowered
    assert "draft" in lowered and "confers nothing" in lowered
