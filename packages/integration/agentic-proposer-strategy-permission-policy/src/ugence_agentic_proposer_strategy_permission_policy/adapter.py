"""The strategy-permission ``PolicyFamilyAdapter``.
...
from .policy import StrategyPermissionPolicy, StrategyPermissionPolicyMetadata
...
def strategy_permission_coordinate(metadata: object) -> PolicyCoordinate:
    """Map this family's metadata envelope onto a family-neutral coordinate."""

    if not isinstance(metadata, StrategyPermissionPolicyMetadata):
        raise PolicyAuthorityRequestError(
            "strategy_permission_coordinate requires a StrategyPermissionPolicyMetadata"
        )
    ...
    if metadata.policy_family != STRATEGY_PERMISSION_POLICY_FAMILY:
        raise PolicyAuthorityRequestError(
            f"metadata reports policy family {metadata.policy_family!r}, not "
            f"{STRATEGY_PERMISSION_POLICY_FAMILY!r}"
        )
    return PolicyCoordinate(
        policy_family=STRATEGY_PERMISSION_POLICY_FAMILY,
...
class StrategyPermissionPolicyFamilyAdapter:
    """Registers the strategy-permission family with the shared authority."""

    @property
    def adapter_id(self) -> str:
        return STRATEGY_PERMISSION_ADAPTER_ID
    ...
    def recognizes(self, artifact: object) -> bool:
        ...
        return type(artifact) is StrategyPermissionPolicy

    def coordinate_for(self, reference: object) -> Optional[PolicyCoordinate]:
        if not isinstance(reference, StrategyPermissionPolicyMetadata):
            return None
        return strategy_permission_coordinate(reference)

    # ------------------------------------------------------------------
    # Description
    # ------------------------------------------------------------------
    def describe(self, artifact: object) -> PolicyArtifactDescriptor:
        if type(artifact) is not StrategyPermissionPolicy:
            raise UnsupportedPolicyArtifactError(
                f"{type(artifact).__name__!r} is not a StrategyPermissionPolicy"
            )

        metadata = getattr(artifact, "metadata", None)
        if not isinstance(metadata, StrategyPermissionPolicyMetadata):
            raise UnsupportedPolicyArtifactError(
                "StrategyPermissionPolicy must carry a StrategyPermissionPolicyMetadata "
                "envelope"
            )
        ...
        return PolicyArtifactDescriptor(
            adapter_id=STRATEGY_PERMISSION_ADAPTER_ID,
            ...
            policy_type=STRATEGY_PERMISSION_POLICY_TYPE,
            coordinate=strategy_permission_coordinate(metadata),
    ...
    def _canonical_projection(artifact: Any) -> dict:
        ...
        if not isinstance(metadata, dict) or "content_digest" not in metadata:
            raise UnsupportedPolicyArtifactError(
                "a strategy-permission policy must carry a metadata envelope with a "
