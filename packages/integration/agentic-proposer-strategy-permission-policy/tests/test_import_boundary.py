...
* the **role projection** — a repository-wide scan in the Agentic Proposer's own
  suite refuses those substrings in every ``.py`` under ``packages/`` outside that
  capability, docstrings and comments included. This distribution never receives
  a role: the resolver boundary carries a reference, not an identity.
...
def test_no_module_imports_a_forbidden_neighbour(module):
    ...
    Each would turn a declarative artifact into something that resolves a
    mechanism for itself, which is precisely what this family is excluded from.
    """
...
def test_every_import_is_stdlib_or_a_declared_dependency(module):
    declared = {"ugence_policy_authority", "ugence_agentic_proposer"}
    stdlib = {"__future__", "dataclasses", "datetime", "re", "typing"}
...
def test_the_distribution_name_is_not_a_shared_contract_name():
    ...
    pyproject = (DIST_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert 'name = "ugence-agentic-proposer-strategy-permission-policy"' in pyproject
    assert "contract" not in "ugence-agentic-proposer-strategy-permission-policy"
