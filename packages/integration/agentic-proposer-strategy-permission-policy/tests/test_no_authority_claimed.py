...
The scans here are the family-side half of the proof obligation the design
records: no compute authorization, no consequential execution authority, and no
operational disposition for a permission failure — which remains deliberately
unruled, so nothing here may name one.
...
contains ``EXPIRED``, ``UNSUPPORTED`` and ``SUPPORTED``. A guard that compared
...
SRC = DIST_ROOT / "src" / "ugence_agentic_proposer_strategy_permission_policy"
...
#: Compute and provisioning claims. ``S2B-D2=A`` bars capability tiers, and
#: permission grants no compute, quota or evidence access.
...
#: Role-lifecycle and authorization verbs. Permission is role-level and the role
#: references this policy; nothing here mints, activates, ends or authorizes.
...
def _string_literals(path: pathlib.Path) -> list:
    ...
    * **``__all__`` entries** — an export list is a list of names, not message
      text. ``PLACEHOLDER_CONTENT_DIGEST`` contains ``HOLD`` under the uppercased
      substring rule and is neither an exception name nor a message template; the
      ratified rule bars a reserved term from what a caller *reads*, not from
      every string that happens to appear in the module.
    """
...
def test_the_exported_surface_holds_no_resolver_registry_or_mapping():
    """The family is the artifact side. Resolution is a separate distribution."""

    for name in family.__all__:
        lowered = name.lower()
        for forbidden in ("resolver", "resolve", "registry", "signer", "verifier"):
