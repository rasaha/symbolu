...
def test_the_distribution_is_named_and_versioned_exactly():
    ...
    assert 'name = "ugence-incident-response"' in text
    assert PKG_DIR.name == "ugence_incident_response" and pkg.__version__ == "0.1.0"


def test_the_public_api_manifest_equals_the_live_package_surface():
    manifest = json.loads(PUBLIC_API.read_text(encoding="utf-8"))
    assert manifest["distribution"] == "ugence-incident-response"
...
def test_py_typed_and_src_layout():
    ...
    assert 'ugence_incident_response = ["py.typed"]' in text
...
def test_the_readme_and_changelog_state_the_posture():
    ...
    assert "never revokes" in readme and "records only" in readme
    assert "does not restart the pilot" in readme
    assert "not an orchestrator" in readme and "auditreference" in readme
