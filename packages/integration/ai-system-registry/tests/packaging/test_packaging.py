...
def test_the_distribution_is_named_and_versioned_exactly():
    ...
    assert 'name = "ugence-ai-system-registry"' in text
    assert PKG_DIR.name == "ugence_ai_system_registry" and pkg.__version__ == "0.1.0"


def test_the_public_api_manifest_equals_the_live_package_surface():
    manifest = json.loads(PUBLIC_API.read_text(encoding="utf-8"))
    assert manifest["distribution"] == "ugence-ai-system-registry"
...
def test_py_typed_and_src_layout():
    ...
    assert 'ugence_ai_system_registry = ["py.typed"]' in text
...
def test_the_readme_and_changelog_state_the_posture():
    ...
    assert "assessedsystembinding" in readme and "portfolio ledger" in readme
