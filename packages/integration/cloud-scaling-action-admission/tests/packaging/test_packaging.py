...
def test_the_distribution_is_named_and_versioned_exactly():
    ...
    assert 'name = "ugence-cloud-scaling-action-admission"' in text
    assert PKG_DIR.name == "ugence_cloud_scaling_action_admission"
...
def test_the_public_api_manifest_equals_the_live_package_surface():
    ...
    assert manifest["distribution"] == "ugence-cloud-scaling-action-admission"
...
def test_py_typed_and_src_layout():
    ...
    assert 'ugence_cloud_scaling_action_admission = ["py.typed"]' in text
...
def test_the_readme_and_changelog_state_the_posture():
    ...
    assert "not execution" in readme and "5c" in readme
