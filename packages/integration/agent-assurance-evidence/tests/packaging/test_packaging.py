...
def test_the_distribution_is_named_and_versioned_exactly():
    ...
    assert 'name = "ugence-agent-assurance-evidence"' in text
    assert PKG_DIR.name == "ugence_agent_assurance_evidence" and pkg.__version__ == "0.1.0"
    assert pkg.CONTRACT_VERSION == "agent_assurance_evidence.v1"


def test_the_public_api_manifest_equals_the_live_package_surface():
    manifest = json.loads(PUBLIC_API.read_text(encoding="utf-8"))
    assert manifest["distribution"] == "ugence-agent-assurance-evidence"
    assert manifest["namespace"] == "ugence_agent_assurance_evidence"
...
def test_py_typed_and_src_layout():
    ...
    assert 'ugence_agent_assurance_evidence = ["py.typed"]' in text
...
def test_the_readme_and_changelog_state_the_posture():
    ...
    for borrowed in ("assessedsystembinding", "evidencereference", "assurancefindinglabel"):
        assert borrowed in readme, borrowed
    for ruling in ("new_record_type", "uninterpreted_label", "both"):
        assert ruling in readme, ruling
    assert "sole evidence identity" in readme
    assert "reference_grade_contract_only" in readme and "not a verdict" in readme
