...
for _path in (
    PKG / "src",
    REPO / "packages" / "policy-authority" / "src",
...
import ugence_change_effect_policy as pkg  # noqa: E402
...
def build() -> dict:
    return {
        "distribution": "ugence-change-effect-policy",
        "namespace": "ugence_change_effect_policy",
        "package_version": pkg.__version__,
        "curated_api_module": "ugence_change_effect_policy",
        "note": (
            "Machine-readable snapshot of the curated public API "
            "(ugence_change_effect_policy.__all__). tests/packaging/test_packaging.py asserts "
