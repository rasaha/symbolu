...
def test_the_distribution_is_named_and_versioned_exactly():
    ...
    assert 'name = "ugence-cloud-scaling-envelope-issuance"' in text
    assert PKG_DIR.name == "ugence_cloud_scaling_envelope_issuance"
...
def test_the_public_api_manifest_equals_the_live_package_surface():
    ...
    assert manifest["distribution"] == "ugence-cloud-scaling-envelope-issuance"
    assert manifest["namespace"] == "ugence_cloud_scaling_envelope_issuance"
...
def test_py_typed_and_src_layout():
    ...
    assert 'ugence_cloud_scaling_envelope_issuance = ["py.typed"]' in text
    assert "Typing :: Typed" in text
    assert PKG_DIR.parent.name == "src" and 'where = ["src"]' in text
...
def test_the_readme_and_changelog_exist_and_state_the_posture():
    readme = (PROJECT / "README.md").read_text(encoding="utf-8").lower()
    assert (PROJECT / "CHANGELOG.md").exists()
    assert "not execution" in readme and "5b-4" in readme
