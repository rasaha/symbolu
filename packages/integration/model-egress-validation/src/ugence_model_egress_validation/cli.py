...
from ugence_model_egress_unit import COMMISSIONING_STATUS, STEP8_REQUIRED_VALUES
...
def _undesignated(designation: dict) -> List[str]:
    step8 = designation.get("step8_required_values", {})
    names = [k for k, v in step8.items() if isinstance(v, str) and v.startswith("UNDESIGNATED")]
    if step8.get("independently_checked_by") in (None, ""):
        names.append("independently_checked_by")
    return names
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.command == "live":
        missing = _undesignated(records["designation"])
        print(f"live verifier refused: commissioning is {COMMISSIONING_STATUS}; no live transport exists in any distribution.")
        print(f"{len(missing)} step-8 designations are undesignated or unchecked (LP-7 ruling 12):")
        for name in missing:
            print(f"  - {name}")
        print("Supplying them closes only the infrastructure-designation blocker; the first live synthetic validation "
              "needs the owner's separate explicit authorization.")
        return 2
    ...
    print(f"infrastructure-dependent rows not executed: {report['infrastructure_dependent_rows_not_executed']}; "
          f"required step-8 values: {len(STEP8_REQUIRED_VALUES)} fields, none supplied here")
