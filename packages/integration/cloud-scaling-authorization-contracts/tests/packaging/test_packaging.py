...
def test_version_is_the_declared_version():
    """``0.4.0`` since R-12: the builder refuses a pairing it used to accept.

    5B-1 took this to ``0.2.0`` — the candidate gained a required field and its digest moved.
    Neither 5B-2 nor R-12 moves a digest or a schema identifier; what each changes is that a
    candidate constructible at the previous version may be refused at this one — R-9 at
    ``0.3.0``, the attestation-instant coherence pair at ``0.4.0``. Pre-1.0 that is still a
    minor bump, and it is the kind a consumer most needs told: nothing they pin looks
    different, and the same inputs now raise.
    """
