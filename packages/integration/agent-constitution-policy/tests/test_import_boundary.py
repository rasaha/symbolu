...
* the **role projection** — a repository-wide scan in the Agentic Proposer's own
  ...
  a role: ``governed_role_refs`` carries references, not identities, and the
  conformance boundary presents plain facts.
...
def test_the_distribution_name_is_not_a_shared_contract_name():
    ...
    pyproject = (DIST_ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert 'name = "ugence-agent-constitution-policy"' in pyproject
    assert "contract" not in "ugence-agent-constitution-policy"
