"""SCR-1: malformed, small-order, non-canonical and identity public points refuse
...
def test_a_small_order_or_non_canonical_anchor_cannot_even_be_constructed(name):
    fields = {f.name: getattr(anchor_of(engine_signer()), f.name)
...
def test_a_record_forged_around_construction_is_refused_at_key_admission(name):
    ...
    forged = dataclasses.replace(signed(), signature=KEYLESS_FORGERY)
    result = verify(verifier(anchor), forged)
    assert result.outcome is ra.ComparisonResultVerificationOutcome.REFUSED
    assert result.refusal_reason is R.KEY_MATERIAL_INVALID, (name, result.detail)
    with pytest.raises(Exception):
        ra.anchor_verification_key(anchor)  # the package-level admission refuses too
...
def test_a_genuine_key_passes_the_point_check_and_a_forgery_still_fails_under_it():
    anchor = anchor_of(engine_signer())
    assert ra.anchor_verification_key(anchor) is not None
    forged = dataclasses.replace(signed(), signature=KEYLESS_FORGERY)
...
def test_the_malleable_s_plus_l_signature_is_refused():
    ...
    result = verify(verifier(anchor_of(engine_signer())), dataclasses.replace(att, signature=malleated))
    assert result.refusal_reason is R.SIGNATURE_INVALID


def test_the_package_imports_no_cryptographic_library_of_its_own():
    """The D-41 pair is reached only through TEA; nothing here is forked."""
