"""The Trusted Evidence Authority's trust anchors, reused unchanged (SCR-1).
...
is the coordinate a comparison-result signer is resolved at — its capability is chosen
by the signer's **role**, never by a caller — and the production posture a
...
from .roles import ComparisonResultAttesterRole, capability_for_role
...
def comparison_result_signer_coordinate(
    *, role: ComparisonResultAttesterRole, signer_identity: str, signer_key_id: str
) -> TrustAnchorCoordinate:
    """The exact coordinate a signer under ``role`` is resolved at.

    The capability comes from the role and from nowhere else: a caller cannot
    ask for an engine signature to be checked under any other capability,
    and an anchor filed under the wrong role is simply not at this coordinate.
    """

    return TrustAnchorCoordinate(
        authority_id=require_canonical_identifier("signer_identity", signer_identity),
        key_id=require_canonical_identifier("signer_key_id", signer_key_id),
...
def anchor_verification_key(anchor: TrustAnchorRecord) -> TrustedEvidenceVerificationKey:
    ...
    libsodium point check (reused from TEA, never re-implemented), through TEA and never re-implemented here.
...
def require_production_resolver(resolver: object) -> object:
    ...
    if not declares_production_posture(resolver):
        raise _ConfigError(
            ...
            f"cannot supply a comparison engine's key (got {type(resolver).__name__}). A "
