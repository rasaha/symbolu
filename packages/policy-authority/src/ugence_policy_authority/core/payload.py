...
def supersession_signing_payload(
    *,
    ...
    """Return the exact bytes a policy-version supersession signature covers.

    ``coordinate`` is the **predecessor** — the version that stops resolving —
    and ``successor_coordinate`` is the version issued over it. Both are
    complete, so a supersession can never reach a version it does not name.
    """

    body = {
        "domain": SUPERSESSION_SIGNING_DOMAIN,
        "authority_protocol": AUTHORITY_PROTOCOL,
        "authority_protocol_version": AUTHORITY_PROTOCOL_VERSION,
        "authority_protocol_id": AUTHORITY_PROTOCOL_ID,
        "canonicalization": CANONICALIZATION_VERSION,
        "supersession_id": supersession_id,
        "superseding_authority_id": superseding_authority_id,
        "key_id": key_id,
        "signature_alg": signature_alg,
        "superseded_at": superseded_at,
        "successor": _coordinate_fields(successor_coordinate),
    }
    body.update(_coordinate_fields(coordinate))
    return _framed(SUPERSESSION_SIGNING_DOMAIN, body)
