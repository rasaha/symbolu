...
This distribution ships ``0.2.2``: BR-2B's non-authoritative lifecycle kernel,
...
(non-authoritative lifecycle kernel), **BR-2C-0 ``0.2.1`` and ``0.2.2``** (BR-2C's
contracts, no capability), BR-2C ``0.3.0`` (cryptographic trust authority), BR-2D ``0.4.0``
(durable registry authority) and BR-2E ``0.5.0`` (production composition and
operations). Nothing beyond ``0.2.2`` exists yet, here or anywhere else in this
repository.
...
106 at ``0.2.1``, and 106 → 107 at ``0.2.2`` under D-34 — is recorded by a
version that moved with it, without taking ``0.3.0``,
which §35.1 defines as the **audited verifier** and which would unlock twelve
