...
"""
    ...
    async def generate_stream_async(
        ...
        context: Optional[str] = None,
        goal_state: Optional[Any] = None,
    ) -> AsyncIterator[Union[str, GenerationResult]]:
        """
        Async streaming variant of :meth:`generate_stream`.

        Uses the adapter's ``call_stream_async()`` for the initial
        generation.  Revisions use ``call()`` via ``asyncio.to_thread``
        and yield their full text as a single chunk.
        """
        import asyncio
        ...
        # --- initial generation (async-streamed) ---
        chunks: list[str] = []
        call_stream_async = getattr(self.llm, "call_stream_async", None)
        if callable(call_stream_async):
            async for chunk in call_stream_async(full_prompt):
                chunks.append(chunk)
                yield chunk
        else:
            # Adapter has no async streaming — run sync call in thread
            text = await asyncio.to_thread(self.llm.call, full_prompt)
            chunks.append(text)
            yield text
        ...
        for revision in range(self.max_revisions + 1):
            ...
            response = await asyncio.to_thread(self.llm.call, revision_prompt)
            yield response
