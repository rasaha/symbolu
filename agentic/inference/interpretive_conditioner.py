...
class InterpretiveStateBuilder(nn.Module):
    ...
    def __init__(self, hidden_dim: int, config: InterpretiveConditionerConfig):
        ...
        if config.enable_polarity:
            try:
                from agentic.inference.polarity_encoding import PolarityGate, PolarityEncodingConfig
        ...
        if config.phase_out_dim > 0:
            try:
                from agentic.inference.phase_coherence_signal import PhaseCoherenceProjection
