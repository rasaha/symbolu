...
4. **Cross-domain governor (cloud + robotics on one core).** No agent framework governs physical actuation and cloud operations with the same frozen decision core (FACT: `Project_documentation/control_plane/acp/ACP_V1_FREEZE.md`). This opens manufacturing/robotics markets the LLM-only frameworks can't touch.
