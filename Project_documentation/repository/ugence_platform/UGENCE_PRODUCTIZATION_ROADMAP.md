...
| FD-13.1 | *Proposed, unruled* — **`SCREEN_5_TYPED_VENDOR_DECLARATIONS`**: seam 9 is a typed vendor-dependency form in the seam-8 shape, completing outline row 5 but for egress restrictions, which have no package. Alternative: `FRONT_DOOR_CEILING_REACHED` | `ADR_UGENCE_STUDIO_FRONT_DOOR_SCOPING.md` §14 |
| FD-13.2 | *Proposed, unruled* — **`LOCAL_SQLITE_UNDER_RUNTIME_VOLUME`**: `vendor-dependency` 0.2.0 adds one sqlite `VendorDependencyPort` implementation plus a single `declare`, tenant-bound and named by one configuration value; the seam-8 posture unchanged | same, §14 |
| FD-13.3 | *Proposed, unruled* — **`V2_AMENDMENT_TWO_OPERATIONS`**: `v2_vendor_declare` and `v2_vendor_list`, re-frozen as amendment v2-A5 with the regenerated client in the same step | same, §14 |
| FD-13.4 | *Proposed, unruled* — **`RISK_POSTURE_UNINTERPRETED`**: `risk_posture_label` is recorded as typed and ordered, compared, scored and evaluated nowhere; no vendor approval, onboarding status or risk verdict is invented | same, §14 |
