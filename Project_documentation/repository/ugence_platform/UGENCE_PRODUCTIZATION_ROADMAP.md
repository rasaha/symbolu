...
| FD-12.5 | **`DECLARE_IS_THE_ONLY_WRITE`** — the seam writes declarations and nothing else: no edit, revocation, admission, verification, scoring or enforcement; a changed declaration supersedes the old; the record carries no data, its labels stay uninterpreted and it confers nothing; egress restrictions are not invented. Documentation only; seam 8 activates by its own implementation prompt | same, §13 |
