...
| | |
|---|---|
| **Status (2026-09-05, re-ruled)** | **Entered by owner ruling `ENTER_LANGFLOW_IMPORT_FIRST`** (§11.3), superseding `DEFER_LANGFLOW_CUSTOMER_GATED` on product judgement: importing an existing agentic workflow is the "govern an existing agent" entry path of the studio (`ADR_UGENCE_STUDIO_FRONT_DOOR_SCOPING.md`, FD-2). GAS-R4 stands in full. Scoped by `docs/architecture/ADR_UGENCE_LANGFLOW_IMPORT_SCOPING.md`, LI-1 to LI-5 ruled 2026-09-05; **contract-only and blocked on evidence** (a genuine secret-free Langflow export fixture is required before implementation begins): no importer, endpoint, dependency or implementation exists `[V]` (`langflow_import_implemented: False`; no `/policy/from-langflow` route in `contracts/openapi_v2.json`). Other platforms (Copilot Studio, Vertex AI, ServiceNow, LangGraph, CrewAI) stay customer-gated. React Flow remains the Ugence-owned authoring surface (GAS-R3). |
| **Entry** | Ruling met 2026-09-05 (`ENTER_LANGFLOW_IMPORT_FIRST`, LI-1 to LI-5). Implementation is entered only once a genuine, secret-free Langflow export fixture is supplied and committed, and only by its own implementation prompt. |
...
| CR-1 | **`SEPARATE_WORKER_UNIT`** (2026-09-05) — a companion governed runtime worker hosts the DBOS engine, the runtime host, the three SQLite stores and the review service; the P3E container is not extended and the studio backend composes nothing | `ADR_UGENCE_REVIEW_SERVICE_COMPOSITION_ROOT_SCOPING.md` §5 |
| CR-2 | **`AMEND_P3E_SERVE_V2`** — P3E gains `UGENCE_STUDIO_REVIEW_SERVICE_URL` and serves the combined v1 and v2 app under its existing gate; `approved-runtime-config` amended | same, §5 |
...
| LI-1 | **`TWO_ARTIFACTS`** (2026-09-05) — the importer produces a Workflow IR artifact preserving the accepted graph and a separate `PolicyPack` that enters as an ordinary unapproved pack; import confers no approval, maturity, authority or permission to execute | `ADR_UGENCE_LANGFLOW_IMPORT_SCOPING.md` §7 |
| LI-2 | **`RECORD_AS_DIAGNOSTIC`**, narrowly — only explicitly allowlisted, non-semantic metadata that cannot affect graph meaning, bindings, policy, authority or execution; unknown semantic fields, code-bearing fields and executable content refuse the import; LI-2 never overrides the strict refusals (malformed, oversized, deeply nested, cyclic, code-bearing, unknown node type, duplicate identifier, inconsistent count, ambiguous edge) | same, §5 and §7 |
...
| FD-6 | **Seam 2 = `POLICY_REGISTRY_AND_DECISION_STORE`** (2026-09-06) — the Authority screen receives read-only access to the policy registry and the decision store through the deployment composition root: typed retrieval and display only; no policy authorship, decision issuance, approval, mutation, credential access, provider execution or LIVE capability; records tenant-bound and linked through their existing canonical references; missing, malformed, cross-tenant, stale or unbound records are a typed refusal or typed gap, never inferred, repaired or synthesized. The registry half is the seam-1 `SqlitePolicyRegistry` with a typed list of policy identities; no durable Decision Authority record store exists, so `decision_store` stays absent and the decision read keeps its typed gap. `GOVERNANCE_HOOK_AND_PROVIDER_REGISTRY` and `CONSOLE_BASE_URL` remain unruled; documentation only, activates no seam | `ADR_UGENCE_STUDIO_FRONT_DOOR_SCOPING.md` §6 |
| FD-7 | **Seam 3 = `GOVERNANCE_HOOK_AND_PROVIDER_REGISTRY`** (2026-09-06) — FD-7.1 `FIXTURE_RUN_ADMISSIBLE`: a run whose every provider is an in-image, no-I/O fixture under a non-LIVE mode is a demonstration, not agent execution; FD-7.2 `P3E_IN_PROCESS`: the studio's own Agent Runtime, no worker relay; FD-7.3 `RUNTIME_DEFAULT_BLOCK`: `provider_registry` only, the governance hook stays the runtime's fail-closed default and is absent by ruling; FD-7.4 `ONE_PINNED_FIXTURE_PROVIDER`: one in-package provider behind one boolean, no discovery; FD-7.5 `PROHIBITED_IN_PROFILE`: no permissive hook is ever constructed and startup integrity enforces it. Shipped as `governance-studio-deployment` 0.5.0 | `ADR_UGENCE_STUDIO_FRONT_DOOR_SCOPING.md` §8 |
| FD-8.1 | **`ABSENT_UNTIL_PREREQUISITES`** (2026-09-06) — the console does not enter the P3E deployment profile until its prerequisites are separately completed and ratified; the root-level prototype, unauthenticated HTTP listener, in-memory audit store and absence from both images are not an acceptable deployable boundary | `ADR_UGENCE_STUDIO_FRONT_DOOR_SCOPING.md` §9 |
| FD-8.2 | **`PIN_SHADOW_AND_REFUSE_UNMAPPED`** — `PublishService` fixes mode to shadow in every governed-loop request; a `compiled_package` without a valid `scenario_id` is the typed refusal `publish_payload_unmapped` before any outbound request, inventing no assertion, action, signal, scenario or default; only the validated `scenario_id` is relayed, no compiled-package field crosses | same, §9 |
| FD-8.3 | **`PACKAGE_FIRST`** — before deployment the console must become a bounded, installable, tested package or deployment unit with explicit public API, configuration, persistence and authentication boundaries, maturity label and import restrictions; a separately scoped future body of work | same, §9 |
| FD-8.4 | **`SECOND_DESTINATION_BY_RULING`** — if seam 4 is later admitted, the console is a second explicitly ruled outbound destination over HTTPS with its own frozen allowlist/configuration entry under the P3E egress and secret constraints; the one-destination freeze is never silently widened | same, §9 |
| FD-8.5 | **`LABELLED_SINGLE_INSTANCE`** — until durable multi-instance storage exists, Observe describes only the identified console instance's records and every response and screen discloses that; never durable, complete, distributed or restart-safe coverage | same, §9 |
| FD-9.1 | **`SCREEN_1_TYPED_REGISTRATION`** (2026-09-06) — seam 5 is a typed registration form over `ai-system-registry`, composed through the P3E root and tenant-bound; the governed hook stays a later worker-relay seam and a durable Decision Authority store a package decision | `ADR_UGENCE_STUDIO_FRONT_DOOR_SCOPING.md` §10 |
| FD-9.2 | **`LOCAL_SQLITE_UNDER_RUNTIME_VOLUME`** — `ai-system-registry` 0.2.0 adds one sqlite `SystemRegistryPort` implementation plus a single `register` append in the seam-1 posture (a file under the runtime volume; no server, driver or DSN); D-5's post-v1 line is the systems-of-record connectors, which stay unbuilt | same, §10 |
| FD-9.3 | **`OWNER_REF_PRESENTED_UNPROVEN`** — `owner_ref` is a typed opaque handle recorded as presented and unproven; `registered_by` is the deployment name and version; no identity is claimed until an issuer exists | same, §10 |
| FD-9.4 | **`V2_AMENDMENT_TWO_OPERATIONS`** — `v2_registry_register` and `v2_registry_list`, validated by the package's own refusal reasons; `openapi_v2.json` and the generated client re-frozen by their own amendment record in the same step | same, §10 |
| FD-9.5 | **`REGISTER_IS_THE_ONLY_WRITE`** — the seam writes registrations and nothing else: no edit, revocation, gate, admission or attestation; a changed system is a new superseding registration; the record confers nothing; SD-2 and every credential and LIVE prohibition unchanged. Documentation only; seam 5 activates by its own implementation prompt | same, §10 |
| FD-10.1 | **`START_IS_A_RELAY`** (2026-09-06) — asking the separate governed runtime worker to start the fixture workflow it already owns, over `FIXTURE_ONLY` providers, with nothing supplied by the studio, is display and transmit in FD-7.1's sense, not execution; SD-2's verbs stay absent from every operation id and path | `ADR_UGENCE_STUDIO_FRONT_DOOR_SCOPING.md` §11 |
...
| FD-11.1 | **`OBSERVE_OVER_WORKER_LEDGER`** (2026-09-06) — seam 7 is a read of the worker's control-plane audit ledger by correlation id, relayed through the review service as seam 6 relays the start; the console stays a packaging body of work (FD-8.3) and a durable Decision Authority store a package decision | `ADR_UGENCE_STUDIO_FRONT_DOOR_SCOPING.md` §12 |
| FD-11.2 | **`READ_PORT_IN_CONTROL_PLANE_ROOT`** — `control-plane-root` 0.2.0 adds one read-only, tenant-scoped read of a tenant's own rows by correlation id in chain order with digests, beside `verify_chain`; D-5 and the README's "no reconstruction API" amended to say a raw, uninterpreted read is not reconstruction; `REVIEW_SERVICE_READ_INDEX` recorded as the admissible alternative, the implementation prompt names which ships | same, §12 |
| FD-11.3 | **`SEVENTH_ROUTE_LEDGER_READ`** — `GET /review/audit/{correlation_id}` on the worker returns its own tenant's entries in `tenant_seq` order with digests and the chain verification result as a typed field; an integrity failure is a typed refusal, unknown is a typed not-found, no list-all route | same, §12 |
| FD-11.4 | **`TWO_LABELLED_SOURCES`** — the Observe screen shows the worker ledger (durable, per-tenant, hash-chained, receipts not narratives) and the console's typed gap as distinct labelled sources, never merged; the screen re-derives nothing and shows the worker's verification result as the worker's | same, §12 |
| FD-11.5 | **`READ_ONLY_ONE_STEP_AMENDMENT`** — CR-2 (seven routes), the P3E egress record and freeze test, the frontend manifest, the v2 contract (v2-A3, one operation) with generated client and `control-plane-root` 0.2.0 amended together with tests; no append, edit or verification-trigger route from the studio; no configuration value, image package, credential or second destination. Seam 7 shipped 2026-09-06 (`control-plane-root` 0.2.0, `governed-review-service` 0.6.0, `governed-runtime-worker` 0.3.0, `governance-studio-deployment` 0.8.0; ADR §7) | same, §12 |
| FD-12.1 | **`SCREEN_5_TYPED_DATA_USE_DECLARATIONS`** (2026-09-06) — seam 8 is a typed declaration form over `data-use-admission`, composed through the P3E root and tenant-bound; the last outline row a studio-alone seam can reach, with the console a packaging body of work (FD-8.3), a durable Decision Authority store a package decision, and the mirror coordinates, Langflow fixture and enterprise issuer owner inputs; `vendor-dependency` is its own later seam | `ADR_UGENCE_STUDIO_FRONT_DOOR_SCOPING.md` §13 |
| FD-12.2 | **`LOCAL_SQLITE_UNDER_RUNTIME_VOLUME`** — `data-use-admission` 0.2.0 adds one sqlite `DataUseDeclarationPort` implementation plus a single `declare` append in the seam-5 posture (a file under the runtime volume named by one configuration value; no server, driver or DSN), bound to the deployment's tenant at first open and never re-bound | same, §13 |
| FD-12.3 | **`DECLARED_BY_PRESENTED_UNPROVEN`** — `declared_by` is a typed opaque handle recorded as presented and unproven; the recording composition is the deployment's name and version; no identity is claimed until an enterprise issuer exists | same, §13 |
| FD-12.4 | **`V2_AMENDMENT_TWO_OPERATIONS`** — `v2_data_use_declare` and `v2_data_use_list`, validated by the package's own refusal reasons; `openapi_v2.json` and the generated client re-frozen by amendment v2-A4 in the same step | same, §13 |
| FD-12.5 | **`DECLARE_IS_THE_ONLY_WRITE`** — the seam writes declarations and nothing else: no edit, revocation, admission, verification, scoring or enforcement; a changed declaration supersedes the old; the record carries no data, its labels stay uninterpreted and it confers nothing; egress restrictions are not invented. Seam 8 shipped 2026-09-06 (`data-use-admission` 0.2.0, `governance-studio-deployment` 0.9.0, v2-A4; ADR §7). It closed the last outline *row* a studio-alone seam can reach, not the row: `vendor-dependency` is the same shape again and, per FD-12.1, its own later seam, audited as seam 9 in ADR §14 | same, §13 |
| FD-13.1 | **`SCREEN_5_TYPED_VENDOR_DECLARATIONS`** (2026-09-06) — seam 9 is a typed vendor-dependency form over `vendor-dependency`, composed through the P3E root and tenant-bound, in the shape seam 8 proved; it completes outline row 5 but for egress restrictions, which have a ratified ADR and no package and stay a gap. With seam 9 shipped the front door under FD-1 reaches its ceiling: the console a packaging body of work (FD-8.3), a durable Decision Authority store a package decision, and the mirror coordinates, Langflow fixture and enterprise issuer owner inputs | `ADR_UGENCE_STUDIO_FRONT_DOOR_SCOPING.md` §14 |
| FD-13.2 | **`LOCAL_SQLITE_UNDER_RUNTIME_VOLUME`** — `vendor-dependency` 0.2.0 adds one sqlite `VendorDependencyPort` implementation plus a single `declare` append in the seam-5 and seam-8 posture (a file under the runtime volume named by one configuration value; no server, driver or DSN), bound to the deployment's tenant at first open and never re-bound; `declared_by` is presented and unproven, the recording composition the deployment's name and version | same, §14 |
| FD-13.3 | **`V2_AMENDMENT_TWO_OPERATIONS`** — `v2_vendor_declare` and `v2_vendor_list`, validated by the package's own refusal reasons; `openapi_v2.json` and the generated client re-frozen by amendment v2-A5 in the same step, the fifth since the v2 freeze | same, §14 |
| FD-13.4 | **`RISK_POSTURE_UNINTERPRETED`** — `risk_posture_label` is recorded stripped and verbatim and ordered, compared, scored, ranked and evaluated nowhere; no vendor approval, onboarding status, tier, certification or risk verdict is invented, because no package computes one; `declare` is the only write, the record carries an opaque `vendor_ref` and never contract terms, pricing or contact data, and it confers nothing. Seam 9 shipped 2026-09-06 (`vendor-dependency` 0.2.0, `governance-studio-deployment` 0.10.0, v2-A5; ADR §7), and with it the front door under FD-1 reaches its ceiling: no studio-alone seam remains. Closed out in ADR §15, which proposes no ruling: what remains is three owner inputs (mirror coordinates, Langflow fixture, enterprise issuer) and three bodies of work (a declared result-egress seam, console packaging under FD-8.3 — scoped 2026-09-06 in `ADR_UGENCE_CONSOLE_PACKAGING_SCOPING.md`; CP-1 to CP-5 ratified the same day (SERVICE_ONLY, PACKAGES_INTEGRATION, ALLOWLIST_THE_FOUR, DECLARE_THE_CEILING, DECLARE_AND_FAIL_AT_INSTALL), implementation shipped 2026-09-06 as `ugence-console-api` 0.2.0 at `packages/integration/console-api` (five served routes, the audit ceiling on every answer, four declared platform dependencies; ADR §9), after being deliberately deferred until the clearance-export seam was audited — done 2026-09-06 in `ADR_UGENCE_CLEARANCE_EXPORT_SCOPING.md`, which found that `ClearanceReceiptBody` already carries eleven of the needed bindings but is unreachable from the studio, unsigned, and missing workflow identity and an identity-assurance field. CE-1 to CE-5 ratified the same day (RECEIVED_CLEARANCE_ONLY, NEW_CONTRACTS_ONLY_PACKAGE, ONE_MEMBER_NOW, INTEGRITY_ONLY_AND_SAY_SO, EXPORT_IS_A_READ); implementation follows console packaging. Begun 2026-09-06 and stopped before any file was written: ADR §11 found both premises false — no allowlisted package can relay a clearance receipt, so a new SD-1 entry is required, and the deployment holds no received clearance and has no path to holding one, so CE-5's read could only ever answer "not found". §12 put ballot CE-6 (which package gets the SD-1 entry) and CE-7 (where a held clearance comes from), and both were ratified as recommended the same day in §13: ONE_ENTRY_EXPORT_PACKAGE (one SD-1 line for `ugence_clearance_export` and no other package, the FD-12/FD-13 shape) and SYNTHETIC_SEEDED_RECEIPTS (pinned fixtures inside the existing SYNTHETIC_DEMONSTRATION_ONLY discipline, so CE-5's read stays a read). §13.1 fixes the limit: a seeded receipt is evidence about serialization and integrity and nothing else, and the artifact carries SYNTHETIC_DEMONSTRATION_ONLY alongside authenticity: UNSIGNED and PRESENTED_UNPROVEN. Implementation of CE-1 to CE-7 shipped 2026-09-06 (ADR §14): `ugence-clearance-export` 0.1.0 at `packages/integration/clearance-export`, contract amendment v2-A6 adding the single read `v2_export_read`, exactly one SD-1 allowlist line, and `governance-studio-deployment` 0.11.0 seeding a read-only receipt source from fixtures inside the pinned synthetic manifest. Every exported artifact carries three honesty claims that cannot be omitted, softened or stripped without changing its fingerprint. What is still absent is unchanged: the deployment holds no clearance any authority granted, so a real export needs a producer and the rulings §11.2 named. The pilot scoping audit followed on 2026-09-06 (`ADR_UGENCE_PILOT_SCOPING.md`): the cross-platform workflow the roadmap's thesis asks for already exists as `products/code-governance` 0.5.0, which calls the canonical `evaluate_clearance` in shipped source and carries its own stage-gate standard. The pilot is blocked on three owner inputs, not on code — a read-only GitHub credential, bounded pilot authorization and one real reviewer — and none of the four things `verify_export` names as unestablished blocks a bounded shadow pilot, though all four block enforcement. Ballot PI-1 to PI-4 proposed; the producer and the export seam cannot currently see each other, and closing that is four owner acts rather than four coding tasks — a Decision Authority producer). The egress-authority ADR is discharged, not unimplemented — it authorized `data-use-admission`, which shipped as seam 8; see its §7 | same, §14, §15; `ADR_UGENCE_DATA_EGRESS_AUTHORITY_SCOPING.md` §7 |
