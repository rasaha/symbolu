...
| 5 | `authorization_expired` wired through `map_request` + native `EXPIRED` outcome | **not done — MAJOR, deliberately separate** |
...
All verified at HEAD, unmoved:

| Gate | Value |
|---|---|
| ActionGate `.api` snapshot | `9eeb66e3…` — unmoved |
| `public_api_manifests["actiongate_provider.api"]` | `9eeb66e3…` — unmoved |
| `core_tree_hashes["actiongate_provider"]` | `9cbeb833…` — unmoved (facade tree untouched) |
| `conformance_hashes[…ugence_actiongate_provider]` | `07e08bd4…` — unmoved |
| behavioural-equivalence `capture_hash` | `d805e6cf…` — before == canonical == legacy |
| clean-wheel dependency verification | unaffected — vNext adds no dependency |

Suites: 151 passed (`packages/providers/actiongate` + `actiongate_provider/tests`),
52 of them new; 195 passed (reference harness).

## Change classification

This change is **MINOR** under `platform/PLATFORM_FREEZE_V1.json`
`compatibility_rules` — *"additive public APIs, new capabilities, new
conformance assertions"*. It adds a subpackage and tests, changes no existing
API or semantics, and moves no frozen hash.

`[V]` Step 5 is **MAJOR**, not PATCH. `PATCH` is defined as *"no API or semantic
change"*, and `MAJOR` explicitly covers *"authority/lifecycle/dependency-
direction/fail-safe changes"*. A live input that today yields `AUTHORIZED` and
after the fix yields a non-authorizing outcome is a fail-safe change by the
manifest's own words. Shipping it as a bug-fix patch would misclassify it.
...
3. `[G]` The behavioural-equivalence probe never sets `authorization_expired` and
   dumps a fixed key list, so it would not have caught the original defect. It
   should vary every dimension.
4. `[G]` CABP signing domain and trust model — unowned, undesigned.
5. `[G]` ActionGate and `action-clearance` share no reason-code vocabulary.
