...
BASE='SERVICENOW_CENTERED_CITY_SCALE_VEHICLE_INTELLIGENCE_REFERENCE_ARCHITECTURE_v1.0'
META=dict(
 title='ServiceNow-Centered City-Scale Vehicle Intelligence — Proposed Reference Architecture, Governed Workflows and Illustrative Technical Case Study',
 author='Ugence Labs',
 company='Ugence Labs',
 subject='Proposed ServiceNow-centered reference architecture and illustrative technical case study for governing city-scale vehicle intelligence; a factual baseline, not an official or endorsed ServiceNow design and not evidence of a deployed solution',
 keywords='ServiceNow-centered, city-scale vehicle intelligence, ANPR, ALPR, Public Sector Digital Services, PSDS, Investigative Case Management, AI Control Tower, IntegrationHub, Workflow Data Fabric, IRM, reference architecture, governed workflow, smart city')
...
def rhtml(r):
    ...
    if r.get('href'): t=f'<a href="{esc(r["href"])}" style="color:#2B6CB0; word-break:break-all;">{t}</a>'
    return t
...
@page { size:A4 portrait; margin:20mm 17mm 16mm 17mm;
 @top-left{ content:"ServiceNow-Centered · City-Scale Vehicle Intelligence"; font:8pt Arial; color:#5b6472;}
 @top-right{ content:"Proposed Reference Architecture · v1.0"; font:8pt Arial; color:#5b6472;}
 @bottom-left{ content:"Proposed reference architecture · Research 15 Aug 2026"; font:8pt Arial; color:#5b6472;}
 @bottom-right{ content:"Page " counter(page) " of " counter(pages); font:8pt Arial; color:#5b6472;} }
...
'''
    ...
    # cover
    parts.append('''<section style="page:cover; height:297mm; box-sizing:border-box; background:#132430; color:#fff; padding:34mm 22mm;">
    <div style="font:11pt Arial; letter-spacing:2px; color:#8fb9c9; text-transform:uppercase;">Proposed Reference Architecture · Factual Baseline</div>
    <div style="height:14mm"></div>
    <div style="font-family:Georgia,serif; font-size:31pt; font-weight:bold; line-height:1.08;">ServiceNow-Centered<br>City-Scale Vehicle Intelligence</div>
    <div style="height:6mm"></div>
    <div style="font-family:Georgia,serif; font-size:15pt; color:#dbeaf0;">Proposed Reference Architecture, Governed Workflows and Illustrative Technical Case Study</div>
    <div style="height:12mm"></div>
    <div style="font-size:12pt; color:#c2d3db; max-width:156mm; line-height:1.5;">The vehicle-intelligence platform determines what the evidence indicates. ServiceNow coordinates the enterprise case, workflow, governance records, policy and controls, approvals, orchestration and reporting. External consequence systems carry out the effect; human authorities make the legally accountable decisions.</div>
    <div style="height:16mm"></div>
    <div style="font-size:13pt;"><strong style="color:#fff;">Prepared by Ugence Labs</strong></div>
    <div style="font-size:11.5pt; color:#c2d3db;">Reference Architecture v1.0 · ServiceNow research 15 August 2026</div>
    <div style="height:14mm"></div>
    <div style="font-size:9.5pt; font-style:italic; color:#e6c6a6; max-width:158mm; line-height:1.5;">PROPOSED REFERENCE ARCHITECTURE — not an official or endorsed ServiceNow architecture, and not evidence of a deployed customer solution. Availability depends on release, SKU, licensing, Store applications and customer configuration; all external integrations are proposed unless explicitly documented otherwise. Not legal advice; legal, privacy, surveillance, evidence, due-process and enforcement requirements must be validated by the responsible jurisdiction. ServiceNow capabilities are cited from official sources; verify on the live documentation and pin to the target release before customer use.</div>
    </section>''')
    # TOC
...
def render_docx():
    ...
    hp=sec.header.paragraphs[0]; hp.text=''; r=hp.add_run('ServiceNow-Centered City-Scale Vehicle Intelligence — Proposed Reference Architecture')
    r.font.size=Pt(8); r.font.color.rgb=RGBColor(0x5b,0x64,0x72)
    fp=sec.footer.paragraphs[0]; fp.text=''
    fr=fp.add_run('Proposed reference architecture v1.0 · Research 15 Aug 2026      Page '); fr.font.size=Pt(8); fr.font.color.rgb=RGBColor(0x5b,0x64,0x72)
    pr=fp.add_run(); pr.font.size=Pt(8); pr.font.color.rgb=RGBColor(0x5b,0x64,0x72); set_field(pr,'PAGE')
    # ---- cover ----
    ...
    NAVY=RGBColor(0x14,0x2A,0x38); VIOL=RGBColor(0x14,0x53,0x2D); GREY=RGBColor(0x55,0x5b,0x66)
    for _ in range(2): doc.add_paragraph()
    cover_par('PROPOSED REFERENCE ARCHITECTURE · FACTUAL BASELINE',10,RGBColor(0x1C,0x7F,0xA8),bold=True,after=10)
    cover_par('ServiceNow-Centered City-Scale Vehicle Intelligence',27,NAVY,bold=True,after=6)
    cover_par('Proposed Reference Architecture, Governed Workflows and Illustrative Technical Case Study',15,VIOL,bold=True,after=12)
    cover_par('The vehicle-intelligence platform determines what the evidence indicates. ServiceNow coordinates the enterprise case, workflow, governance records, policy and controls, approvals, orchestration and reporting. External consequence systems carry out the effect; human authorities make the legally accountable decisions.',12,RGBColor(0x37,0x41,0x51),after=16)
    cover_par('Prepared by Ugence Labs',13,NAVY,bold=True,after=1)
    cover_par('Reference Architecture v1.0 · ServiceNow research 15 August 2026',11,RGBColor(0x37,0x41,0x51),after=16)
    cover_par('PROPOSED REFERENCE ARCHITECTURE — not an official or endorsed ServiceNow architecture, and not evidence of a deployed customer solution. Availability depends on release, SKU, licensing, Store applications and customer configuration; all external integrations are proposed unless explicitly documented otherwise. Not legal advice; legal, privacy, surveillance, evidence, due-process and enforcement requirements must be validated by the responsible jurisdiction. Verify ServiceNow capabilities on the live documentation and pin to the target release before customer use.',9.5,GREY,italic=True,after=2)
    doc.add_page_break()
    # ---- TOC ----
    ...
    def add_hyperlink(p, url, text):
        part=p.part; r_id=part.relate_to(url, 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink', is_external=True)
        hl=OxmlElement('w:hyperlink'); hl.set(qn('r:id'),r_id)
        nr=OxmlElement('w:r'); rpr=OxmlElement('w:rPr')
        col=OxmlElement('w:color'); col.set(qn('w:val'),'2B6CB0'); rpr.append(col)
        u=OxmlElement('w:u'); u.set(qn('w:val'),'single'); rpr.append(u)
        nr.append(rpr); t=OxmlElement('w:t'); t.set(qn('xml:space'),'preserve'); t.text=text; nr.append(t)
        hl.append(nr); p._p.append(hl)
    def add_runs(p, runs):
        for r in runs:
            if isinstance(r,str): p.add_run(r)
            elif r.get('href'): add_hyperlink(p, r['href'], r['text'])
            else:
