...
> 26262 traceability template (12 clauses, **21 indexed BCVF
...
> the post-hoc triage tool to a runtime SRE surface. **491 tests
> passing**, up from 400 in v0.6 (12 audit pinning tests + 15
> statistical-significance tests + 20 safety-case-traceability tests
> + 29 streaming-monitor tests added post-v0.7). v1 file at
...
- **491 tests passing** (+270 since v0.2; +12 audit pinning tests + 15 statistical-significance tests + 20 SOTIF / ISO 26262 traceability tests + 29 streaming-monitor tests added post-v0.7 — the Q3 traceability template was pulled forward (12 clauses × 21 indexed BCVF artifacts × machine-checked markdown snapshot), and the fleet harness gained a `StreamingFleetMonitor` with rolling-window summaries + threshold alerts that turn it from a triage tool into an SRE one) across the autonomy
...
| Area | Current state |
|---|---|
| **Test suite** | 491 passing across 22 test modules (+12 pinning tests from the post-v0.7 audit pass: 7 covering the `_binomial_tail_geq` k=1 off-by-one fix in both copies, 3 covering the `attribution_within_top_half` ceil-vs-floor fix and the sign-test single-decisive-win regression, 2 covering the documented `_cluster_majority` greedy-non-transitive contract; +15 statistical-significance tests for the certification-grade primary grid: 5 Wilson CI primitives, 3 pinning the 60-seed cadence + magnitude grouping, 2 pinning the 0.94 / 0.91 / 0.886 textbook bounds at 60-of-60, 59-of-60, 58-of-60, 1 binding the threshold-edge `accelerating[accel_mag=0.3]` config, 4 covering `summarize_grid` exposure + sabotage of a single config; +20 SOTIF / ISO 26262 traceability tests: 3 matrix-shape pins, 5 per-clause hygiene + artifact resolution, 6 audit-priority clause pins, 4 snapshot-parity + reverse-index pins, 2 markdown-render determinism; +29 StreamingFleetMonitor tests: 3 constructor, 4 ingestion + clock injection, 3 eviction + prune, 5 windowed-summary + window=None, 2 batch-parity invariant, 7 alert rule semantics + min_episodes + dotted-path metrics + None-resolution, 5 misc serialisation / hygiene); reproducible on CPU in < 4 min (4 host-speed-dependent perf benchmarks + 4 long-running sweep / timing tests deselected) |
