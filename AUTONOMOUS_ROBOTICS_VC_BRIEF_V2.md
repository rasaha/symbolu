...
**Xozence Labs | BCVF Autonomy Runtime**
...
> broke static analysis (now Optional everywhere).
> **Calibration parameter management + drift detection
> framework landed post-v0.7.x** as `CALIBRATION_DESIGN.md`
> + thin-shim implementation in `calibration/`: a frozen,
> hash-identified `CalibrationSet` bundles 8 typed configs
> (BCVFConfig + ConsumerV2Config + BicycleConfig +
> RealTimeBudget + DDSQoSProfile + SafetyStateMachineConfig +
> per-predictor FailureConfig + expected_metrics range map)
> into one JSON artifact a fleet operator distributes, signs,
> and version-controls. SHA-256 digest over canonical JSON is
> the bundle's identity; `kernel_version` field is validated
> against `bcvf_autonomous.__version__` at load — drift
> surfaces loud unless the caller explicitly opts in via
> `allow_version_drift=True`. Strict-validation rejects
> tampered bundles (digest mismatch =
> `CalibrationDigestError`), malformed embedded configs (each
> re-instantiates its source dataclass at construction so a
> malformed `BCVFConfig` can't ride into the bundle), non-ISO
> 8601 `created_at`, and whitespace-only `calibration_id`.
> `CalibrationDriftDetector` walks the bundle's
> `expected_metrics` against a live `WindowedFleetSummary`
> and emits typed `CalibrationDriftAlert` records per range
> violation — composes with `StreamingFleetMonitor` via the
> same dotted-path metric resolver `AlertRule` walks; the
> deployment partner unions both alert streams. SOTIF clause
> 12 (release-to-market + configuration management) gains
> both the bundle and the drift detector as configuration-
> management deliverables — sibling artifacts to the
> CycloneDX SBOM that shipped with §9-row-#2.
> SOTIF_TRACEABILITY.md regenerates 37 → **39 indexed
> artifacts**. **Post-landing audit pass on the §9-row-#6
> implementation surfaced six findings**: `RealTimeBudgetError`
> smuggled past the embedded-config validator (the narrow
> `(KeyError, TypeError, ValueError)` filter let
> `RealTimeBudget.__post_init__` exceptions through uncaught —
> breaking the framework's "no malformed config can ride into
> the bundle" contract; now broadened to catch every domain-
> specific exception at the gate); kernel-version validation
> only fired from `load_calibration_set` so an in-memory
> caller using `CalibrationSet.from_dict` directly bypassed
> the version-drift detector entirely (now path-independent
> with `allow_version_drift` kwarg on both call sites);
> NaN / Inf bounds on `expected_metrics` slipped past
> `lo > hi` (NaN comparisons all False) and silently disabled
> drift detection for the metric (now rejected loud at
> construction with `math.isfinite` check + `allow_nan=False`
> on the canonical-JSON path); zero-observation windows raised
> KeyError on every metric path walk (now mirror
> `StreamingFleetMonitor`'s `min_episodes` discipline and
> return empty); double validation on every
> `build_calibration_set` call (now single-pass via hand-
> rolled payload); design-doc / impl drift on `mppi_config`
> (was documented + design-pin-tested but never implemented;
> removed from the doc + pin since `MPPIConfig` nests
> `BCVFConfig` and the kernel knobs already ship via
> `bcvf_config`).
> **Sensor attestation interface landed post-v0.7.x** as
> `SENSOR_ATTESTATION_DESIGN.md` + thin-shim implementation
> in `attestation/`: closes the UN ECE R155 cybersecurity
> loop the adversarial family opened. `SensorAttestation` is
> the typed per-message record (predictor_name +
> firmware_version + signature + nonce + issued_at +
> data_digest + metadata) a sensor stack attaches to its
> trajectory output; `SensorAttestationPolicy` is the per-
> predictor verification policy (firmware allowlist +
> freshness/replay windows + key_id + enabled flag); the
> `SensorAttestationVerifier` runs **seven §4 checks** in
> documented order — policy lookup, policy-enabled short-
> circuit, firmware allowlist, freshness, future-dating,
> replay, data binding, HMAC-SHA256 signature with constant-
> time `hmac.compare_digest`. Stdlib-only: `hashlib` + `hmac`
> + `secrets`; no `cryptography` library, no PKI, no key-
> management — the integrator wires a `key_resolver` callable
> to their HSM / TPM / secure enclave so key material never
> enters the framework. **Composition with the existing
> exclusion path**: failed verification unions into the same
> `is_excluded` mask that deadline-driven (BCVFNode) and
> state-machine-driven (SafetyStateMachine) exclusions
> populate; three exclusion sources stack via
> `np.logical_or`; the audit log differentiates causes. The
> attestation gate is upstream of the kernel — a stealth-bias
> spoof the kernel's Lemma-1 invariance can't see by
> construction is rejected before reaching the kernel,
> closing SOTIF clause-8 Insufficiency #3 (the in-scope
> mitigation the safety case has been pointing at since v0.7).
> SOTIF clause 8 (functional insufficiencies + mitigations)
> gains both the record + the verifier as the "in-scope
> mitigation" evidence alongside the existing adversarial
> family + V2 chatter mitigation + safety state machine.
> SOTIF_TRACEABILITY.md regenerates 39 → **41 indexed
> artifacts**. Ten new symbols enter `PROVISIONAL_API`
> (count lock 67 → **77**); `STABLE_API` graduation gated on
> five explicit criteria in `SENSOR_ATTESTATION_DESIGN.md` §8
> (deployment partner one quarter against HSM / TPM key-
> resolver, real attestation-failure detection across a
> firmware regression, asymmetric-attestation extension
> subclass, external auditor sign-off as UN ECE R155 §7.3.4
> evidence, replay-cache persistence layer for cross-process
> nonce dedup). The §9 row #8 of `INDUSTRY_FEATURES_ROADMAP.md`
> is struck through with a pointer to the design doc — and
> with it the LAST entry in `_ROADMAP_TOKENS` is removed.
> **Six §9 roadmap rows shipped post-v0.7** (rows #1, #2, #3,
> #4, #6, #8); rows #5 (HD-map predictor) and #9 (domain-
> specific predictors) are hardware-adjacent + need real
> predictor-stack data, not sandbox-implementable; row #7
> (SBOM) shipped absorbed into row #2 (CycloneDX 1.5 manifest).
> **Post-landing audit pass on the §9-row-#8 implementation
> surfaced nine findings** — security code is the wrong place
> to triage, so all five real bugs got fixed: replay-window <
> freshness-window opened a replay window in the gap between
> cache eviction and freshness expiry (now invariant-checked
> at policy construction); naive ISO 8601 timestamps were
> interpreted as host-local time (now timezone-aware required
> at construction with explicit TZ-skew rejection); disabled-
> policy short-circuit didn't record the nonce (now updates
> the cache even on disabled passes — captured-during-
> disabled-rollout attestations can't replay after enable);
> `int(s, 16)` accepted `+`, `-`, PEP 515 underscores in hex
> digests (now strict `[0-9a-fA-F]{64}` character check);
> `verified_at` used wall-clock time instead of the injected
> clock (now single time-of-check across freshness, replay,
> and audit-log timestamp). Plus four coverage tightenings:
> control / format characters in field values rejected
> (NUL-byte log-truncation + visually-identical-but-byte-
> different bypass); 32-byte minimum HMAC key length enforced
> on both sender + verifier (a deployment partner using an
> 8-byte key thought they had 2^256 security; reject loud);
> brittle `inspect.getsource` constant-time pin replaced with
> a behaviour-based `unittest.mock.patch.object(hmac,
> 'compare_digest')` test asserting the helper is actually
> called at runtime; clock-rewind cache survival pinned. Doc
> drift fixed (§4 said "five checks" but listed seven; now
> "seven"); §3 documents the new replay/freshness invariant
> + the timezone-aware ISO 8601 requirement + the 32-byte
> key minimum.
> **1117 tests passing**
...
| **Calibration management + drift detection** *(post-v0.7.x, provisional)* | Deployment unit — `CalibrationSet` hash-identified, kernel-version-validated bundle of 8 typed configs + `CalibrationDriftDetector` over `WindowedFleetSummary` | *Can the same calibration deploy across a fleet > 10 vehicles, and does the live fleet drift from it?* — the configuration-management gate every multi-vehicle deployment partner asks. SHA-256 digest = identity; expected_metrics ranges = drift signal. See `CALIBRATION_DESIGN.md`. |
| **Sensor attestation interface** *(post-v0.7.x, provisional)* | Predictor-admission gate — `SensorAttestation` per-message record + `SensorAttestationVerifier` seven-check pipeline | *How do you stop a stealth-bias spoof the kernel's Lemma-1 invariance can't see by construction?* — closes the UN ECE R155 cybersecurity loop the adversarial family opened. Stdlib-only HMAC-SHA256 + replay cache + freshness window + firmware allowlist + data-binding digest + constant-time compare. Failed verification unions into the existing `is_excluded` mask. See `SENSOR_ATTESTATION_DESIGN.md`. |
...
| **Calibration management + drift detection (post-v0.7.x)** | `calibration/` package + `CALIBRATION_DESIGN.md`. **The "fleet > 10 vehicles" deal-unlock answer**: a frozen, hash-identified `CalibrationSet` bundles 8 typed configs (BCVFConfig + ConsumerV2Config + BicycleConfig + RealTimeBudget + DDSQoSProfile + SafetyStateMachineConfig + per-predictor FailureConfig + an `expected_metrics` range map) into a single JSON artifact a fleet operator distributes, signs, and version-controls. **SHA-256 digest over canonical JSON is the bundle's identity** (same content → same digest; tampering / hand-edit fails the digest gate at load time as `CalibrationDigestError`); **`kernel_version` field is validated against `bcvf_autonomous.__version__`** at load (`CalibrationVersionError` unless caller explicitly opts in via `allow_version_drift=True` after verifying the kernel changes don't affect their tuning). Strict-validation rejects whitespace-only `calibration_id`, non-ISO-8601 `created_at`, expected-metric ranges with inverted bounds, and — load-bearing — every embedded config dict re-instantiates its source dataclass at construction so a malformed `BCVFConfig` (missing key / unknown `CostOrder` name / ndarray shape mismatch) can't smuggle past the validator. **CalibrationDriftDetector** walks the bundle's `expected_metrics` against a live `WindowedFleetSummary` and emits typed `CalibrationDriftAlert` records per range violation — composes with `StreamingFleetMonitor` via the same dotted-path metric resolver `AlertRule` walks; the deployment partner unions both alert streams. The detector is intentionally NOT integrated into the monitor by default — the monitor stays calibration-agnostic, the integrator wires the detector alongside their existing `evaluate_alerts(rules)` call. SOTIF clause 12 (release-to-market + configuration management) gains both `CalibrationSet` + `CalibrationDriftDetector` as sibling deployment artifacts to the CycloneDX SBOM that shipped with §9-row-#2. SOTIF_TRACEABILITY.md regenerates 37 → **39 indexed artifacts**. Nine new symbols enter `PROVISIONAL_API` (count lock 58 → **67**); `STABLE_API` graduation gated on five explicit criteria in `CALIBRATION_DESIGN.md` §8 (deployment partner one quarter on a fleet ≥ 10 vehicles, real fleet drift detection across a known calibration-mismatch incident, signed bundle field via deployment-partner key, external auditor sign-off on bundle JSON shape as ISO 26262 §12 evidence, expected_metrics schema stabilised across ≥ 3 deployment partners' real tuning-range distributions). The §9 row #6 of `INDUSTRY_FEATURES_ROADMAP.md` is struck through with a pointer to the design doc per the §11 maturation path. **36 pinning tests + 10 design-doc pins** (digest determinism + canonical-JSON stability; round-trip + tamper detection at every embedded-config gate; ISO-8601 + semver validation; expected-metrics bounds validation; drift-detector range classification — in-range / above / below / missing-metric / typo-path-raises; canonical to_dict shape pin). |
| **Sensor attestation interface (post-v0.7.x)** | `attestation/` package + `SENSOR_ATTESTATION_DESIGN.md`. **The UN ECE R155 cybersecurity loop, closed**: the safety case has documented the Lemma-1 trapdoor since v0.7 (stealth-bias spoofs invisible to the kernel by construction); SOTIF clause-8 Insufficiency #3 named sensor attestation as the in-scope mitigation; this surface makes the mitigation real. **Stdlib-only HMAC-SHA256** (`hashlib` + `hmac` + `secrets`; no `cryptography` library; no PKI; no key-management — the integrator wires a `key_resolver` callable to their HSM / TPM / secure enclave so key material never enters the framework). **`SensorAttestation`** is a typed per-message record (predictor_name + firmware_version + signature + nonce + issued_at + data_digest + metadata) with construction-time validation of every field including hex-and-length checks on the digests. **`SensorAttestationPolicy`** is the per-predictor verification policy (firmware allowlist + freshness window + replay window + key_id + enabled flag); validates positive-window discipline + non-empty firmware names at construction. **`SensorAttestationVerifier.verify`** runs **seven checks in §4 order**: (1) policy lookup raises `UnknownPredictorError` on configuration bug, (2) policy-disabled short-circuit returns `passed=True` with `policy_enabled=False` for the audit trail, (3) firmware allowlist check (empty = test mode = accept any), (4) freshness window check, (5) future-dating rejection (clock-skew / mis-signed-clock attacks), (6) replay-cache nonce check (per-predictor `OrderedDict` bounded by `replay_window_seconds`; entries older than the window evict on next access), (7) data-binding digest check (closes swap-trajectory attacks), (8) HMAC-SHA256 signature check via `hmac.compare_digest` (constant-time — pinned by behaviour test that inspects the source for the helper invocation). `verify_batch` walks a sequence + raises `AttestationError` on count mismatch. The verifier is intentionally NOT integrated into `BCVFNodeBehaviour` by default — the integrator runs it alongside the node, unions `not result.passed` into the existing `is_excluded` mask the trust computer consumes; **three exclusion sources stack via `np.logical_or`** (deadline-driven from BCVFNode + state-machine-driven from SafetyStateMachine + attestation-driven from this surface). The audit log differentiates causes via the typed `failure_reason` field. **Sender-side helpers**: `compute_data_digest(trajectory)` produces a deterministic SHA-256 over a C-contiguous float64 cast (invariant under integrator's downstream dtype upcasting); `canonical_signing_payload` builds the pipe-delimited byte string the HMAC signs (rejects pipe in field values to prevent ambiguous-bytes attacks); `sign_attestation` is the symmetric reference signer (the verifier's counterpart). SOTIF clause 8 (Insufficiency #3) gains both `SensorAttestation` + `SensorAttestationVerifier` as the **in-scope mitigation evidence** alongside the existing `_ADVERSARIAL_FAMILIES` (the scope-boundary documentation) and `_SAFETY_STATE_MACHINE` (the system-level escalation). SOTIF_TRACEABILITY.md regenerates 39 → **41 indexed artifacts**. Ten new symbols enter `PROVISIONAL_API` (count lock 67 → **77**); `STABLE_API` graduation gated on five explicit criteria in `SENSOR_ATTESTATION_DESIGN.md` §8 (deployment partner one quarter against HSM key-resolver, real attestation-failure detection across a firmware regression, asymmetric-attestation extension subclass, external auditor sign-off as UN ECE R155 §7.3.4 evidence, replay-cache persistence layer for cross-process nonce dedup). The §9 row #8 of `INDUSTRY_FEATURES_ROADMAP.md` is struck through with a pointer to the design doc — and **the LAST entry in `_ROADMAP_TOKENS` is removed**: every six §9-shippable rows (#1, #2, #3, #4, #6, #8) now landed; the non-promotion gate is empty. **45 pinning tests + 11 design-doc pins** (per-field construction validation incl. hex / length / non-whitespace / ISO-8601 + verifier construction guards + happy path + each of the seven failure-reason regressions individually + replay-cache eviction + within-window persistence + verify_batch shape + sender-side helper invariance under dtype upcast + pipe-rejection in canonical signing + composition with the existing exclusion mask + AttestationResult serialisation + `hmac.compare_digest` source-inspection pin). |
| **Public-API stability commitment (v0.4.0)** | `_api.py` registry — 38-symbol `STABLE_API` + **77-symbol** `PROVISIONAL_API` (was 20; +9 SafetyStateMachine surfaces post-v0.7, +12 ROS 2 / DDS / SBOM integration-contract surfaces post-v0.7.x, +10 replay-framework surfaces post-v0.7.x, +7 real-time-budget surfaces post-v0.7.x, +9 calibration-management surfaces post-v0.7.x, +10 sensor-attestation surfaces post-v0.7.x), both as flat tuples of canonical `submodule.Symbol` paths machine-checked at every commit. `API_STABILITY.md` documents the three tiers (stable / provisional / internal), the semver mapping (patch / minor / major triggers), and the deprecation cycle (post-1.0 stable removal requires one-minor-version notice + `DeprecationWarning` + release-note line). `__version__ = "0.4.0"` and `VERSION_INFO = (0, 4, 0)` agree by test pin. Top-level `from bcvf_autonomous import X` continues to work for the existing re-exports, but the contract is the explicit 38-symbol registry — the v0.2 brief's "tested integration contract" promise becomes the machine-checkable thing the test suite enforces. |
