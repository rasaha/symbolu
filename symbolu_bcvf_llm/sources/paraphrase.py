...
# Increment this tag whenever the paraphrase template or cleaner changes
# *intentionally* — it's a human-readable sibling to the content hash.
PARAPHRASE_VERSION_TAG = "v2.1-seed-diversity"


def paraphrase_pipeline_version() -> str:
    """Stable hash of everything that affects paraphrase output.

    Used by the disk paraphrase cache to refuse to load entries
    generated by a *different* template / directive set / cleaning
    behavior. Without this guard, a cache file written by V1's
    corrupted template would silently feed wrong data to a V2
    benchmark run.

    Changing any of:
      - DEFAULT_REWRITE_INSTRUCTION
      - V1_REWRITE_INSTRUCTION (mostly for provenance; V1 is frozen)
      - _SEED_STYLE_DIRECTIVES
      - PARAPHRASE_VERSION_TAG (for force-bump without content changes)
    will change this hash and cause old caches to be discarded on load.

    Returns: first 16 chars of a sha256 over the canonical json serialization.
    """
    spec = {
        "tag": PARAPHRASE_VERSION_TAG,
        "default_template": DEFAULT_REWRITE_INSTRUCTION,
        "v1_template": V1_REWRITE_INSTRUCTION,
        "style_directives": list(_SEED_STYLE_DIRECTIVES),
    }
    serialized = _json.dumps(spec, sort_keys=True).encode("utf-8")
