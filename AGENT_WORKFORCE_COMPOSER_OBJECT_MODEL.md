...
Hiring/Model Selection (`Candidate`); those are flagged **⚠ reconcile** and are resolved by the canonicalization
decision in the roadmap Phase 0 (either AWC owns the canonical name and H16 re-exports — the
`execution_gate`→`ugence_model_selection` facade pattern — or AWC uses a distinct name).
...
**`AgentProfile`** ⚠ reconcile with H16 `AgentProfile` — evidence-backed agent manifest.
```
...
**`AgentAssignment`** ⚠ reconcile with H16 `AgentAssignment` — role→agent binding.
```
