...
*Prepared for technical diligence. Every capability is presented as a problem it
attacks, a specific mechanism, the evidence that exists today, an honest
conclusion, and the measurable objective a customer pilot must prove.*

---

## How to read this document

Each card separates two questions: **has the mechanism been built correctly?**
(tests, invariants, deterministic replay, benchmarks — where most modules are
strong today) and **does it create customer value?** (fewer unsupported assertions,
unauthorized actions caught, faster audit reconstruction — what the pilots are
designed to prove). Ugence has substantial software proof today; the round buys
business proof.

**Maturity taxonomy** (a module may carry several labels):
**Implemented** (passes code/integration tests) · **Internally Validated**
(synthetic/benchmark/internal data) · **Pilot Ready** (packaged for shadow-mode
evaluation) · **Pilot Validated** (bounded customer environment) ·
**Production Validated** (live customer workflow) · **Research** (hypothesis/
prototype).

> **Evidence boundary.** Unless a card says otherwise, all metrics are from our own
...
The portfolio is not a set of equal-weight inventions. It resolves into three tiers:

| Tier | Modules | Role |
|---|---|---|
| **Core commercial product** | ActionGate · Truth Assurance Platform · Decision Governance · Autonomous Control Plane · Agent Runtime | What the enterprise buys, governs, and enforces |
| **Product accelerators** | Context Minimization · Hybrid LLM · LLM Steering Controller · Model Selection · KVPro · Cloud Scaling Controller | Make the core cheaper, better, broader |
| **Research & optional IP** | Autonomous Runtime (BCVF robotics) · PSE | Valuable IP; not the initial commercial focus |

**Flagship shadow-mode pilot:** *AI-Assisted Hiring* — the origin domain that
proved the Decision Governance model and now consumes it as one application
(detailed under Decision Governance below).

---

# Tier 1 — Core commercial product

*The governed boundary the enterprise buys: authorize the exact action, clear it
against live safety, validate what the AI asserts, and record who was accountable —
external to the runtime that proposes.*

---

## 1 · ActionGate — authorize the exact action
...
[^ag]: *Caveats.* Does not yet establish production risk reduction. Independent
architectural validation sits at `SUPPORTED_WITH_LIMITATIONS` (single-host store,
trusted signing root, pure-Python crypto). Cross-domain breadth beyond Kubernetes
(AWS, GitHub, Terraform, databases) is roadmap, not shipped; no third-party
external red-team yet.

---

## 2 · Truth Assurance Platform (TAP) — validate what the AI asserts
...
where baselines escape 0.67–1.00.
...
endpoint by an order of magnitude in synthetic testing.[^tap]
...
[^tap]: *Caveats.* This is an **emerging** capability: architecture specified, one
layer prototyped on synthetic data; production efficacy is not established and no
ROI is claimed. A disclosed "no-tell" ceiling failure (fabricated provenance) still
escapes 1.000. All results are on synthetic, deterministic corpora.

---

## 3 · Decision Governance Middleware — record who was accountable
...
byte-identical decision hashes pinned in tests (517 → 528 after extraction, no
existing assertion modified). The AI-Hiring reference application passes its full
suite (**553/553** at terminal freeze).[^dg]

**Evidence basis.** Code tests (deterministic unit/contract/boundary) + synthetic
walkthroughs (e.g. prohibited-field quarantine). No production usage claimed.
...
kernel with a second domain already depending on it.[^dg2]
...
binding guarantees against messy real data.

[^dg]: *Test-count scoping.* "55" = new tests added in Phase 4A within AI-Hiring;
"413" = the AI-Hiring module suite total at that snapshot — **not** kernel-dedicated
counts. The kernel's own dedicated tests are 6 files / 29 functions; the AI-Hiring
reference app reaches 553/553 at terminal freeze.

[^dg2]: *Caveats.* The decision→action→execution linkage to the live control plane
(ActionGate/ACP) is **specified but not yet built end-to-end**. The module provides
controls that *may support* compliance — it does not ensure compliance or assert
bias-freedom. Automated semantic mapping and proxy-attribute detection are research.

### Flagship pilot — AI-Assisted Hiring
...
recommendation applied, which human authorized the binding decision, and whether
what executed matched what was authorized.
...
plus the ActionGate/control-plane port, enforcing the AI-advisory / human-binding
separation and emitting one append-only, hash-linked audit event per stage. A
bounded shadow pilot replays a synthetic cohort using **offline deterministic
adapters — no production HRIS writes, emails, payroll, or identity provisioning.**

**Current evidence.** The AI-Hiring reference application is terminal/frozen at
**553/553 tests passing**. A validation harness replays a **12-case synthetic
cohort deterministically** and asserts that authorization denials, reconciliation
mismatches, and evidence-insufficiency cases are all surfaced (never silently
passed); an audit-completeness check verifies 7 critical records including
hash-chain integrity; a reconstruction test proves a broken link fails
reconstruction.[^hire]

**Evidence basis.** Code tests + synthetic shadow-pilot design (in-memory
repositories, offline deterministic adapters, static identity stand-in).

**Honest conclusion.** For hiring, the decision contract — admissible-evidence
boundary, pinned rubric version, type-enforced no-AI-binding-decision, four
append-only records, reconstructable lineage — is built and tested on a bounded
synthetic cohort with deterministic replay.

**Next validation step / pilot success measures.** Run the harness in true shadow
mode alongside a live hiring workflow (real ATS/HRIS read feeds, real identity
provider, durable append-only audit backend, cryptographic hash-chain). Measures,
portable to a real cohort: reconstruction completeness, audit-completeness ratio
(zero critical-record failures on executed cases), evidence-insufficiency rate,
authorization-denial rate, reconciliation-mismatch / duplicate-execution rate,
override rate, and time to produce a complete audit package per case.

[^hire]: *Caveats.* Not yet shown running against a real hiring workflow; no
production persistence or real identity provider; makes no fairness or
legal-compliance claim. Fairness analysis is descriptive-only and the 12-case
cohort is explicitly flagged too small for any fairness conclusion (a real group
size ≥ 10 per label would unlock deferred fairness/counterfactual checks).

---

## 4 · Autonomous Control Plane (ACP) — clear it against live safety
...
**Evidence basis.** Code tests + clean-room differential conformance.

**Honest conclusion.** The operational-safety-clearance mechanism is built,
runtime-independent, and cross-domain, verified against fixtures.[^acp]
...
[^acp]: *Caveats.* ACP runs against authored fixtures in **shadow-only** mode and
actuates nothing — no live cluster/database telemetry yet. A reference control
plane with a proven contract, not a certified production deployment.

---

## 5 · Agent Runtime — the governed proposer
**Implemented · Internally Validated · Pilot Ready**
...
and used only to *raise scrutiny*, never to authorize.[^agent]
...
[^agent]: *Caveats.* "Runtime independence" here is demonstrated interoperability in
the repo — **not** a claim of market adoption; the CER is not yet an adopted
industry standard. A related internal-signal pilot (N=30) moved AUROC from 0.893 →
0.916, which was **not statistically significant** (p=0.183) — reported as an honest
negative, not a headline.

---

# Tier 2 — Product accelerators

*Make the core cheaper, better, and broader. Individually valuable; strategically,
they compound the core rather than stand alone.*

---

## 6 · Context Minimization — cheaper governed context
**Implemented · Internally Validated**
...
[^ctx]: *Caveats.* Recommendation is `LIMITED_GO`: absolute downstream accuracy is
depressed by a model-side tool-argument ceiling (repaired in a V2 benchmark), not by
the compression. The moat is the invariance contract — "a real head start, not an
insurmountable moat."

---

## 7 · Hybrid LLM — the governed reasoning substrate
**Implemented · Internally Validated**
...
**Honest conclusion.** The separation-of-duties architecture is built and, under
controlled conditions, admits zero unsupported facts where an ungoverned baseline
admits many.[^hybrid]
...
[^hybrid]: *Caveats.* All enterprise results are on a synthetic, controlled corpus
using a simulated interpreter, not a live model; live-model interpretation and
generalization beyond the corpus are not yet established.

---

## 8 · LLM Steering Controller — deterministic generation steering
**Implemented · Internally Validated**
...
with improved framing and preserved factuality on one open model; its
differentiators — determinism, auditability, model-agnosticism — are true *by
construction*.[^steer]
...
[^steer]: *Caveats.* Measured quality gains are single-model, single-set,
rubric-scored, and not yet human-validated.

---

## 9 · Model Selection & Governed Inference — govern which model may act
...
pre-registered safety criteria in synthetic evaluation.[^msel]
...
[^msel]: *Caveats.* **Real human validation is not yet performed** — two calibration
rounds were correctly *blocked* (no real reviewers; eligibility gate excluded a
stakeholder). It is a cross-cutting policy service at research maturity, not an
eleventh canonical component.

---

## 10 · KVPro — KV-cache efficiency (compression + eviction)
...
[^kv]: *Caveats.* Decode is throughput-negative (**~0.13–0.67× bf16**) on the current
unoptimized path (throughput recovery is a funded v2 item); patent-pending;
pre-revenue. The **eviction** sub-capability is at legacy/spec maturity — earlier
CTM+ hit-rate/concurrency numbers are historical, not independently reproduced on
the current ship, and CTM+ matches LRU (and slightly loses to ARC) on structureless
traces; present it as a roadmap lever within KVPro, not a separately-proven product.

---

## 11 · Cloud Scaling Controller (Autoscaling Safety Interlock) — scaling-decision quality
**Implemented · Internally Validated · Pilot Ready**
...
correct futility detection across simulation and a real trace replay.[^scale]
...
[^scale]: *Caveats.* This is a reliability/safety play, **not** cost-optimization
(measured savings are marginal and deliberately not the pitch); no production/
customer validation, and a pre-registered kill signal is defined.

---

# Tier 3 — Research & optional IP

*Valuable intellectual property and adjacent markets. Not the initial commercial
focus; included for completeness and optionality.*

---

## 12 · Autonomous Runtime (BCVF Autonomy Runtime) — physical-AI arbitration
**Implemented · Internally Validated · Pilot Ready** *(adjacent vertical: robotics)*
...
realistic-noise scenarios, with the safety-case artifacts a Tier-1/OEM asks
for.[^auto]
...
[^auto]: *Caveats.* No production deployment; explicitly does **not** transfer to
LLM hallucination routing (clean null, AUC ≈ 0.5); cannot alone catch a stealth
spoof below the kernel threshold. Robotics is an adjacent vertical, not the initial
enterprise wedge.

---

## 13 · PSE — deterministic engine for naming & verbal identity
...
(studio UI, observation platform, enterprise APIs) are the build ahead.[^pse]
...
[^pse]: *Caveats.* Explicitly claims **no intrinsic sound meaning** and no scientific
validation of the vocabulary — observations are measured associations, never
universal truths. The "schema as interchange standard" is a high-variance bet, a
moat only if an ecosystem adopts it.
