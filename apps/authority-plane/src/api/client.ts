 ...
 * worker implements the two grant writes but serves neither under AP-3 (§18: the owner
 * reversed AW-1), and until the contract names them served this client cannot name them.
 */

const BASE = import.meta.env.VITE_WORKER_BASE_URL || "/api";

/**
 * The header the worker reads a write's proof from, as its contract names it. This
 * build sends no write and therefore never sends this header; it is declared so the
 * boundary verifier can hold the client to the contract's header when a write is
 * re-served.
 */
...
async function get<T extends ReadEnvelope>(path: string): Promise<Outcome<T>> {
  ...
  try {
    // a read carries no proof: the header above is never sent
