...
studio's review client cannot reach anything but the seven audited routes, an
...
# the outbound edge
# --------------------------------------------------------------------------- #
def test_the_review_client_reaches_exactly_the_seven_audited_routes():
    assert len(REVIEW_ALLOWED_ROUTES) == 7
    assert [m for m, _p in REVIEW_ALLOWED_ROUTES].count("POST") == 2
    assert REVIEW_ALLOWED_ROUTES[-2] == ("POST", "/review/runs")
    assert REVIEW_ALLOWED_ROUTES[-1] == ("GET", "/review/audit/{correlation_id}")
