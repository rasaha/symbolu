...
**Notes.** Modules: kernel (``core.py``), trust shaping (``trust.py``), planner (``mppi_planner.py``), diagnostics (``trust_diagnostics.py``), runner (``runner.py``), analysis (``analysis/``). Interfaces are typed dataclasses (``BCVFConfig``, ``RunConfig``, ``ConsumerV2Config``); each module ships a DESIGN.md.
