...
def _canonical_json(payload: Dict[str, Any]) -> str:
    ...
    no whitespace, ensure_ascii=True. Same byte-output for
    semantically-equal dicts is the load-bearing property —
    the digest is computed over this string."""
    return json.dumps(
    ...
    def from_dict(
        ...
        *,
        ...
        bundle = cls(
            ...
            kernel_version=str(payload["kernel_version"]),
...
def build_calibration_set(
    *,
    ...
    bundle_no_digest = CalibrationSet(
    ...
    digest = bundle_no_digest.computed_digest()
    # Re-construct with the digest field set. The dataclass is
    # frozen so we can't mutate; build a fresh instance.
    return CalibrationSet(
        calibration_id=bundle_no_digest.calibration_id,
        kernel_version=bundle_no_digest.kernel_version,
        created_at=bundle_no_digest.created_at,
        bcvf_config=copy.deepcopy(bundle_no_digest.bcvf_config),
        consumer_v2_config=copy.deepcopy(bundle_no_digest.consumer_v2_config),
        bicycle_config=copy.deepcopy(bundle_no_digest.bicycle_config),
        realtime_budget=copy.deepcopy(bundle_no_digest.realtime_budget),
        dds_qos_profile=copy.deepcopy(bundle_no_digest.dds_qos_profile),
        safety_state_config=copy.deepcopy(bundle_no_digest.safety_state_config),
        per_predictor_failure_thresholds=copy.deepcopy(
            bundle_no_digest.per_predictor_failure_thresholds
        ),
        expected_metrics=copy.deepcopy(bundle_no_digest.expected_metrics),
        metadata=copy.deepcopy(bundle_no_digest.metadata),
        digest=digest,
    )
