"""Shared fixtures + kernel-lifecycle harness for ActionGate tests."""
...
def run_actiongate_lifecycle(control_plane) -> LifecycleResult:
    """Drive the full DGM lifecycle with ActionGate as the control plane.
    ...
    if resp.outcome.value not in ("AUTHORIZED", "AUTHORIZED_WITH_CONSTRAINTS"):
        # not authorized → no dispatch / execution / reconciliation
        try:
