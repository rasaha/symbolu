...
## DEC-3C — Phase 3C outbox relay + push delivery (DILCHAT-D3C-1…4, I1…I8)
**Status:** Accepted (owner-ratified) · **[Architecture/Privacy]** · Implemented as migration `f6a7b8c9d0e1`, the `ugence_dilchat.relay` package, and the `/v1/devices` surface. Phase 3C ends at notification plumbing; the mobile chat experience remains Phase 3D.

- **DEC-3C-1 — Expo Push is the V1 pilot transport (D3C-1 = A).** Push is advisory and non-authoritative (I5): REST/database state stays canonical and clients always re-fetch. Privacy boundary: push infrastructure exposes delivery tokens and timing metadata to Expo/Apple/Google — acceptable for the internal pilot, and an explicit privacy/legal review item plus APNs/FCM credentialing before any public production use.
- **DEC-3C-2 — MESSAGE_CREATED notifies; everything drains (D3C-2).** The relay consumes ALL five outbox event types and durably marks them published (I3); only `MESSAGE_CREATED` emits a notification, to the non-sender member only, with the fixed generic text "You have a new message." — never a sender name, body, relationship/astrology/safety information, or event ids. The transport port carries tokens only, so content-freedom holds by construction (I7). No push for provisioning, deletion, read-state, or revocation events.
- **DEC-3C-3 — Published-row pruning at 30 days (D3C-3).** Rows with `published_at <= now − 30d` are deleted; unpublished work is never age-pruned (I8, additionally DB-enforced by the worker's DELETE policy). Outbox infrastructure retention only: no chat message is deleted, message-retention policy and tombstone semantics are unchanged, and the D3B retention purge stays off.
- **DEC-3C-4 — Standalone worker, no HTTP surface (D3C-4).** `python -m ugence_dilchat.relay` under the `dilchat_worker` posture (I6); the web/API process never acquires the worker's outbox privileges. If the relay is down, messaging stays correct, REST and mobile polling continue, and push degrades independently (DEC-058).
- **Delivery semantics.** PUBLISHED means "handed to the configured external transport according to its acknowledgement contract" — never received/read by a user. Delivery is at-least-once from durably committed outbox rows only (I1/I2, bounded retry with exponential backoff); an unknown event type is parked with a content-free error code, never silently published (I4). Multiple relay processes may run concurrently without publishing the same claimed row (mechanism-neutral invariant; realised on PostgreSQL with skip-locked row claims).
- **Device model.** A push endpoint is a device installation belonging to the USER — never a session credential and never usable for authentication. One user may hold multiple active devices; a token maps to at most one ACTIVE registration globally (a new sign-in on a handed-over device displaces the previous owner's registration via the bounded `app_release_push_token` definer); permanent provider rejection deactivates the registration; logout of the registering session revokes that device, logout-all revokes them all; tokens are SENSITIVE — never logged, audited, or echoed, and withheld from the read-only and safety roles entirely.

---

## DEC-3C-M — Phase 3C mobile slice: config-gated push registration (DILCHAT-D3C-M1/M2)
**Status:** Accepted (owner-ratified) · **[Product/Security]** · Mobile-only slice over the merged 3C backend (`/v1/devices`); no Phase 3D chat-UI, WebSocket, presence, or media work.

- **DEC-3C-M1 — Narrow amendment of DEC-M2-4.** The minimized Android permission posture is amended ONLY for the ratified content-free push capability: `INTERNET` plus the notification permission the pinned `expo-notifications` build demonstrably requires (`POST_NOTIFICATIONS`, contributed by the LIBRARY manifest at Gradle merge time — not added to the app manifest by hand). `READ/WRITE_EXTERNAL_STORAGE`, `SYSTEM_ALERT_WINDOW` stay prohibited; `VIBRATE` stays blocked (the pinned build functions without it and least privilege governs); `allowBackup=false` unchanged. The generated app manifest was audited post-`expo prebuild` (unchanged: `INTERNET` active, four removals) and the expected ACTIVE permission set is now PINNED — exactly `INTERNET` — in `scripts/check-native-android.sh` and the android-build CI guard, so a later Expo change cannot silently widen permissions.
- **DEC-3C-M2 — Config-gated token acquisition.** The EAS project id is deployment configuration resolved via the established Expo config mechanism (`DILCHAT_EAS_PROJECT_ID` → `extra.eas.projectId`), never hardcoded (guarded by `scripts/check-config.js`). Absent id ⇒ NO acquisition attempt, fake token, placeholder registration, startup failure, or warning loop — the app runs on REST + polling. The gate chain is config → signed-in → permission → acquisition → authenticated `POST /v1/devices`; every failure mode (denied permission, missing config, acquisition/provider/registration failure) degrades silently and never blocks login, pairing, or chat, nor invalidates a session. Missing config is distinguishable from transport failure in the returned status (`skipped_no_config` vs `failed_transport`) without exposing tokens. A user who declined is never re-prompted (at most one prompt per launch, only from `undetermined`). Notification permission is consent to content-free delivery notices only — never to analytics, marketing, or content disclosure. Device-token posture follows DEC-3C: not an authentication credential, never logged or displayed, replaceable and revocable; logout revocation is the backend's contract.

**Architectural invariant:** push availability never determines DilChat messaging correctness.

---

## DEC-PR — Production-readiness track (DILCHAT-D-PR-1…5)
**Status:** Accepted (owner-ratified) · **[Product/Architecture/Privacy]** · Next milestone after the merged Phase 3C mobile slice.

- **DEC-PR-1 — Milestone (D-PR-1 = A, with B at the tail).** Production-readiness foundation → operational/security/privacy hardening → retention-policy implementation → deployment validation → minimal privileged moderation read-back → readiness review. The moderation tail stays deliberately minimal (list reports/cases, read one case, read authorized evidence, provenance + audited access) with NO automated adjudication, AI moderation, reporter-facing evidence read-back, or broad admin console — preserving DEC-3B-3.
- **DEC-PR-2 — Deployment target (D-PR-2 = INTERNAL_PILOT).** Engineering target: production-capable architecture. Release target: controlled internal pilot (authorized participants, non-public distribution). Public production is a separate later owner launch decision that re-assesses privacy/legal gates, moderation operations, retention, provider credentials/licensing, incident response, and support. A successful pilot never auto-authorizes launch.
- **DEC-PR-3 — REVOKED_PENDING_POLICY retention (D-PR-3).** Revocation retains the conversation ≥30 days after `revoked_at` (the existing bounded post-revocation reporting window must never be undercut). After the window, purge ELIGIBILITY — not automatic deletion — requires ALL of: `retention_purge_enabled=true`, age ≥30d, state still REVOKED_PENDING_POLICY, no preservation reason, no PRESERVED_FOR_REPORT, no legal/operational hold, no policy exception. PRESERVED_FOR_REPORT dominates unconditionally. **Amendment:** the flag stays `false` until purge tests, preservation-state tests, report-window boundary tests, documented backup implications, legal/privacy review of the period, and dry-run/report-only evidence all exist.
- **DEC-PR-4 — Reviewer principal model (D-PR-4).** Internal operator tooling over the dedicated `dilchat_safety` DB posture: authenticated internal operator → operator authorization layer → safety service → bounded reads. The DB role is enforcement, never the human identity model — each access carries an individual authenticated reviewer principal and produces an audit event; no shared admin identity, no direct DB connections, least-privilege reads, content never in general logs, no reporter access. A separate privileged authn/admin surface waits until moderation grows real workflow needs, and is expected before broader external production.
- **DEC-PR-5 — AI Assist and Guna stay parked (D-PR-5).** No AI moderation, AI relationship advice, LLM access to report evidence, executable Guna scoring, user-visible compatibility scoring, or new astrology decision logic in this track. Production-readiness hardens what exists; it does not expand the semantic surface.

**Round PR-A (implemented with this entry):** staging/production fail-fast guards (debug off, explicit `postgresql+asyncpg` DSN — dev default refused, https Expo URL, signing key required) with `Settings` repr credential hygiene; telemetry content-freedom hardening (extended structlog redaction keys; relay `last_error_code` clamped to machine codes) with pinning tests; migration-cycle and backup/restore validation scripts wired into CI; operations runbook (`docs/DILCHAT_OPERATIONS_RUNBOOK.md`). No retention purge, moderation, or deployment-artifact work — those are later rounds (PR-B…PR-E).

**Round PR-B (implemented with this entry):** purge **eligibility** per DEC-PR-3 as a pure, total evaluation (`services/retention.py`) plus a read-only dry-run report (`python -m ugence_dilchat.scripts_retention_report`) that lists what a purge would consider and deletes nothing — the only executable retention path in this round. Migration `a7b8c9d0e1f2` adds the `hold_reason`/`hold_placed_at` columns so the ratified "no legal/operational hold" condition is represented honestly rather than assumed; a held row and a `PRESERVED_FOR_REPORT` row are never eligible at any age. New setting `chat_retention_revoked_days` (30) is refused at construction if it would undercut `chat_report_after_revocation_days`. `retention_purge_enabled` stays `false`; no destructive code path exists, pinned by a source guard test. Backup implications documented in the operations runbook: purging the live database does not purge backups, so the backup retention period is part of the activation decision.

**Round PR-C (implemented with this entry):** deployment artifacts and pilot operations. One image (`deploy/Dockerfile`, non-root, no AGPL `swiss` extra and no test toolchain) runs BOTH processes; role separation (DEC-3C-4 / I6) is expressed by the DSN each is given — `deploy/docker-compose.pilot.yml` wires a one-shot owner-role migration, then web as `dilchat_app` and the relay as `dilchat_worker` with no ports. Migrations are never run by an application process. New `python -m ugence_dilchat.scripts_preflight` validates configuration, database reachability, the CONNECTED ROLE against the expected one, and the schema revision against the code head before a process serves; new `python -m ugence_dilchat.scripts_relay_health` reports relay liveness from a content-free heartbeat file (timestamp only, atomic write, best-effort so a failing heartbeat never stops delivery), with a startup guard that the staleness bound exceeds the idle poll interval. Docs: `DILCHAT_PILOT_CHECKLIST.md` (nothing ticked without a command or a named sign-off; explicitly does not authorize public launch) and `DILCHAT_INCIDENT_RUNBOOK.md` (standing rules: messaging correctness never depends on push; never widen access to diagnose). CI gains a `deployment-artifact` job that builds the image and proves the non-root user, dependency posture, both entry points, single migration head, and fail-closed probes.

**Round PR-D (implemented with this entry):** the minimal privileged moderation READ-BACK ratified by DEC-PR-4, and nothing more. Migration `b8c9d0e1f2a3` adds `safety_reviewers` — one row per INDIVIDUAL internal reviewer with an Argon2 credential (shown once at provisioning, never stored in plaintext, logged, or echoed) and a revocable ACTIVE/REVOKED lifecycle — reachable ONLY by the safety posture: the application role that serves every user-facing route is granted nothing at all, so a user-facing API cannot authenticate a reviewer even by mistake, and `dilchat_readonly` cannot see reviewers either (proven under non-owner roles in `tests/security/test_reviewer_rls.py`). `services/moderation.py` exposes exactly three reads — list cases, read one case, read a report's preserved evidence — each requiring a `ReviewerPrincipal` by type signature and a machine-style access reason, and each appending an immutable case event attributed to that individual (`actor_internal_id`) with the reviewer label, role, per-invocation session id, and reason; the new `CASE_ACCESSED` event type records that a reviewer LOOKED at a case, not only that one changed. Audit metadata stays content-free: it records THAT a case or N evidence items were read, never what they said. Operator tooling is `python -m ugence_dilchat.scripts_moderation` (credentials via environment only — argv is world-readable), which offers no adjudication subcommand. Reports stay SUBMITTED and case state is untouched (DEC-3B-3 preserved): reading a case is not deciding it. No reporter-facing evidence read-back (DEC-3B-5), no shared admin identity, no direct-DB reviewer access, no AI moderation, no broad admin console — pinned by source guards in `tests/security/test_moderation_boundaries.py`.

**Round PR-E (implemented with this entry):** the internal-pilot readiness review (`docs/DILCHAT_PILOT_READINESS_REVIEW.md`), assessing the merged tip against every item of the pilot checklist with `[V]`/`[I]`/`[R]`/`[G]` evidence labels. Verdict: **the engineering track is complete; the pilot is NOT ready to start.** Ten of the 27 items are fully verified by exercises actually run against a migrated PostgreSQL 16 database and the running app — migration cycle, preflight, health/readiness, production-like refusal, relay heartbeat probe, a full register→profile→invite→accept→chat journey, outbox drain, retention dry run, backup/restore with real chat data, reviewer provisioning and audited listing, and the 401-test suite. The other 17 carry an unresolved dependency and cannot be settled from the repository: they require a deployed pilot environment or owner/operator sign-off (Swiss licensing decision, named incident-response operator, named accountable reviewer, participant authorization and disclosures, push-transport choice, LOGIN grants and secret-store supply). Blocking gaps recorded: no pilot environment exists, and no backup job runs on a schedule with a rehearsed restore. This review does not authorize a public launch — that remains a separate owner decision (DEC-PR-2), and the standing limits (purge off, moderation reads but never adjudicates, AI/Guna parked) are unchanged.

---

## DEC-PL — Internal pilot posture (DILCHAT-D-PL-1…5)
**Status:** Accepted (owner-ratified, complete) · **[Product/Operations/Privacy]** · Authorizes **preparation** for a controlled internal pilot only. D-PL-1…5 are fully ratified: the D-PL-3 names were supplied in a follow-up owner completion and are recorded below.

- **DEC-PL-1 — `qa` posture with the Swiss AGPL development adapter (D-PL-1 = C).** The first pilot runs under `qa` so that no Swiss production-licensing decision is created prematurely; that decision (Professional License **or** documented AGPL-compliance posture) remains required before staging/public production, and nothing here claims this configuration is production-suitable. The trade-off is explicitly accepted: `qa` is not `is_production_like`, so the round PR-A guards would not apply automatically. The owner nevertheless required the pilot to mirror production configuration discipline. **Implemented as enforcement, not convention:** `DILCHAT_PILOT_MODE=true` applies the strict guards whatever the environment label says (debug off, non-ephemeral signing key, explicit `postgresql+asyncpg` DSN, https push endpoint) and refuses the synthetic `fake` provider outright — even where the `qa` opt-in would permit it. Guard messages name `pilot_mode` rather than "production" when it is the pilot enforcing them, and preflight reports `pilot_mode` and `strict_config_guards` so the posture is visible rather than assumed.
- **DEC-PL-2 — Null push transport (D-PL-2).** The first pilot sends no notification to Expo/Apple/Google. The relay still consumes the transactional outbox, exercises claim/retry/publish bookkeeping, drains every supported event type, and preserves the Phase 3C operational path — push remains advisory (I5). Enabling Expo Push is a later pilot round under the already-ratified DEC-3C-1 privacy boundary.
- **DEC-PL-3 — Named human accountabilities (D-PL-3).** An Incident Response Owner (triage, participant-impact assessment, authority to suspend the pilot, escalation, recovery, incident closure) and a Safety Report Reviewer (review submitted reports, access evidence only through the authorized reviewer surface, maintain confidentiality, record review activity). One person may hold both for the initial pilot. These are human accountabilities, not technical roles: the provisioned reviewer principal must map to the named reviewer, never to a shared admin identity. Report review implies no adjudication or enforcement workflow. **Named accountabilities (owner completion):** Incident Response Owner — **Rakesh Mohan**; Safety Report Reviewer — **Rakesh Mohan**. The same individual is authorized to hold both accountabilities **for the initial controlled internal pilot only**; separation of incident response from safety review must be reconsidered before broader/public production, and before any additional operator or reviewer is introduced. The provisioned reviewer principal must map to Rakesh Mohan as an individually attributable operator identity — never a shared generic admin identity (the surface has no such identity to offer, DEC-PR-4).
- **DEC-PL-4 — Invite-only, non-public distribution with an approved disclosure (D-PL-4).** Participants are individually approved by the pilot owner, access is revocable, and distribution uses a non-public channel (TestFlight / EAS internal). No open registration, public download link, store listing, or uncontrolled invitation forwarding. The owner-approved participant disclosure is recorded verbatim in `docs/DILCHAT_PILOT_DISCLOSURE.md`, including the push paragraph that must be added **only** if Expo Push is later enabled — and must not appear while the transport is `null`.
- **DEC-PL-5 — Daily backups, 35-day retention, monthly restore rehearsal (D-PL-5).** Encrypted at rest, access restricted, restoration documented, failures surfaced operationally. A successful backup is not validated recovery — only an actual restore rehearsal is; an extra rehearsal is required before any purge activation. 35 days deliberately exceeds the 30-day purge-eligibility window so backup retention never undercuts application retention. **Backup retention is not a mechanism for circumventing deletion policy:** an application purge is not immediate physical disappearance from historical backups, and the deletion design must state how purged data ages out of backups before `retention_purge_enabled` may ever become true (a D-PR-3 privacy/legal review item).

**Pilot boundary.** This authorizes preparation for a controlled internal pilot only. It does not authorize public production, store release, the Swiss production licensing posture, Expo Push activation, Phase 4A AI Assist, Guna execution, automated moderation, or `retention_purge_enabled = true`.
