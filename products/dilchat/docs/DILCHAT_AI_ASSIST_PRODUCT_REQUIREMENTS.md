...
gradually, and never on elapsed time or raw message count alone. The 60 % start
and 30 % floor are **binding**; a finer split of the remaining share
(**versioned calibration default**: 25 %/15 % explicit-preferences/early-evidence
at cold start → 20 %/50 % at maturity) is **tunable through controlled
evaluation** — see
[fusion §3 / §3.1](DILCHAT_RELATIONSHIP_SIGNAL_FUSION_REQUIREMENTS.md#3-structural-prior-weighting-60--30).
Moon receptivity is applied **separately** as a **bounded, temporary modifier**
(`TopicReceptivity = baseline_topic_affinity × moon_climate_modifier`; calibration
default bound 0.80–1.20) and never replaces the Guna signal. The binding
invariants vs. tunable calibration defaults are catalogued in
[acceptance criteria §0](DILCHAT_AI_ASSIST_ACCEPTANCE_CRITERIA.md#0-two-tiers-binding-invariants-vs-configurable-calibration-defaults).
