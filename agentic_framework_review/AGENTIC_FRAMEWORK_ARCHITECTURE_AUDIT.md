...
5. **FACT — naming/version incoherence.** The same product is "Agentic Framework" / "Sentinel" / shipped under "Ugence Labs" (now "Ugence Labs"); versions span v1.4.0 → v6.2.0 across docs; two Python import roots (`agentic.agentic_framework` vs `symbolu.agentic_framework`) are documented.
