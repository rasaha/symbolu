...
adds a three-layer workflow (PMM → PR → CR) that makes the reasoning explicit and hardens the test against
...
- **Layer 1 — Percentage Mapping Mapped (PMM):** how well-supported the frozen *mapping itself* is for this
...
## 3. Definitions — PMM ≠ PR ≠ CR (independent measurements)

### 3.1 Percentage Mapping Mapped (PMM) — Layer 1, documentation only

> **PMM (Percentage Mapping Mapped):** The percentage representing how strongly the frozen varṇa mapping is
> supported by objective evidence from philosophy, history, linguistics, symbolism, function, or established
> traditional understanding for that specific word. PMM measures how well the mapping itself is evidenced, **not**
> whether the word resolves or embodies that mapping.

For each consonant occurrence, record the **strongest evidence supporting** and the **strongest evidence
opposing** the frozen mapping's evidential relevance to the word (philosophy, history, linguistics, symbolism,
function, traditional literature, stable characteristics, ordinary understanding — bidirectional, per V1 Stage-B
discipline). Then assign PMM on the frozen absolute scale `{0,25,50,75,100}`: **100** overwhelming evidence the
mapping genuinely relates to the word · **75** strong · **50** plausible-but-mixed · **25** weak/indirect · **0**
no defensible evidence.

**Clarifications (explicit):**
- PMM is an **evidence-strength score only**.
- PMM **never** contributes to `PASS`, `PARTIAL_FIT`, `FAIL`, or `INDETERMINATE`.
- A mapping may have **PMM = 100% while PR = 0%** if the word strongly **embodies** that affliction rather than
  resolving it (a high PMM is not support — it is equally consistent with embodiment; see §7 krodha).
- **PR remains the only verdict-determining layer.**
- **CR remains explanatory only and cannot rescue PR.**
...
**CR must NEVER:** modify PMM or PR; rescue a failed component; remove an embodiment; convert a PR PASS/FAIL into
a different verdict; or turn PASS into "SUPPORT." CR is an **explanation of PR**, not evidence.

**Stated explicitly: PMM ≠ PR ≠ CR — three independent measurements. Only PR determines PASS or FAIL.**

## 4. Hard anti-rescue rule (preregistered)

> **If any component fails under the frozen PR rules (PR = 0, or the word fails V1 §8), neither PMM nor CR nor
...
2. **Layer 1 — PMM (new, documentation):** bidirectional evidence for/against each mapping's relevance; assign
   PMM. Records reasoning; touches no verdict.
...
5. **Report** all three (PMM, PR, CR) per component/word with full V1 §9 R14 traceability. **Evidence (PMM) →
...
These four demonstrate the *logic* of PMM vs PR vs CR. They are **not** precommitted words (V1 §10 forbids
...
- **PMM:** all ~**100** — grasping, an annihilation/destructive impulse, and craving are *obviously and
...
The mapping is very strongly evidenced (PMM = 100), but the stable referent **embodies** rather than resolves it
(PR = 0). Therefore the component **fails regardless of PMM**.
- **Verdict: FAIL** (multiple PR = 0). **High PMM did not help; high CR cannot rescue.** ← primary demonstration.
...
- **PMM:** ~**100/75** — loss of discernment and self-doubt are clearly related to fear.
- **PR:** ~**0/25** — fear **embodies** loss of discernment and self-doubt.
- **CR:** ~60. **Verdict: FAIL.** Again: high PMM, low PR → embodiment; CR cannot lift it.
...
- **PMM:** *j* ~75 (intellectual ego is a real theme), *ñ* ~25–50 (weak/indirect), *n* ~50–75 (attachment to
  views).
...
- **PMM:** all ~75–100 (freedom-from-desire and freedom-from-attachment are central to peace; peace-vs-dull-
...
**Takeaways demonstrated:** (i) **high PMM routinely coexists with low PR** — relevance is maximized by
...
**Changed (evaluation methodology only):** added Layer-1 **PMM** (non-scoring evidence documentation); added
Layer-3 **CR** (explanatory-only combined score); added the **HOLISTIC-ONLY FIT** diagnostic; added the
**hard anti-rescue rule** binding PMM/CR under V1 §13/§6; specified the evidence→PR-verdict→CR-explanation
ordering.
...
adversarial ~8–10-word mix (V1 §10) **before** any packet, then run Stage A → PMM → PR → CR and report per V1
...
B1.11 all unchanged; no run, no freeze. Only PR determines the verdict; PMM and CR can never rescue a failure.
