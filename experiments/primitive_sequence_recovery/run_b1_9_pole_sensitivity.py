"""B1.9 POLE-SENSITIVITY driver (Q2) + freeze gate (gated; blinded; mock-tested).

Tests whether the binding/liberating RESOLUTION carries meaning, with ZERO content confound: for each word W it
generates a reading at the CORRECT pole (frozen referent-ontology rule) and at the FLIPPED pole — same word, same
varṇas, same context, same plane, only the pole differs. Primary contrast: POLE_CORRECT vs POLE_FLIPPED.

Anti-circularity: the correct pole is fixed by frozen/b1_9_pole_referent_classification.json, which must be
operator-APPROVED (classification_approved == true) BEFORE any real run — the gate refuses otherwise.

Reuses the B1.6 adapter + shared leak matcher. NO real generation here (FakeAdapter only in tests). NO judging,
NO GENUTILITY_*. B1.4b′ remains NULL_RETURN_BOTTOM. Structure, not validated meaning.
"""
...
SCAFFOLD_FILE = FROZEN / "b1_9_pole_scaffold.json"
...
PREREG_FILE = HERE / "B1_9_POLE_SENSITIVITY_PREREG.md"
PROMPT_RUBRIC_FILE = HERE / "B1_6_SYMBOLU_GENERATIVE_UTILITY_PROMPTS_AND_RUBRIC.md"

MODE = "b1_9_pole_sensitivity_probe"
REPRESENTATION = "B1.9_pole_sensitivity"
ATTESTATION = ("B1.9 pole-sensitivity probe only; correct vs flipped pole on identical varṇas; correct pole "
               "from the frozen referent-ontology classification; no semantic-truth claim; no GENUTILITY "
               "terminal label; B1.4b′ remains NULL_RETURN_BOTTOM.")

ARMS = ("POLE_CORRECT", "POLE_FLIPPED", "PLAIN_PROMPT_BASELINE", "SEMANTIC_LLM_BASELINE")
FACET_ARMS = ("POLE_CORRECT", "POLE_FLIPPED")
PRIMARY_CONTRAST = ("POLE_CORRECT", "POLE_FLIPPED")

HASH_INPUTS = {
    "prereg_sha256": PREREG_FILE, "scaffold_sha256": SCAFFOLD_FILE,
    "classification_sha256": CLASSIFICATION_FILE, "targets_sha256": TARGETS_FILE,
...
BAD_MODES = {"pilot_generation", "exploratory_10_sample_generation_probe",
             ...
             "b1_9_generation_corrected_control_probe"}
...
def leaked(text: str) -> List[str]:
    """Blinding filter: ONLY hard method/arm identifiers (whole-word). General content words are NOT filtered —
    both poles draw from the same varṇas, so no differential attrition can arise from content filtering."""
    return list(G.leaked_tokens(text))


def classification_approved() -> bool:
    try:
        return json.loads(CLASSIFICATION_FILE.read_text()).get("classification_approved") is True
    except Exception:  # noqa: BLE001
        return False


# --------------------------------------------------------------------------------------
# Freeze gate (adds the classification-approval requirement)
# --------------------------------------------------------------------------------------
def verify_freeze_gate(decl_path: pathlib.Path, expected_mode: str = MODE,
    ...
    if decl.get("artifact") != "b1_9_pole_sensitivity_EVIDENCE_FREEZE_DECLARED":
        reasons.append("artifact != b1_9_pole_sensitivity_EVIDENCE_FREEZE_DECLARED")
...
# --------------------------------------------------------------------------------------
# Records / blinding / run — same structure as the B1.9 generation driver
# --------------------------------------------------------------------------------------
...
def make_judge_visible(rec: Dict, text: str) -> Dict:
    ...
    bad = {"arm", "true_arm", "plane", "stratum", "correct_pole", "flipped_pole"} & set(pkg.keys())
    if bad:
...
def make_hidden(rec: Dict, gen_code: Optional[str], gen_id: Optional[str]) -> Dict:
    return {"blinded_output_id": rec["blinded_output_id"], "true_arm": rec["arm"],
            ...
            "stratum": rec["stratum"], "plane": rec["plane"], "correct_pole": rec["correct_pole"],
            "flipped_pole": rec["flipped_pole"], "representation_version": REPRESENTATION}
...
def run_part(mock: bool = False, adapter=None, settings=None, decl_path: Optional[pathlib.Path] = None,
    ...
    if not mock:
        if decl_path is None:
            raise PermissionError("real run requires a B1.9 pole-sensitivity evidence-freeze declaration path")
    ...
    part = {
        "artifact_type": "b1_9_pole_sensitivity_part", "mode": "MOCK" if mock else "REAL",
    ...
    if write and out_dir:
        ...
        (out_dir / "b1_9_pole_part.json").write_text(json.dumps(part, ensure_ascii=False, indent=2))
    return part
...
def merge_parts(parts: List[Dict], out_dir: Optional[pathlib.Path] = None, write: bool = False,
    ...
    if reasons:
        return {"label": "B1_9_POLE_BLOCKED_ARM_RENDERING", "reasons": reasons}
    ...
    manifest = {
        "artifact_type": "b1_9_pole_sensitivity_run_manifest", "mode": parts[0].get("mode"),
        "representation_version": REPRESENTATION, "run_label": "B1_9_POLE_SENSITIVITY_PROBE",
        ...
        "note": "Pole is the ONLY variable between POLE_CORRECT and POLE_FLIPPED (same varṇas). Judge-visible "
                "re-blinded. No judging. No GENUTILITY_*.",
    }
    if write and out_dir:
        out_dir = pathlib.Path(out_dir); out_dir.mkdir(parents=True, exist_ok=True)
    ...
    return {"label": "B1_9_POLE_SENSITIVITY_DRIVER_READY_MOCK_TESTED", "manifest": manifest,
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="B1.9 pole-sensitivity driver (gated; mock-tested).")
    ...
    mp.add_argument("--reblind-seed", type=int, default=20260711)
    ...
    if args.cmd == "merge":
        ...
        if res["label"] != "B1_9_POLE_SENSITIVITY_DRIVER_READY_MOCK_TESTED":
