...
| **Planning algorithm** | RUNTIME-INDEPENDENT | RUNTIME-INDEPENDENT — the only "planner" token is a disclosure trust-level, not an algorithm (`remediation.py:38–42`) | RUNTIME-INDEPENDENT — "ACP consumes a planner," is not one (`acp/ACP_ARCHITECTURE.md`) |
