...
(async()=>{
  ...
  await page.goto('file://'+path.resolve('mirror.html'),{waitUntil:'networkidle0'});
  await page.pdf({path:'UGENCE_SERVICENOW_PRESENTATION.pdf',preferCSSPageSize:true,printBackground:true});
  const boxes=await page.$$('.page');
  for(let i=0;i<boxes.length;i++){ const bb=await boxes[i].boundingBox();
    await page.screenshot({path:`mslide-${String(i+1).padStart(2,'0')}.png`,clip:{x:bb.x,y:bb.y,width:1280,height:720}});
  }
