...
- Not novelty over the ISP category, the generic primitives, KV-on-flash, or the read-skip selection. MDPC/VSP/QACC as *generic* storage techniques are prior art; **KV offload to a flash/FTL tier is anticipated by InstInfer/HiFC** (§9.3 W2); and **the read-skip selection is approximate heavy-hitter retention** (Quest/H2O/SnapKV/StreamingLLM — §9.3 W1, verified against `readskip_select.py`), *not* an exact/bit-identical mechanism, so it is not claimable either. The single defensible wedge (if pursued for IP) is **W3 in §9.3**: *quantization-protection-structured KV placement across NAND tiers, whose robust value is capacity (≈2.22× tokens/GB vs bf16)* — the protection-structured tiering had no direct prior art in the 2026-06-12 sweep. The USE→splay scheduler is novel cross-portfolio reuse, not a novel scheduler per se (repulsive Kuramoto / desynchronization splay states are known — cf. DESYNC, IPSN'07).
...
**W1 — exact / output-equivalent attention-guided selection.** ***Anticipated — NOT a differentiator. Verified against the code 2026-06-12.*** Inspection of `CTM_plus/KVPolicy/kv_policy/readskip_select.py` shows the read-skip selection is **approximate heavy-hitter retention**: pinned attention sinks + recent window (StreamingLLM) ∪ top-`budget` blocks by EMA decode-attention score (H2O / Quest / SnapKV) + neighbor expansion, with an observe/refresh cadence that periodically reads everything. Low-attention middle blocks are *dropped* during steady decode — the same approximate family as Quest (ICML'24), H2O (NeurIPS'23), SnapKV, StreamingLLM, all prior art. The "`gather == full-read` / bit-identical / output-identical" claims are real but narrower than exact-vs-full-attention: they cover (a) the **gather primitive being lossless** — retain-all ≡ `range(seq_len)`, and gathering the retained set reads exactly those positions; and (b) **CUDA-kernel-vs-Python-reference byte-equivalence** (Phase 6E) — both implementation-correctness invariants — plus (c) **empirical** greedy-token / needle preservation on tested prompts (Phase-9 GREEN). None is a provable exact-vs-full-attention guarantee. **W1 is therefore not an open wedge.**
...
**Narrowest defensible claim (revised after code verification 2026-06-12):** the only surviving wedge is **W3 — quantization-protection-structured KV placement across NAND tiers, whose robust value is capacity** (≈2.22× tokens/GB vs bf16; see §9 capacity model), independent of selection exactness. **Do NOT claim exact / bit-identical attention-guided selection (W1):** the read-skip is approximate heavy-hitter retention (Quest/H2O/SnapKV/StreamingLLM family). **Do NOT claim KV-on-flash (W2):** anticipated by InstInfer/HiFC. Lead any filing with W3-capacity (protect-mask → tier placement), treating the read-skip and flash-offload as integration context, not novel claims.
