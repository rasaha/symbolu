...
## Decision 11 — CI design
One experiment-specific CI job, suggested name **`unseen-identifier-integrity`**. It runs **only**:
unit tests · deterministic fixture generation · tokenizer checks · parameter-count checks ·
source-hash checks · forbidden-component scans · seed-registry checks · protocol/config consistency
checks · manifest-schema tests · CLI authorization-refusal tests. CI **must not**: train the model ·
generate smoke/development/final cohorts · consume reserved seeds · produce scientific evidence ·
issue a scientific verdict. (Fixture-only seeds `993000–993004`; wall-clock small.)

## Decision 12 — Future lifecycle after implementation authorization
Frozen remaining lifecycle (this session authorizes **none** of steps 3–18):
1. implementation-authorization draft opened → 2. independent audit of the authorization draft →
3. authorization PR merged → 4. implementation branch created → 5. code implemented →
6. implementation tests + CI pass → 7. implementation PR opened → 8. independent
implementation-integrity audit → 9. implementation PR merged → 10. separate smoke/development
execution authorization → 11. smoke seed 9070 executed → 12. shortcut precheck executed →
13. development seeds 9071–9073 executed → 14. implementation-integrity + development-evidence audit
→ 15. separate reserved-final execution authorization → 16. final seeds 90760–90764 executed →
17. mechanical verdict reconstruction → 18. independent result audit.

## Implementation-authorization boundaries
A later **merged** authorization may permit **only** code implementation and **fixture-only** tests.
It must **not** permit: generating the smoke cohort · generating development cohorts · generating
reserved final cohorts · training · evaluating scientific metrics · consuming seed 9070 · consuming
seeds 9071–9073 · consuming seeds 90760–90764 · changing the locked protocol · implementing an
intervention. Fixtures use the separately-reserved testing namespace `993000–993004`.

## Claim boundary (unchanged from the lock)
No probe outcome supports typed structure over prose · enterprise reasoning · tenant-aware
competence · evidence grounding generally · multi-hop · temporal · BindingSlots · KDA · production
readiness. Preserved: `ORIGINAL_BINDINGSLOTS_NEURAL_ROUTING_UNRESOLVED` ·
`E1_TEMPORAL_TRANSFER_PARTIAL` · `KDA_VALIDATION_BLOCKED`.

## Draft status
When this design is complete and reviewed, the package emits only:
**`UNSEEN_IDENTIFIER_COPY_SELECTION_IMPLEMENTATION_AUTHORIZATION_DRAFT_READY`** — the implementation
scope and controls are fully specified for independent review; **no implementation or execution is
authorized by this draft**. `IMPLEMENTATION_AUTHORIZED`, `EXECUTION_AUTHORIZED`, and any capability
verdict are **not** emitted.