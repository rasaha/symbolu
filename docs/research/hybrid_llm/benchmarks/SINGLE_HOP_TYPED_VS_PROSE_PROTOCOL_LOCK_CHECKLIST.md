...
| # | Item | Status |
|---|---|---|
| 1 | Exact B0 grammar frozen | ✅ PASS — fixed order, templates, punctuation, missing/conflict/cross-tenant forms, no serializer search |
| 2 | Exact B1 schema frozen | ✅ PASS — required five top-level fields, fixed object shapes/order, no answer/evaluator fields |
| 3 | Paired examples provided | ✅ PASS — S1–S8 and A1–A6 B0↔B1 examples over identical fact graphs |
...
| 6 | Same parameter count confirmed by design | ✅ PASS — same baseline class/config/vocabulary head for B0 and B1; byte-identical paired initialization required |
| 7 | No arm-specific model component | ✅ PASS — no typed encoder/head; representation is the only arm variable |
| 8 | Model architecture and training recipe frozen | ✅ PASS — 128D, 2 layers, 4 heads, 512 FFN, 768 max sequence, AdamW, 1200 steps, greedy decode |
...
| 14 | Token-budget policy frozen | ✅ PASS — 512 shared tokenizer-token input cap, complete fact sets, no truncation/padding |
| 15 | Primary and sensitivity cohorts defined | ✅ PASS — complete information-equivalent cohort primary; ≤10% token-difference subset non-overriding |
...
| 18 | Episode counts and distractor density frozen | ✅ PASS — six domains, per-split cohort counts, six-entity S1–S7 density, bounded S8 density |
...
| 21 | Fact-set hash specified | ✅ PASS — `B0_fact_hash == B1_fact_hash` required for 100% of pairs |
