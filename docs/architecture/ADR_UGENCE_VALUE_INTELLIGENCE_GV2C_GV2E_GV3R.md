...
>
> The enum is `EvidenceUsageScope` (members `GENERAL`, `EVALUATION_ONLY`); there is no type named `UsageScope`. Implementation is gated on the pre-implementation impact record at [`UVI_S26_RULINGS_IMPLEMENTATION_IMPACT.md`](UVI_S26_RULINGS_IMPLEMENTATION_IMPACT.md).
