...
(`ACC-PR-BASE`, `ACC-PR-1` – `ACC-PR-5`), over the ballot merged by PR #1540
and pinned at commit `3c00046f` (document SHA-256 `d971acfb…68f3`, 297 lines).
This ruling rides the same branch as that ADR and takes effect when they merge
together; it decides nothing the ratification did not already scope.

`[V]` An earlier pair of these documents, on the closed and never-merged PR
#1537, pinned a different transcription of the same ballot; that pin is
withdrawn by the ratification this ruling rides with, and the two rulings below
are unchanged in substance by the substitution — the ballot rows they answer are
byte-identical across the two documents.
