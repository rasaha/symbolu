...
> # ⚠️ SUPERSEDED BY SEMANTIC PARITY AUDIT
>
> **Status: SUPERSEDED — DO NOT IMPLEMENT.**
>
> This plan's central premise — that the canonical shipped kernels
> `ugence-decision-authority` and `ugence-actiongate-provider` can be
> **substituted** for the Risk Authority reference components behind
> `DecisionAuthorityPort` / `ActionGatePort` (see §1, §8, §9, §15
> *"REFERENCE PORTS → PRODUCTION KERNEL ADAPTERS"*) — was **disproven** by the
> mandatory Phase 1 audit this plan itself required (§7).
>
> The audit — [`RISK_AUTHORITY_RA45_PHASE1_PARITY_AUDIT.md`](./RISK_AUTHORITY_RA45_PHASE1_PARITY_AUDIT.md)
> — found the kernels enforce **strictly less** than the reference components and
> **cannot represent** RA authority-critical semantics (authority `Scope`,
> `Scope_issued ⊆ Scope_delegated` monotonicity, amount ceilings, decision
> expiry, revocation, authority epoch, signed-envelope verification, exact-action
> binding). Verdict: **RA45_BLOCKED** for the substitution model.
>
> **The corrected, authoritative design is:**
> [`RISK_AUTHORITY_RA45_GOVERNANCE_COMPOSITION_PLAN.md`](./RISK_AUTHORITY_RA45_GOVERNANCE_COMPOSITION_PLAN.md)
> (additive, fail-closed governance composition — Risk Authority retains
> machine-authority ownership; the kernels contribute vetoes/restrictions only).
...
>
> This document is retained **only** as the historical record of the rejected
> approach and of the audit gate (§7) that caught it before code was written.
> Everything below the line is preserved verbatim and must **not** be treated as
> current guidance.

---

**Status (original, now historical): DESIGN / NOT IMPLEMENTED**
