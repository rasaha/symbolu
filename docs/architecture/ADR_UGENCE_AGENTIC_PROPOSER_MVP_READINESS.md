...
**OD-6 — ratified, not yet implemented.** `[G]` Unlike OD-1 – OD-5, no guard in
`packages/capabilities/agentic-proposer/tests/` yet enforces any of OD-6's three parts:
this ratification is a documentation-only change to the specification and this ADR,
following A11's own precedent that ratification and implementation are independent
events. Implementing it against the S1 contract module `6ef305fbe3ee0ff9960a7b52a1810a26f1e11953`
already introduced requires: (i) adding the C9 validator to
`src/ugence_agentic_proposer/contracts.py`'s `AdvisoryCandidateSet`, on the same pattern
as `_completion_is_unconstructible`; (ii) defining `CrossContractViolationError` and
replacing the bare `ValueError` raised at the eight `identity.py`/`verification.py` call
sites implementing R-1b's cross-contract clauses and R-5, R-6, R-7, R-9 and R-10; and
(iii) replacing `tests/test_process_ordering_obligation.py`'s own text — which currently
asserts that `ProposerProcessState`'s cardinality and R-4's comparison basis are open
questions this specification must still settle — with coverage of the now-ratified
answer, since that text would otherwise contradict OD-6(iii) as soon as this ADR merges.
Until that follow-on change lands, `public_api.json` and `version.py` remain exactly as
commit `6ef305fbe3ee0ff9960a7b52a1810a26f1e11953` left them, and the pre-OD-6 behaviour
(refusal at the builder; a bare `ValueError`; an untested enum ratification) remains
...
was never an authorization — the review and the merge are. **OD-6 carries only the first
status today**: it is ratified, but no named guard implements it yet, on the terms
recorded under *OD-6 — ratified, not yet implemented* above.
