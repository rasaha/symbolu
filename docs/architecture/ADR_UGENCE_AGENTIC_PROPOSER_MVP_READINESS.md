...
active until it is. **OD-1 through OD-10 are decided; no owner decision is
...
algorithm.** `[G]` **No production or behavioural guard exercises the unimplemented
OD-7 selection surface** in `packages/capabilities/agentic-proposer/tests/`, `src/` is
unmodified, and neither C7's nor C9's validator has moved. `[V]` The
**documentation-consistency guards** in `tests/test_documentation_consistency.py` do
pin the OD-8/OD-9/OD-10 meanings and the OD-7 statements those rulings amended — part
5's replay rule and part 7's fail-closed table — and nothing else in OD-7; they are
**not production enforcement** and prove nothing about the future implementation. Unlike OD-1 –
...
**OD-8, OD-9 and OD-10 — RATIFIED 2026-08-28, not implemented.** `[R]` **OD-8**:
...
through OD-10 carry only the first of the three so far.** The
...
ratified and implemented* above; **for OD-7 through OD-10, no production or behavioural
guard exercises the unimplemented OD-7 selection surface** — `[V]` the
...
replay rule and part 7's fail-closed table — and nothing else in OD-7, but those are
**not production enforcement** and prove nothing about the future implementation, so
the second status stays open — on the terms recorded
under *OD-7 — ratified, not implemented; a boundary, not a complete executable
algorithm* and *OD-8, OD-9 and OD-10 — RATIFIED 2026-08-28, not implemented* above
(their prospective behavioural guards are `I8.12`–`I8.15` in the specification); and **S1 production implementation
