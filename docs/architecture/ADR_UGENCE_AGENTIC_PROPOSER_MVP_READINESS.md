...
*(OD-7 is ratified but not yet implemented, and is recorded as an additional row in
the table below, on its own explicitly different terms; the heading above states the
status of OD-1 – OD-6 only.)*
...
and OD-7 are the two decisions of the seven that bear on contract shape; OD-4 is
resolved below and the specification implements the resolution, while OD-7 is resolved
below but **not yet implemented** — it amends the frozen S1 surface rather than
...
criterion or the `INCONCLUSIVE` mapping, and naming those two separately is what keeps
"OD-7 is decided" from being misread as "nothing about S2 selection remains open."
...
| **OD-7** — **RATIFIED 2026-08-27; NOT YET IMPLEMENTED** | **Scopes the S2 domain-evaluation and candidate-selection boundary that removes C7 and C9, in eight parts — a boundary, not a complete executable algorithm.** (1) Domain evaluation and candidate selection are separate responsibilities in one ordered boundary; selection must never determine, influence or retroactively complete domain evaluation. (2) The domain evaluator is a narrow injected `DomainEvaluationProvider` protocol this package owns but does not implement, echoing back both the profile identity and the `candidate_id` it evaluated; no concrete evaluator is imported or embedded, and no network, storage, service-discovery or plugin-loading mechanism is authorized. (3) `DomainCheckCompletion`'s substantive reading — every check reached a *per-check* determinate result, regardless of whether those results converge — is stated here for the first time, not carried over from C7's own wording, which only closes the enum; that reading is what makes `INCONCLUSIVE` reachable even though it is itself a determinate aggregate value. The aggregate result is a new, separate closed vocabulary, `DomainEvaluationOutcome` (`SATISFIED`, `NOT_SATISFIED`, `INCONCLUSIVE` — deliberately not `INDETERMINATE`, which D4 reserves elsewhere), on a new `CandidateAdvisory.domain_evaluation_outcome` field coupled to `domain_check_completion`. (4) Selection, for the S2 MVP, is a deterministic, versioned, in-package function considering only eligible, `SATISFIED` candidates; `[G]` **OD-8** — the selector's substantive ranking criterion — is not ratified by OD-7 and is outstanding. The `candidate_id` ascending tie-break is a total order over already-unique keys and is therefore always decisive on its own; it cannot itself leave a tie unresolved. (5) `P_unsigned` must bind the domain-evaluation profile identity, each candidate's result, and the selector-policy identity — recording any of these on `ProposerProcessRecord` alone is rejected because that record sits outside `P_unsigned`. Four **C5b** fields (`Token`-typed) are added to `AdvisoryCandidateSet` (mirrored onto `ProposerAdvisory`) and one to `CandidateAdvisory`. `verify_domain_evaluation` takes an independently supplied `expected_profile_id`/`version` so its profile check is not circular, and also checks the echoed `candidate_id`; `verify_deterministic_selection` additionally checks the stored selector-policy identity against this package's own ratified selector constants. A malformed input returns `False` (never raises); a provider exception during replay returns `False`; a provider exception during the original build raises `DomainEvaluationProviderError`; missing evidence warns and routes to `NEED_EVIDENCE` without calling the provider. (6) Execution order is fixed: eligibility, then domain evaluation, then verification, then selection, then readiness — `CandidateAdvisory` stays frozen throughout, and is constructed exactly once, after evaluation, on the same one-expression G2 discipline `ProposerAdvisory` already follows; Equation 2 **gains a seventh term**, `DomainEvaluationSatisfied` (`candidate.domain_evaluation_outcome is DomainEvaluationOutcome.SATISFIED`), amending Part F. An earlier draft held that no term was needed because Equation 2 runs only after selection on the already-`SATISFIED` selected candidate; `[V]` that is withdrawn, because `evaluate_readiness` is an exported public symbol (`equations.py:32`, `__init__.py:106`, `public_api.json`) with no caller in `src/`, so the call order is unenforceable against a consumer, and a `COMPLETE` + `NOT_SATISFIED` candidate would otherwise satisfy R-2's `PROPOSAL` condition. `[V]` Today's V13 (`contracts.py:747-763`) is a blanket refusal of `PROPOSAL` that never calls `evaluate_readiness` — it can be blanket only because C7 makes `COMPLETE` unconstructible — so the exposure opens not on C7's removal alone but when V13 is reimplemented to enforce R-2's recomputation, which part 8 requires to land in the same change set. (7) A fail-closed table covers missing evidence, an `INCONCLUSIVE` outcome, no eligible candidate, and an unverifiable provider or policy; an earlier draft's "unresolved tie" and "evaluators disagree" rows are withdrawn as structurally unreachable and unratified (multi-provider evaluation), respectively. `[G]` **OD-9** — the `INCONCLUSIVE`-to-terminal-outcome mapping — is not yet ratified and is outstanding. (8) C7 and C9 remain active until every OD-7 contract field, vocabulary member, protocol, replay function and test obligation is ratified **and** implemented in the same change set; neither validator may be removed in isolation. | **yes — resolved, not yet implemented** | `[G]` **not implemented.** No guard exists in `packages/capabilities/agentic-proposer/tests/`; C7 and C9 are unmodified; `public_api.json` and `version.py` are untouched by this ratification. Full ruling, field-ownership table, C5 classification, new vocabulary, exception, replay-function signatures, rejected alternatives and prospective `I8.1`–`I8.11` enforcement obligations are recorded in `packages/capabilities/agentic-proposer/docs/S1_CONTRACT_AND_EQUATION_SPECIFICATION.md`'s `OD-7` entry. **OD-8** and **OD-9**, named within that entry, are separate outstanding decisions, not implementation detail. |
...
the specification is written for OD-4's resolution, and would need to be amended, once
independently reviewed and implemented, for OD-7's. The *decisions* are
ratified, and the guards named above enforce all but OD-7. Neither ratification nor a
guard authorizes production code; OD-7's own transition controls additionally forbid
implementing it outside a single change set that also removes C7 and C9 together.
...
**OD-7 — ratified, not implemented; a boundary, not a complete executable
algorithm.** `[G]` No guard exists in `packages/capabilities/agentic-proposer/tests/`,
`src/` is unmodified, and neither C7's nor C9's validator has moved. Unlike OD-1 –
OD-6, OD-7 is deliberately not followed here by an implementation note, because its
own transition controls (part 8 of the ruling) forbid building any part of it outside
a single change set that also removes C7 and C9 together, and that change set does not
exist yet. What exists is the ratification itself: eight numbered parts, a
...
actually produce; six rejected alternatives; and eleven prospective enforcement
obligations (`I8.1` – `I8.11`) — all recorded in
...
**Two decisions named inside OD-7 — OD-8 and OD-9 — are themselves outstanding, not
implementation detail.** `[G]` **OD-8**: the selector's substantive ranking criterion.
`[G]` **OD-9**: the exact mapping from an `INCONCLUSIVE` domain-evaluation outcome to
`ABSTAIN` or `ESCALATE`. Both are tracked by name in the specification's `OD-7` entry
precisely so neither is mistaken for resolved once OD-7 itself is implemented; OD-7
ratifying its boundary does not ratify either. Neither this entry nor the
specification's own `OD-7` section authorizes any change to `src/`, `public_api.json`
or `version.py`; per the owner's ruling, that follows only once the amendment is built
and has passed an independent consistency review.
...
2026-08-26, and OD-6 and OD-7 are resolved, ratified 2026-08-27**; all seven are
recorded once, under *Owner decisions OD-1 – OD-6* above (OD-7 as an additional row,
on its own explicitly different terms).
**Of the seven, OD-4 and OD-7 bear on contract shape; OD-5 and OD-6 were ruled not
to.** **OD-4 is resolved (a)**: `ProposerAdvisory` carries its `CandidateAdvisory`
...
identity-bound fields across three contracts — **but, unlike every other decision here,
resolved does not mean built**: OD-7's own transition controls forbid implementing any
part of it outside the single change set that removes C7 and C9 together, and that
change set does not exist.

**Each of OD-1 – OD-6 carries three statuses, and they must not be collapsed; OD-7
carries only the first of the three so far.** The
...
ratified and implemented* above; **for OD-7, no guard exists**, on the terms recorded
under *OD-7 — ratified, not implemented; a boundary, not a complete executable
algorithm* above; and **S1 production implementation
...
S1's own status**: OD-7 gates S2 work behind C7 and C9, not the already-frozen S1
surface, which A12 covers and which OD-7 does not touch until it is itself built and
reviewed.
