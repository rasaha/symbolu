...
role document, no test, no proof, and it settles nothing about how the pilot is
...
**Baseline:** default head `ac70994d9d4478628a17e96d0a656418302fbbdf` — the
ballot's own stated baseline, the merge of PR #1534, which shipped the `ACC-IA`
issuance & activation distribution at `0.1.0`.
...
**as that file stands at commit `f4b0c94c693375ee512bf71cf0b144bdabfc46e0`**:

| Identity value | Ratified value |
|---|---|
| File path | `docs/architecture/AGENT_CONSTITUTION_INVOICE_RECONCILER_PILOT_SCOPING_BALLOT.md` |
| Commit | `f4b0c94c693375ee512bf71cf0b144bdabfc46e0` |
| Document SHA-256 | `bbb8d6c3685549ad857fe99dff0d3214ccd7158ac9069cffd7ad3e331854c0f6` |
| Line count | 79 |
| Ballot-block SHA-256 (the line beginning `Agent Constitution — invoice-reconciler pilot scoping ballot` through the line ending `ruling that follows it.`, inclusive; 59 lines) | `b3ca6a282a85c8116b175915b2693eda9aa8f667d907ca4ddf7e1ecdc3adea90` |
...
`[R]` The ballot document is itself a transcription of record: the owner put
the ballot in conversation against this baseline and directed its verbatim
transcription into the repository as an owner act, so that this pin could be
taken on the standing precedent of pinned ballot documents. The conversational
act is the authority; the transcription at `f4b0c94c` is its record, and this
declaration continues to govern the text at that commit should the file gain
further commits.
...
`IR-1` – `IR-5` labels remain the ballot's, and the `ACC-PR-*` labels are the
citable ones.
...
* **Until the implementation-authority ruling and its change set land, all of
  `ACC-FC-3`'s gap remains open** — this ADR closes nothing by itself.
...
covers `.py` files only. A committed JSON role declaration under `pilot/` is
not reached by either scan, so no automated guard yet measures such a document.
...
Two new documentation files across two commits: the pinned ballot transcription
at `f4b0c94c` and this ADR. **No production source, test, specification,
CHANGELOG, `public_api.json`, `version.py`, package metadata, CI workflow or
platform-freeze artifact is modified.** The activation distribution remains at
`0.1.0`; the pinned ballot document is unmodified by this ADR: its commit,
...
pilot change set that `ACC-PR-1` – `ACC-PR-4` scope. Until it lands, no pilot
file may be committed under this record.
