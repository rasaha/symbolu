...
| `custody_lease_id`, `custody_authority_id` on `egress_result`; `CHECK egress_result_genuine_call_requires_custody` replacing `CHECK egress_result_no_genuine_call` | migration 2 | LP-2b under §0.3.1: the database admits `genuine_call = true` only with both custody identifiers and `RESPONSE` provenance. `EgressResult` refuses it until both predecessor gates of the commissioning ADR §0.5 hold (`genuine_call_admitted()`: status `PENDING_VALIDATION`/`MET` and the owner's live-validation authorization); MET is the outcome, never the prerequisite `[V]` |
...
| `step8_required_values` in `MEU_LIVE_PROVIDER_DESIGNATION.json` | beside the package | the seventeen obligations, each `UNDESIGNATED`, with the attestation `independently_checked_by: null` listed separately, the live-validation authorization recorded as not given and the production record as none |

## Addendum, 0.4.1 (2026-09-12): the genuine-call gate is two predecessor gates, never MET

| Changed | Where | What it closes |
| --- | --- | --- |
| `LIVE_SYNTHETIC_VALIDATION_AUTHORIZATION` (`NOT_GIVEN`), `GENUINE_CALL_ADMITTING_STATUSES`, `genuine_call_admitted()` | `version.py`, consulted by `records.py` and the adapter | the circularity the owner named (commissioning ADR §0.5): row 12's genuine result required `MET`, which row 12's evidence helps establish. Now G1 (`PENDING_VALIDATION` or `MET`) ∧ G2 (the owner's separate live-validation authorization), both release constants; each alone admits nothing `[V]` |
