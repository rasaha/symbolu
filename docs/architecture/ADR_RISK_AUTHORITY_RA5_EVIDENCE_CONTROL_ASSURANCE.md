...
- **Status:** Proposed (design/discovery). Gated by preconditions — see the
  companion plan `RISK_AUTHORITY_RA5_TAP_CONTROL_ASSURANCE_PLAN.md`.
- **Date:** 2026-08-10
- **Baseline:** default head `143f9f3f` (merge of PR #1402). RA-1→RA-4 (#1396) and
  RA-4.5 (#1402) merged and stable.
- **Verdict:** `RA5_READY_WITH_PRECONDITIONS`.
...
- Implementation is **gated** until the preconditions are met: ratify an in-repo RA-5
  spec, settle the TAP admission-owner/naming, fix the Control-Assurance boundary,
  add trust-binding fields, and preserve reference-mode conformance.
