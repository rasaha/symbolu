...
§3 below changes the premise of that note's §7.
...
Three passes, all reproducible from the saved inventory:
...
| **Retrieval (RAG)** | infrastructure across 37 Python files and `.github/workflows/core-rag-ci.yml` | `[V]` |
...
space — including §7 of the workflow-fit note — refers to the enum only. The
...
|---|---|---|---|
| W01 | Retrieval-Augmented Reasoning | **exists as infrastructure**, not a workflow | 37 files; `core-rag-ci.yml` `[V]` |
...
| W05 | Self-Consistency | **absent** — the 7 grep hits are ontology / digest / coherence consistency, not the reasoning method | `[V]` |
...
| W11 | Scenario Planning | **absent** as workflow; `forecast_horizon` and `AssessmentStage.FORECAST` exist, forecast engine deferred | `evidence.py:369` `[V]` |
...
(`ADR_UGENCE_S2B_ROUND2_VOCABULARY_RATIFICATION.md:64`). A verifier in a
separate trust domain is what the ADR's "producers never self-verify" rule
permits. The two are not variants of one workflow; they sit on opposite sides of
a ratified line.
...
| SP05 | Tool requirement | `TOOL_READINESS`, `INTEGRATION_READINESS` (`enums.py:125-135`); `mcp_gateway.py:12`; `trust/hallucinated_capability.py:5` | exists `[V]` |
...
| `ToolsAvailable` | `trust/hallucinated_capability.py` — "pure set-membership + alias resolution", PROVISIONAL, advisory-only (`:5`); `mcp_gateway.py` | **partial** `[V]` |
...
- **"Least-resource" presumes a scalar ordering over resources.** The
  ...
  function, no invented constants" (`WORKFLOW_FIT_READINESS_SCOPING_NOTE.md:70`).
...
(`thresholds.py:41`) `[V]`; policies are the Policy Authority `[V]`; `Q(w,t)`
...
| Ambiguous: models disagree, cost ceiling close, change freeze, several replica counts | Tree of Thought | replica concept in 83 files `[V]`; **change freeze: 0 hits; cost ceiling: 0 hits** — the scenario's own conditions are unmodelled in the cloud-scaling packages `[G]` |
