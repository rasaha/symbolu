...
| **DD-12** | Which vetted, maintained third-party Ed25519 implementation BR-2C's capability-bearing release adopts | **OPEN — added 2026-08-23 by D-40 (§35.2).** §23 does not bar a third-party cryptographic dependency (D-40); BR-2's own no-cryptography gates stay binding until a ratified release transition replaces them (D-33, D-40), and no repository evidence ratifies one package over another for BR-2. **Options.** (i) Adopt the identical `cryptography`+`PyNaCl` pair `ugence-trusted-evidence-authority` already uses (`packages/trusted-evidence-authority/pyproject.toml:80-83`) — a maintained, vetted, dual-backend pattern with one closure audit's worth of precedent (TEV-2). (ii) Select independently for BR-2 — a single-profile need (`ED25519_SHA512_V1`, D-29(1)) may not need a second library the way TEV-2's stricter point-validation posture did. (iii) Depend on `ugence_trusted_evidence_authority` directly for the primitive — **closed**, not merely deferred: it is BR-2's first forbidden import already (`packages/benchmark-registry-authority/tests/packaging/test_dependency_boundary.py:46`), and §23 restricts BR-2 to `governance-contracts`. **Consequences.** (i) trades independent selection for consistency and a working, once-audited precedent, at the cost of carrying PyNaCl's stricter validation surface even if BR-2C's narrower profile does not need it. (ii) fits the narrower need more precisely but starts BR-2C's own audit trail from zero rather than reusing TEV-2's. Selection is an owner decision for the ratified release transition that replaces BR-2's crypto gates (D-40), not for this entry, and remains outstanding regardless of D-38's or D-32(4)'s status. |
