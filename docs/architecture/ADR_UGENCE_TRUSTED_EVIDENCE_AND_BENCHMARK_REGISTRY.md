...
Thirty-seven owner decisions, **D-01 through D-37**, all ratified — nineteen at
the original ratification (D-01–D-19) and eighteen added by later amendment.
...
D-37**, and the rows are not swept when code moves.
...
`README.md:157` — and three further citations went stale *during* the BR-2C-0
closure itself, two of them moved by that closure's own insertions. A governance
record cannot track line numbers it does not own. **File paths and symbol names
below are live and are corrected when wrong; line numbers are not.**

Three deliberate exceptions, each re-verified against the tree at the pass that
wrote it: **D-33**'s, **D-36**'s and **D-37**'s citations, because those rows turn on
exactly which assertion sites exist and a reader needs to reach them.
