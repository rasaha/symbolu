...
| **D-30** | Does BR-2B's distribution-text residue get corrected now, in its own BR-2B remediation commit, or deferred into BR-2C? | **RATIFIED 2026-08-20. Corrected now, in a BR-2B remediation commit scoped to the residue.** It is **not** deferred into BR-2C, and the commit ships no BR-2C capability, no port change, no new type, no refusal member and no version bump — it corrects statements about the distribution `0.2.0` already is. *Ground:* the deferral §35.8 records was a ruling about text that was still **true when written** — the four-phase prose described the ratification in force until the 2026-08-20 amendment, so correcting it once with BR-2B rather than twice cost nothing. This residue is not that: it is text that is **false about the shipped artifact at the version that shipped it**. `api.py:23` and `contracts/canonical.py:684` say the distribution mints and registers **fifteen** domains and classes, and the block comment at `contracts/canonical.py:254-255` says "Fifteen classes, fifteen domains", while `canonical_domain_inventory.json` at `package_version` `0.2.0` registers **18** `root_canonicalizable` classes and `CHANGELOG.md:47` records the move "15 -> 18"; three of those four statements are inside `src/` and therefore shipped in both the wheel and the sdist. Second and independent: `CHANGELOG.md:53` already asserts that BR-2B corrected "the distribution's own four-phase prose", so the residue makes a **closure claim of BR-2B's own** untrue, and a closure claim is repaired by the phase that made it, not by the next one. Third: the milestone labels at `api.py:1` and `api.py:11` and the `"milestone": "BR-2A"` field at `public_contract_inventory.json:5` describe a surface whose `api.__all__` already exports BR-2B's planning symbols — `BenchmarkTransitionPlan`, `BenchmarkTransitionRefusal`, `BenchmarkPlanningOutcome` and `BenchmarkConflictRecordPayload` (`public_api.json`, 92 symbols) — so an installer reading the shipped surface is told it is BR-2A's. **That D-24 and D-25 mint further domains at BR-2C is not a ground for deferral**: domain counts move at every subphase that mints a domain, so waiting for the next move is an argument that never expires, and it would trade a mechanical second edit against shipping a knowingly false statement for the whole of BR-2C — which D-23 classifies as blocked by **governance as well as engineering**, so its duration is not the correcting phase's to assume. *Surfaces moved:* the four stale count statements — `packages/benchmark-registry-authority/src/ugence_benchmark_registry_authority/api.py:23`, `contracts/canonical.py:254-255` and `contracts/canonical.py:684`; the three milestone labels — `api.py:1`, `api.py:11` and the `"milestone"` field at `packages/benchmark-registry-authority/public_contract_inventory.json:5`, whose value is emitted by `packages/benchmark-registry-authority/tools/generate_manifests.py:443`, so **the generator moves with the manifest** or the next regeneration reverts it. Three further stale count statements sit **outside `src/`** and are in scope for the same commit for the same reason: `packages/benchmark-registry-authority/README.md:157` and the generator's own strings at `tools/generate_manifests.py:190` and `:447`, the latter feeding the note at `public_contract_inventory.json:6`. Already correct and **not** to be touched: `contracts/canonical.py:73-74`, `:129` and `:362`, which read "BR-2A's fifteen and BR-2B's three". **This row moves no port, no type, no refusal member and no count assertion**: `api.__all__` and `public_api.json` stay at 93 and 92, the pinned refusal counts stay at 17 and 34, and `canonical_domain_inventory.json` and `pinned_canonical_vectors.json` are already correct at 18 and are not regenerated by it. It unlocks no capability token and no authority-issued name. |
...
**Residue that correction missed — D-30 rules it corrected now, not at BR-2C.**
`CHANGELOG.md:53` records that BR-2B carried out the correction ruled in the
paragraph above, but four statements still assert **fifteen** minted domains or
registered classes against the 18 that `canonical_domain_inventory.json`
registers at `package_version` `0.2.0`
(`packages/benchmark-registry-authority/src/ugence_benchmark_registry_authority/api.py:23`,
`contracts/canonical.py:254-255`, `contracts/canonical.py:684`), and three
milestone labels still read **BR-2A** at that version while `api.__all__`
already exports BR-2B's planning symbols (`api.py:1`, `api.py:11`,
`public_contract_inventory.json:5`). This residue is **not** part of the
seventy-seven four-phase statements above and is not governed by the
correct-once ruling: those were true when written, whereas these are false about
the artifact `0.2.0` shipped. **D-30** (§35.2) rules them corrected in a BR-2B
remediation commit scoped to the residue, and records the moved surfaces —
including the generator at `tools/generate_manifests.py:443`, without which the
manifest correction is reverted by the next regeneration.
