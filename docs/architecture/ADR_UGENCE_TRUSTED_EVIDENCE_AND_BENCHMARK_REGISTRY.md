...
| **DD-12** | Which vetted, maintained third-party Ed25519 implementation BR-2C's capability-bearing release adopts | **CLOSED 2026-08-24 by D-41 (§35.2).** Ruled **option (i) — the `cryptography` + `PyNaCl` pair, the same two maintained backends `ugence-trusted-evidence-authority` declares** (`packages/trusted-evidence-authority/pyproject.toml:80-83`), reused as a *selection* and never as an import. No longer deferred; the remaining BR-2C blockers are D-38(i), D-39's unclosed grammar and D-32(4), none of which this closure touches. **Recorded 2026-08-23 by D-40 as OPEN**, with the options below. §23 does not bar a third-party cryptographic dependency (D-40); BR-2's own no-cryptography gates stay binding until a ratified release transition replaces them (D-33, D-40), and D-41 authorizes no such transition. **Options.** (i) Adopt the identical `cryptography`+`PyNaCl` pair `ugence-trusted-evidence-authority` already uses (`packages/trusted-evidence-authority/pyproject.toml:80-83`) — a maintained, vetted, dual-backend pattern with one closure audit's worth of precedent (TEV-2). (ii) Select independently for BR-2 — a single-profile need (`ED25519_SHA512_V1`, D-29(1)) may not need a second library the way TEV-2's stricter point-validation posture did. (iii) Depend on `ugence_trusted_evidence_authority` directly for the primitive — **closed**, not merely deferred: it is BR-2's first forbidden import already (`packages/benchmark-registry-authority/tests/packaging/test_dependency_boundary.py:46`), and §23 restricts BR-2 to `governance-contracts`. **Consequences as recorded on 2026-08-23.** (i) trades independent selection for consistency and a working, once-audited precedent, at the cost of carrying PyNaCl's stricter validation surface even if BR-2C's narrower profile does not need it. (ii) fits the narrower need more precisely but starts BR-2C's own audit trail from zero rather than reusing TEV-2's. **Corrected by D-41:** the cost recorded against (i) is measured to be no cost — the two libraries differ on no malleable signature and on no input BR-2C's profile count reaches, and differ only on which *public keys* may be installed as a trust anchor, where `cryptography` alone reports a successful verification for a signature forged against an identity or small-order key. BR-2C's narrower profile removes none of that exposure, so (i)'s stated cost does not exist and (ii)'s stated cost does. Selection was an owner decision for the ratified release transition that replaces BR-2's crypto gates (D-40); D-41 makes the selection **without** performing that transition, and the transition remains outstanding, as do D-38(i) and D-32(4). |
...
Forty-three owner decisions, **D-01 through D-43**, all ratified — nineteen at
the original ratification (D-01–D-19) and twenty-four added by later amendment.
This section is governance; the supporting architecture brief that produced it
is evidence, not a repository artifact, and is not authority here.
...
**Version rungs are not subphases (D-33).** The five subphases above are
unamended. `tests/_milestones.py`'s ladder carries one further **version rung**,
`BR-2C-0`, meaning *BR-2C's contract surface landed; no BR-2C capability did*.
It carries **three** versions — `0.2.1`, `0.2.2` and `0.2.3`, ruled by D-36 —
because D-18 requires a version bump at each surface-count move while the rung
names what a version ships, not how many times it shipped. It mints no closure
audit and no separate ratification, and
...
awaits BR-2D. **Four further questions the BR-2C contract slice left
open are ruled by D-33 (the version ladder), D-34 (the anchor-resolution seam),
D-35 (which refusals a verified result may carry) and D-36 (how many versions
the `BR-2C-0` rung carries); none of the four touches
the engineering half of this blocker.**

### 35.2 Decision register — final dispositions

**In-row line citations are frozen at ratification.** A `file:line` inside a
ruling below records where the evidence stood **when that row was ratified**, and
is **not** re-verified as the tree moves. Read them as pointers to a historical
state, not as live references: resolve the *symbol* the row names, not the line
number it gives. This rule covers **every row in this section, D-01 through
D-43, without exception**, and the rows are not swept when code moves.

The rule is recorded rather than the citations chased because chasing them does
not hold. D-30's enumeration alone carries six that no longer resolve —
`contracts/canonical.py:254-255`, `canonical.py:684`,
`tools/generate_manifests.py:190` and `:447`, `adversarial_probes.py:338`, and
`README.md:157`. **The count of citations that went stale during the BR-2C-0
closure was recorded as three; the measured figure is larger, and the exception
that shielded the rows carrying them does not hold** [V]. Measured at this head:
**fifteen** in-row line citations no longer resolve — **eleven of D-33's**
(`test_milestone_boundary.py:111`, `:117`, `:423`; `test_trust_contracts.py:410`;
`verify_benchmark_registry_authority_distribution.py:157-158` and `:349-354`;
`gate_mutation_sweep.py:1111` and `:1232`; `pyproject.toml:15`; `__init__.py:15`
and `:27`) and **four of D-36's** (`test_milestone_boundary.py:104`, `:114`,
`:178`, `:690`, where the live `VERSION_SUBPHASE[api.__version__]` sites are
`135`, `145`, `209` and `721`). Seven of the fifteen sit in
`test_milestone_boundary.py`, which the closure's own final two commits rewrote;
D-36's four still resolved at `552a6ca1` and were displaced by `01bad73f`, so
they went stale inside the closure that wrote the exception claiming they were
re-verified. A governance record cannot track line numbers it does not own.
**File paths and symbol names below are live and are corrected when wrong; line
numbers are not.**

**The former exception for D-33, D-36 and D-37 is withdrawn.** It claimed those
citations were re-verified "because those rows turn on exactly which assertion
sites exist and a reader needs to reach them"; fifteen of them do not resolve, so
the exception described a discipline that was not kept. D-37 was listed in it
while carrying no line citations at all. Resolve the **symbol** every row names.
**D-38 is written to the frozen rule from the start: symbols and test names
only, no line numbers.**
...
| **D-30** | Is the BR-2A-era prose that survived the `0.2.0` surface move corrected in place, and does correcting it move anything? | **RATIFIED 2026-08-21.** **Yes, corrected in place; nothing else moves.** Ten statements that were true at BR-2A and false at BR-2B are corrected where they stand: **seven stale counts** fifteen → eighteen — `api.py:23`, the domain-block comment at `contracts/canonical.py:254-255`, the `canonical_bytes` docstring at `contracts/canonical.py:684`, the canonicalization heading at `README.md:157`, the `_root_canonicalizable_classes` docstring at `tools/generate_manifests.py:190`, the public-contract inventory note at `tools/generate_manifests.py:447`, and that note's committed copy in `public_contract_inventory.json` — and **three milestone labels** BR-2A → BR-2B: the `api.py` module docstring's title (`:1`) and layer sentence (`:11`), and the `milestone` field of `public_contract_inventory.json` together with its generator source at `tools/generate_manifests.py:443`. Eighteen is the number of root-canonicalizable classes the sealed registry holds at `package_version` `0.2.0` — BR-2A's fifteen and BR-2B's three — the phrasing already correct at `contracts/canonical.py:73-74`, `:129` and `:362`, which is not touched. **Why the residue is prose-only, and why that is a ruling rather than an observation.** A count that a gate asserts cannot go stale without turning a suite red; a count that only a docstring carries can, and every one of these ten did. The asserted eighteen was already correct and independently derived before this correction — `tests/contract/test_canonicalization.py:90`, `adversarial_probes.py:338` and `verify_benchmark_registry_authority_distribution.py:373` — so **no gate was wrong, only the prose describing it**, which is exactly why the drift survived the BR-2B suite. Two of the ten sites never reach a consumer at all: `MANIFEST.in:20-21` prunes `tests` and `tools` from both artifacts. The one machine-readable file that moves, `public_contract_inventory.json`, moves **only** its `milestone` field and one word of its generator-authored `note`; `tests/packaging/test_inventories.py` re-derives every *column* from the live surface, and those two strings are authored by `tools/generate_manifests.py`, so the source literal must move first and the file be **regenerated, never hand-edited** — a hand-edit would be reverted by the next regeneration and would make the committed file disagree with its own generator in the interval. **What this ruling does not do.** `package_version` stays `0.2.0`; `api.__all__` stays 93 and `public_api.json` 92 symbols; the BR-2 refusal vocabulary stays at 17 and the combined BR-1/BR-2 list at 34; no canonical domain, pinned vector or digest moves; **no capability token unlocks** under D-19's milestone-conditional tables; and no BR-2C implementation begins. **Recorded because it was cited before it existed:** the remediation commit `b648c5bc` on `claude/br2b-residue-remediation-nm11xp` carries the citation *(D-30)* against a register that ended at D-29. The correction is the shipped work; this row is the decision it names. **The enumeration above is not complete**, and D-31 rules the three residue items it omits. |
| **D-31** | What is ruled on the three residue items D-30's enumeration did not cover? | **RATIFIED 2026-08-21.** **All three are corrected now; none is deferred to BR-2C and none is left as is.** **(a) `README.md` — correct now, in three parts, and the wider README staleness is separated from them.** D-30 moved the heading at `README.md:157` to "Eighteen artifact classes, eighteen domains, eighteen pinned vectors" over a table that still holds **fifteen rows**, so the document now contradicts itself where before it was merely stale — a strictly worse state than either endpoint, and the reason this cannot be left. The three absent rows are `BenchmarkRegistrySnapshotAssertion`, `BenchmarkTransitionPlan` and `BenchmarkTransitionRefusal`; their domains are committed at `packages/benchmark-registry-authority/canonical_domain_inventory.json:17-19` (`…/registry-snapshot-assertion/v1`, `…/transition-plan/v1`, `…/transition-refusal/v1`) and their pinned digests in `pinned_canonical_vectors.json` (`1d60f269…`, `83b342be…`, `efa9a8e8…`), so **adding them is transcription from committed manifests and carries no judgment**; the rows are copied, never recomputed by hand. `README.md:1`'s `BR-2A` title moves to `BR-2B` on the same ground that moved `api.py:1` under D-30. Separately, and **not** as prose: the "Measured results at this revision" table at `:241-248` reports **1423 tests, 48 gates, 43 KILLED / 5 SURVIVED** against a tree whose committed ledger records **60 inventoried, 55 killed, 5 survived** (`gate_inventory.json`, regenerated at `dfd6aa5e`) and whose suite is 1731. Those are **measurements, not counts of a fixed set**, and restating them requires a run, not an edit. That table is therefore **marked at this revision as BR-2A-era** — a heading change, no number invented — and **re-stated from a fresh sweep and verifier run in a README pass at BR-2C**. **AMENDED 2026-08-21 — the marking obligation over `README.md:31` and `:46` is WITHDRAWN as erroneously imposed.** As first ratified this clause required those two lines to be marked BR-2A-era alongside the measured-results table, on the stated ground that they record `0.1.0 (BR-2A)` as the shipped state. They do not, and the requirement is therefore withdrawn outright rather than merely re-scoped: it obliged a correction that would have made the document less true. **Both lines are accurate and neither is marked.** `README.md:31` is the five-subphase version ladder listing `0.1.0` through `0.5.0` with their dependency deltas, a roadmap and not a claim about the current release; `:46`'s BR-2A row carries version `0.1.0`, ships "Registry and exact-resolution **contracts**" and has status **shipped**, which is true, while `:47` already carries `0.2.0` for **BR-2B** with status **this release**. Neither is a measurement and neither is stale, so marking either "BR-2A-era" would ship a **fresh falsehood in place of nothing** — precisely what ruling (b) of this same row forbids when it refuses "eighteen builders for eighteen classes". The marking obligation is therefore the measured-results table **alone**, discharged at `9a9e6cfc8`; after the marking note that table sits at `README.md:241-260`. Correcting the table and the title while leaving the measured-results table unmarked is prohibited: a partially corrected README implies the remainder is current, which is the defect D-30 created at `:157` repeated at larger scale. **(b) `tests/_builders.py:300` — correct now, and it is wrong in two independent ways.** The class count is now eighteen. The **builder count was never fifteen**: twenty builder functions cover the eighteen classes, because `rejected_admission_decision` (`:191`) is a second fixture for `BenchmarkAdmissionDecisionPayload` and `unoccupied_assertion` (`:320`) a second for `BenchmarkRegistrySnapshotAssertion`. The corrected comment therefore states **eighteen classes** and **drops the claim of a one-to-one builder count**, rather than substituting "eighteen builders for eighteen classes" and shipping a fresh falsehood in place of a stale one. Prose-only on the same ground as D-30: `MANIFEST.in:20` prunes `tests` from both artifacts and no gate reads the comment. **(c) `gate_mutation_sweep.py:1147` — correct now, and it is the more consequential of the three.** It is the exact analogue of `tools/generate_manifests.py:443`, whose literal D-30 already moved, with one difference that cuts the other way: its output **ships**. `MANIFEST.in:17` includes `gate_inventory.json` in the sdist, so this stale `BR-2A` label sits in a **distributed artifact**, while the generator literal D-30 moved sits only in a pruned harness. The ledger's payload is already BR-2B-current — 60 gates inventoried, 55 killed, 5 survived, 0 errored, regenerated at `dfd6aa5e` — so **only the label is stale**, and correcting it restates no measurement. Two constraints. The literal moves in `gate_mutation_sweep.py` and `gate_inventory.json` is **regenerated by an actual sweep run**, never hand-edited, on D-30's regeneration rule; if that run's numbers differ from the committed ledger, **the new numbers are recorded and reported, not suppressed to preserve the diff**. And the milestone is **not** derived from `tests/_milestones.py:22`'s `VERSION_SUBPHASE` map: that would couple a root-level harness to a pruned test module and is new engineering, unratified here and not in scope for a prose correction. **Two defects in the remediation's own record, corrected with it.** Its `CHANGELOG.md` entry says "**Four** stale counts corrected from fifteen to eighteen" and then lists **three** sites, the remaining four being in the separate bullet and the generated note; and the same entry's "Not corrected" paragraph is now superseded by this row and must say so rather than continue to report the three items as awaiting an owner. **What this ruling does not do:** identical to D-30's closing paragraph — no `package_version` bump, no movement of `api.__all__` (93), `public_api.json` (92), the refusal counts (17 and 34), any canonical domain, pinned vector or digest; **no capability token unlocks**; no BR-2C implementation begins. **Residual `BR-2A` literals are correct and are not touched:** `tests/packaging/test_milestone_boundary.py:499-503` and `tests/_milestones.py:22` record the subphase that *owns* a prefix or a version, which is BR-2A and stays BR-2A. |
| **D-32** | Is the distinct-reviewer requirement D-21 leaves open satisfied before BR-2C, or waived? | **RATIFIED 2026-08-21 — WAIVED FOR BR-2C ONLY, AND THE AUDIT CLAIM IS NARROWED WITH IT. AMENDED 2026-08-22 BY D-38: THE WAIVER IN (1) AND (2) IS WITHDRAWN FOR ANY CAPABILITY-BEARING BR-2C RELEASE.** *Amendment note (2026-08-22).* D-38 does not supersede this row; it withdraws part of it and leaves the rest standing. **(1)** and **(2)** waived D-21's distinct reviewer for BR-2C. That waiver now covers only the contract-surface rung `BR-2C-0`: for any BR-2C release that ships a cryptographic capability, `0.3.0` first among them, D-38 restores a distinct-reviewer requirement in the form stated there. **(3)**, **(5)** and **(6)** are unamended. **Two distinct preconditions now stand, and they are not the same gate:** **(4)** requires an **external cryptographic audit of the verifier**, a precondition to **production use** of it; **D-38** requires a **distinct independent authority to review and adversarially audit the exact release head**, a precondition to **shipping `0.3.0` at all**. Satisfying either leaves the other outstanding, and if the D-38 review is unavailable `0.3.0` remains blocked regardless of (4). *The original ruling, unchanged below.* D-21 rules that naming a distinct reviewer is an open owner action for any subphase that ships a capability, and §35.1 records BR-2C as the first that does. The requirement is **not satisfied**: `.github/CODEOWNERS:113-118` names `@rasaha` alone for this package's source and manifests, and this distribution's own `CHANGELOG.md` states that a single owner approving their own change is not an independent review. **It is waived rather than met, and this row says so plainly rather than recording a control that does not exist.** **(1) What is waived.** The distinct in-repo reviewer for BR-2C's development, review and merge. BR-2C may proceed with the owner as sole reviewer. **(2) Scope, and it is narrow.** **BR-2C only.** The waiver expires with the subphase. **BR-2D and BR-2E re-raise D-21's question on their own terms** and are not covered — deliberately, because BR-2D ships the first authoritative registry and the identity-locked composition root, where an unreviewed change has consequences BR-2C's does not. **(3) What is NOT waived.** The **engineering half** of the §35.1 blocker stands untouched: a secure verifier and composition-root trust-resolver design, reusing neither the Policy Authority nor the Risk Authority Ed25519 implementation, and the contract change D-24 and D-25 require to replace the `bool`-returning approval-verifier and publisher-trust-directory ports (`contracts/ports.py:177-185` and `:154-158`). A reviewer waiver decides nothing about either. **(4) The audit claim is narrowed, not dropped.** "Independently audited" in §35.1 and §35.9 means, for BR-2C, an **external cryptographic audit of the verifier**, and that audit is a **hard precondition to any production use** of it — not to BR-2C development or merge. Until it is obtained and recorded, **no BR-2C artifact, README, CHANGELOG or manifest may describe the verifier as audited, independently reviewed or production-ready**, and the composition root that would admit it stays a BR-2D concern. **(5) What this costs, recorded rather than left to be discovered.** BR-2C's cryptographic code will be merged on the strength of the author's own tests, probes and mutation sweep. Three closure audits across BR-2B found seven defects, none in the boundary and every one in a rule asserting it, which is the measured base rate for author-owned assurance in this package. This waiver accepts that rate for BR-2C on the ground that BR-2C ships no store, no clock and no authority-issued result, so an unreviewed defect there cannot make a transition occur; it does not accept it for BR-2D. **(6) Lapse.** The waiver lapses immediately if a second maintainer with write access to this repository exists before BR-2C completes, in which case D-21's requirement applies unchanged. **What this ruling does not do:** no `package_version` bump, no capability-token unlock, no amendment to any port, refusal member, test or `CODEOWNERS` entry under this decision, and no BR-2C implementation begins. |
| **D-33** | The BR-2C contract slice moved a public surface at an unchanged `package_version`. What does the version ladder do? | **RATIFIED 2026-08-21 — ONE CONTRACT-ONLY RUNG IS MINTED; `0.3.0` STAYS THE AUDITED VERIFIER.** `tests/_milestones.py:28`'s ladder gains a sixth rung, **`BR-2C-0`**, at index 2 between `BR-2B` and `BR-2C`, and `VERSION_SUBPHASE` (`:49-58`) maps **`"0.2.1"` → `"BR-2C-0"`**. `BR-2C-0` means *BR-2C's contract surface landed; no BR-2C capability did.* It is a **version rung, not a subphase**: it mints no closure audit and no separate ratification, and D-01's five separately auditable subphases are unamended [R]. *Measured costs* [V], all at `fd604dc4` against a 1970-passing baseline: minting the rung turns **two** tests red — the two version literals at `packages/benchmark-registry-authority/tests/packaging/test_milestone_boundary.py:111` and `packages/benchmark-registry-authority/tests/packaging/test_public_api.py:26` — 1968 passed, every capability ban intact. A pre-release string mapped to BR-2B measures identically (2 red, 1968 passed) and is closed on a different ground below. Bare `0.3.0` turns **five** red, and three of them are the assertions that the bans did not weaken: `packages/benchmark-registry-authority/tests/contract/test_confusable_and_ports.py:104`, `packages/benchmark-registry-authority/tests/packaging/test_milestone_boundary.py:117` and `:172`. Those three failing is not a re-pinning chore — it is the ladder's own enforcement reporting that the bump unlocked capability the slice does not ship — so bare `0.3.0` is closed. **`0.3.0` unlocks twelve tokens, not eight** [V]. The CHANGELOG's `package_version` section counts only the eight at `packages/benchmark-registry-authority/tests/packaging/test_milestone_boundary.py:44-51` (`signature_verifier`, `key_parser`, `trust_anchor_store`, `approval_verifier` and their unseparated spellings); four more unlock from the exported-symbol table at `packages/benchmark-registry-authority/tests/contract/test_confusable_and_ports.py:50-53` — `denyall`, `deny_all`, `verifier`, `trust_store` — which D-24 and D-25 already cite by name in their own *Surfaces moved* clauses. **That miscount is a shipped-text defect of the kind D-30 and D-31 correct in place**, and it is corrected when this row's version change lands [G]. **And the token bans are not the only mechanical enforcement** [V]: five gates are unconditional at every subphase and a version bump moves none of them — no cryptographic dependency may be declared (`packages/benchmark-registry-authority/tests/packaging/test_dependency_boundary.py:88`); no module may import a forbidden package, `ugence_trusted_evidence_authority` first among them (`:93-94`, list at `:45-63`); nothing in the package performs cryptography (`packages/benchmark-registry-authority/tests/packaging/test_milestone_boundary.py:423`); no concrete class satisfies any port (`packages/benchmark-registry-authority/tests/contract/test_confusable_and_ports.py:205`); and no cryptographic module is imported at all (`packages/benchmark-registry-authority/tests/contract/test_trust_contracts.py:410`). The rung therefore protects the twelve tokens, not the blocker: the blocker's load is carried by D-32(4)'s external cryptographic audit, which no test in this repository can assert. *Ground:* D-18 rules the pinned surface counts are milestone-scoped snapshots that move **deliberately at each version bump**, with every assertion site moving with them; at `fd604dc4` they moved 93 → 106 and 92 → 105 (`packages/benchmark-registry-authority/tests/packaging/test_public_api.py:40-41`) while the literal at `:26` did not, so the distribution now carries two different `0.2.0` surfaces [V]. §35.1 defines `0.3.0` as the **audited verifier**, which this slice does not ship and D-32(4) forbids describing as audited. Accepting the move at `0.2.0` leaves D-18's rule false with nothing recording it; holding the surface back discards governance D-24–D-28 already ratified and returns nothing for it; a pre-release string maps a `0.3.0`-family version to BR-2B while §35.1 maps `0.3.0` to BR-2C, so one `major.minor.patch` would resolve to two subphases by suffix and every later BR-2C pre-release would need its own `VERSION_SUBPHASE` entry. *Surfaces moved:* `packages/benchmark-registry-authority/src/ugence_benchmark_registry_authority/version.py` — `__version__` `0.2.0` → `0.2.1`, and the module docstring that states the ladder in prose; `tests/_milestones.py`'s `SUBPHASE_LADDER` (`:28`) and `VERSION_SUBPHASE` (`:49-58`), with the module docstring and its D-01 citation; the two version literals above, and the name of the test at `test_milestone_boundary.py:110-111`, which reads `..._is_the_br2b_version`; `public_api.json`'s `package_version` on regeneration by `tools/generate_manifests.py`, never hand-edited; the CHANGELOG's `[Unreleased]` heading, its `package_version` section, and the "eight capability tokens" miscount; `pyproject.toml:15`; and the `0.2.0` statements at `src/ugence_benchmark_registry_authority/api.py:11,38,46` and in `README.md`. **Five further sites, each found by running the checks or re-reading the shipped prose rather than by reading this row, and added to it here rather than left to be discovered again** [G]: the two version pins and their labels at `packages/benchmark-registry-authority/verify_benchmark_registry_authority_distribution.py:157-158`, which fail the offline distribution verifier on the wheel and sdist filenames until they move; **the independent `api.__all__` and manifest-symbol counts the same verifier re-derives at `:349-354`**, which are a second, deliberately separate pin from the suite's own at `tests/packaging/test_public_api.py:40-42` and fail whenever a rung carries a surface move — so this is a site every D-18 count movement must touch, not only a version bump; the `milestone` literal at `tools/generate_manifests.py:465`, which writes `public_contract_inventory.json`; the `milestone` literal at `gate_mutation_sweep.py:1232` with the banner at `:1111`, which writes `gate_inventory.json`; and the package docstring's own milestone-boundary list at `src/ugence_benchmark_registry_authority/__init__.py:15,27`, which marked BR-2B as "this release" and described the contract surface as shipping "ahead of the version rung". All three are generator-owned, so the manifests move by regeneration and are never hand-edited. `banned_capability_tokens` compares by ladder **index** (`tests/_milestones.py:61-88`), so inserting a rung shifts the BR-2C, BR-2D and BR-2E indices without unlocking anything — measured, not assumed [V]. **What this row does not do:** it unlocks no capability token, moves no port, no refusal member, no contract type and no manifest symbol, and begins no BR-2C implementation. D-01's five subphases, §35.1's five-row table and BR-2C's allocation to `0.3.0` are unamended. The rung is not a closure: BR-2C closes at `0.3.0`, under D-32's terms and its external cryptographic audit, not here. |
| **D-34** | `resolve_anchor` returns `Optional[BenchmarkTrustAnchorRecord]`, so `None` cannot separate `TRUST_ANCHOR_NOT_FOUND` from `TRUST_DIRECTORY_UNAVAILABLE`. What fills the seam? | **RATIFIED 2026-08-21 — A RESOLUTION TYPE: RECORD XOR TYPED REFUSAL.** `BenchmarkPublisherTrustDirectoryPort.resolve_anchor` returns a frozen `BenchmarkTrustAnchorResolution` that binds the exact `(role, identity, key_id)` triple it was asked and carries **exactly one** of an anchor record or a typed refusal reason — enforced in `__post_init__`, so neither a resolved-and-refused nor a neither state is constructible and no Boolean success flag exists. The seam still performs **no lifecycle evaluation** and takes no trusted instant: `TRUST_ANCHOR_REVOKED`, `_DISABLED`, `_NOT_YET_VALID` and `_EXPIRED` remain the verification seam's to decide on D-28's published order, and the only reasons this seam may carry are `TRUST_ANCHOR_NOT_FOUND` and `TRUST_DIRECTORY_UNAVAILABLE` [R]. **Were D-25 and D-28 ever read as barring a further type? No** [V]. D-25's *Surfaces moved* clause enumerates what **that row** moves, and D-28's "no further type is minted here" is scoped by *here* to D-28's own row; neither is a prohibition on a later row, and §35.9 Q-2C-2's options (a) and (b) decide *predicate versus resolver*, not the arity of the resolver's return. The collapse arrived by implementing both rows literally and is recorded as exactly that in this distribution's `CHANGELOG.md` under "Left open rather than decided", which states the `Optional` shape "follows the rulings literally" and that neither remedy "is available without a further owner decision". This row is that decision and amends nothing in D-25 or D-28. *Ground:* D-28 rules fail-closed on unavailable trust state with **never** a fallback to a cached, default or previously successful verification, and an unreachable directory that reads as *no such anchor* is a default answer substituted for an unavailable one [V]. D-27 requires the anchor-lifecycle distinctions preserved in verification and audit results and locates any deliberate collapse at a later **external read boundary**, not at the seam. D-24 replaced the Boolean because a shapeless answer cannot say why; `Optional[...]` is a one-bit answer wearing a type. The shape is already ratified elsewhere in this repository: `TrustAnchorResolution` at `packages/trusted-evidence-authority/src/ugence_trusted_evidence_authority/authority/trust.py:429-488` enforces this XOR at `:458-465` and refuses an anchor answered at a coordinate other than the one asked at `:466-472`. It is **copied and never imported**: `ugence_trusted_evidence_authority` is the first entry on BR-2's forbidden-import list (`packages/benchmark-registry-authority/tests/packaging/test_dependency_boundary.py:46`, enforced at `:93-94`), §23 restricts BR-2 to `governance-contracts`, D-04's fourth constraint forbids importing the trusted-evidence layer's trust-anchor directory, and **D-22(4) is the ratified precedent for copying a shape and not the code** [V]. *The two closed options.* A ratified exception in the `contracts/errors.py:30-86` family is closed: that family is a **contract-refusal** family raised by every `__post_init__` (`:33`) when *structure* is wrong, and `:37-41` rules that the absence of a refusal proves structure and nothing else — an unreachable directory is a runtime trust condition, not a malformed contract; D-24's whole ruling is that verification answers are **returned as evidence-bound values, never raised**; and an exception cannot be a field of a verified result, which is where D-27 requires the distinction to survive. Ratifying the collapse is closed because it would ratify at contract level the exact shapeless answer D-24 removed, for two conditions D-27 and D-28 ratified as separate members in the same slice. *Surfaces moved:* the return type at `packages/benchmark-registry-authority/src/ugence_benchmark_registry_authority/contracts/ports.py:180-185`, with the docstring at `:186-198` moving with it; one exported contract type, taking `api.__all__` 106 → **107** and `public_api.json` 105 → **106**, re-pinned at `packages/benchmark-registry-authority/tests/packaging/test_public_api.py:40-42` and re-derived independently by `packages/benchmark-registry-authority/verify_benchmark_registry_authority_distribution.py`; one manifest row of `kind: "contract"` carrying its field list (`tests/packaging/test_public_api.py:91-93`). The type is **deliberately not registered** as root-canonicalizable in `contracts/_seal.py`, so the encoder refuses to render it and `canonical_domain_inventory.json`, `pinned_canonical_vectors.json` and `public_contract_inventory.json` **all stay at 22** — both inventory gates are keyed on the sealed registry rather than on `__all__` (`packages/benchmark-registry-authority/tests/packaging/test_inventories.py:50-53` and `:109-110`) [V]. The ground for leaving it unsealed is §05's prohibition on byte space an artifact does not need, and D-25's ruling that the anchor record's canonical digest **is** the anchor revision: a second digest over the resolution would be the parallel identity D-25 refuses. **No refusal member and no refusal count move** — both reasons exist already under D-27 and D-28 — so the counts stay 24 and 41. **What this row does not do:** it mints no third state, so absence and unavailability remain the only two refusals this seam may carry and a third would need its own ratification; it does not make the seam evaluate lifecycle, read a clock or accept a trusted instant; it unlocks no capability token and none of the three reserved authority-issued names at `contracts/_authority.py:74-82`; it permits no implementation of the port to ship, which stays the composition root's under D-04 and BR-2D's under D-01; and it begins no verifier engineering, which D-32(3) leaves standing in full. |
| **D-35** | Which refusal reasons may a `REFUSED` verified result carry? | **RATIFIED 2026-08-21 — THE `TRUST_AND_AUTHENTICITY` FAULT CLASS PLUS `INDETERMINATE`: TWELVE OF THE TWENTY-FOUR.** A `REFUSED` `BenchmarkPublisherVerifiedResult`, `BenchmarkApprovalVerifiedResult` or `BenchmarkRevocationVerifiedResult` carries a reason classified `TRUST_AND_AUTHENTICITY` in `BENCHMARK_REGISTRY_REFUSAL_FAULT_CLASSES` — **eleven** members, measured against the live map: `NO_TRUST_ANCHOR_CONFIGURED`, `PUBLISHER_UNTRUSTED`, `SIGNATURE_INVALID`, `APPROVAL_UNVERIFIED`, D-27's five `TRUST_ANCHOR_*` and D-28's `TRUST_DIRECTORY_UNAVAILABLE` and `STALE_TRUST_SNAPSHOT` [V] — **or `INDETERMINATE`**, and nothing else. **`INDETERMINATE` joins, and this row rules it explicitly rather than leaving it to be read off the class map** [R]: it is its own fault class and not a trust one (`packages/benchmark-registry-authority/src/ugence_benchmark_registry_authority/contracts/enums.py:186-188`), so the class alone would exclude it, while §35.9 Q-2C-5's *Context* records that a resolver becoming unreachable in flight "currently refuses through `INDETERMINATE`, so it fails closed" — a live verification-time answer. A subset with no fail-closed catch-all would force an undeterminable condition into a specific trust claim or into no representable answer at all. Twelve admissible; twelve excluded. *Ground:* `TRUST_AND_AUTHENTICITY` is defined at `contracts/enums.py:176-179` as "the class BR-2C's verifier will attach", so this row narrows the constructor to what the vocabulary already says of itself. §35.1's BR-2C row forbids the subphase to ship "any storage, and any registry state whatsoever", yet the biconditional at `packages/benchmark-registry-authority/src/ugence_benchmark_registry_authority/contracts/trust.py:249-251` accepts any of the twenty-four, so a verified result can today refuse with `IDEMPOTENT_DUPLICATE`, `COORDINATE_SLOT_CONFLICT`, `DIGEST_ALREADY_BOUND`, `CONFUSABLE_COORDINATE`, `LIFECYCLE_CONFLICT`, `UNAUTHORIZED_TRANSITION`, `STALE_REGISTRY_SNAPSHOT`, `STORE_INTEGRITY_INVALID` or `STORE_UNAVAILABLE` — **nine registry-state facts a phase with no registry state cannot know** [G] — besides the three the question names. `UNSUPPORTED_SUPERSESSION` is D-10's out-of-scope refusal and BR-2 infers no supersession at any phase; `NOT_FOUND` and `NOT_ADMITTED` are `READ_NON_DISCLOSURE`, and D-27 deliberately places the non-disclosure collapse at a later external read boundary rather than in the verification result, so admitting the read vocabulary into that result inverts D-27 [V]. *The two closed options.* The full vocabulary is closed on the ground above, and because "unconstrained beyond membership of the BR-2 vocabulary" is what the CHANGELOG **records as left open**, not what any row ruled. Excluding only the three category errors is closed twice: it leaves the nine registry-state members admissible, and it is a **subtractive list** that must be re-edited by hand each time §35.6's append-only rule adds a member — where the fault class is derived from a classification map D-27 and D-28 already require to be **total**, and so maintains itself. *Surfaces moved:* the biconditional validator at `contracts/trust.py` narrows from membership of `BenchmarkRegistryRefusalReason` to membership of the admissible set, with its docstring moving with it — cited without line numbers deliberately, since this row moves the very lines a number would name; one exported specification constant, `BENCHMARK_VERIFICATION_REFUSAL_REASONS`, on the model of `BENCHMARK_TRUST_ANCHOR_EVALUATION_ORDER` (`src/ugence_benchmark_registry_authority/api.py`) and **derived from the fault-class map rather than written out**, so `api.__all__` and `public_api.json` each move by one — 107 → 108 and 106 → 107 when this row lands with D-34, 106 → 107 and 105 → 106 if it lands alone [I] — re-pinned at `packages/benchmark-registry-authority/tests/packaging/test_public_api.py:40-42`, re-derived by the offline distribution verifier, and carried as a `kind: "constant"` row whose value is re-checked against the live object at `tests/packaging/test_public_api.py:98-100`. Nothing here is root-canonicalizable, so the canonical manifests stay at 22. **No refusal member is added, removed, renamed, re-valued or re-ordered**: the counts stay 24 and 41 and §35.6's append-only guarantee is untouched. *Found while re-deriving the map, and corrected in place here* [G]: D-27 and D-28 cited the fault-class map as `contracts/reasons.py:226` and the composite docstring as `:213-218`; the slice that added their seven members moved both, to **`:294`** and **`:280-287`**. Stale in-row line citations of the kind D-30 corrects in place, so they are corrected in D-27 and D-28 rather than only noted here — a citation is not a ruling, and neither row's ruling is touched. **What this row does not do:** it rules nothing about what any **non**-verification result may carry — the subset binds the three verified-result types and nothing else; it adds no member for a condition the twelve cannot express, and any such member needs its own ratification under §35.6; it does not permit a verifier to *attach* one, since no verifier exists; and it unlocks no capability token and no authority-issued name. |
| **D-36** | D-33 maps only `"0.2.1"` to `BR-2C-0`, `tests/_milestones.py` maps `0.2.2` and `0.2.3` to it as well, and neither D-34's nor D-35's *Surfaces moved* clause authorises a version bump. Does the rung carry all three versions? | **RATIFIED 2026-08-21 — YES: `BR-2C-0` CARRIES `0.2.1`, `0.2.2` AND `0.2.3`, AND THE TWO LATER BUMPS ARE RATIFIED HERE RATHER THAN READ OUT OF D-34 AND D-35.** One rung, three versions. `VERSION_SUBPHASE` (`packages/benchmark-registry-authority/tests/_milestones.py`) maps all three to `BR-2C-0`, and this row rules the map that is already there rather than changing it: the gap was a **missing ratification, not a wrong mapping** [R]. *The measured position* [V]: `api.__version__` is `0.2.3`; the curated surface moved at each of the three, `api.__all__` 93 → 106 at `0.2.1` under D-33, 106 → 107 at `0.2.2` under D-34 and 107 → 108 at `0.2.3` under D-35, with `public_api.json` tracking one behind at 92 → 105 → 106 → 107 because `__version__` is carried as `package_version` and is not a member of `symbols`; the live count is **108**, pinned at `packages/benchmark-registry-authority/tests/packaging/test_public_api.py:40` and re-derived independently by `packages/benchmark-registry-authority/verify_benchmark_registry_authority_distribution.py`. **The unmapped alternative was never a silent one** [V]: every consumer reads the map by exact version string — `VERSION_SUBPHASE[api.__version__]` at `packages/benchmark-registry-authority/tests/packaging/test_milestone_boundary.py:104`, `:114`, `:178` and `:690`, and at `packages/benchmark-registry-authority/tests/contract/test_confusable_and_ports.py:98` — so an unmapped `0.2.2` or `0.2.3` raises `KeyError` and fails closed. A version can be *unruled*; it cannot be *quietly unbanned*. *Ground:* D-18 rules the pinned surface counts are milestone-scoped snapshots that move **deliberately at each version bump**, with every assertion site moving with them, and D-33 mints the rung precisely because a surface that moved at an unchanged `0.2.0` left the distribution carrying two different `0.2.0` surfaces. D-34 and D-35 each moved `api.__all__` and `public_api.json` by one, so each **required** a version of its own under D-18; that neither row's *Surfaces moved* clause names the bump is a gap in the enumeration, of the kind D-33 itself records and adds to rather than treats as a prohibition [G]. And the rung's meaning is true of all three: `BR-2C-0` says *BR-2C's contract surface landed; no BR-2C capability did*, and D-34 mints no third state, reads no clock and unlocks no token, while D-35 adds, removes, renames, re-values and re-orders no refusal member — the counts stay 24 and 41 — and unlocks no token either. Three versions that ship contract surface and no capability must ban the **same twelve** tokens, so they belong on the same rung. *The two closed options.* **A rung per version** is closed: a rung is a claim about capability, and `banned_capability_tokens` compares by ladder **index** (`tests/_milestones.py`), so a second and third rung banning exactly what `BR-2C-0` bans would be indices that rule nothing — the reverse of what D-33 minted the mechanism for. *Corrected in place:* this row first added "while D-01's five subphases would be outnumbered by rungs carrying no ruling", which is **false** — a rung per version leaves five subphases against three rungs — and it is **struck**. The substantive ground is unaffected and stands alone: the extra indices produce ban sets identical to `BR-2C-0`'s, so they rule nothing, and that is the whole of why the option is closed. **That ground is now pinned by a committed check rather than derived** [V]: `packages/benchmark-registry-authority/tests/packaging/test_milestone_boundary.py`'s `test_splitting_the_br2c_0_rung_per_version_changes_no_ban_set` builds the rejected ladder and compares ban sets at the three split rungs and at `BR-2C`, `BR-2D` and `BR-2E`, across **both** unlock maps — `FORBIDDEN_CAPABILITY_UNLOCK` and `EXPORTED_IMPLEMENTATION_UNLOCK`, since D-33 records that `0.3.0` unlocks twelve tokens across the two and not eight across one — while `test_no_token_unlocks_at_the_br2c_0_rung_itself` asserts the precondition the ground rests on: no token names `BR-2C-0` as its unlock phase. A future token that did would be shippable at one of the rung's three versions and not the others, which is exactly when this row's ground fails, and it now fails that check by name instead of silently falsifying this row [G closed]. **This row's *ruling* is pinned too, and was not at first** [G closed]: an independent audit found that re-mapping `0.2.2` to `BR-2B` left the suite green, because the fail-closed `KeyError` this row relies on catches an *unmapped* version and never a *mis*-mapped one. `test_the_rung_carries_exactly_the_three_versions_d36_ruled` now asserts the mapping itself, and `test_a_token_is_banned_below_its_unlock_rung_and_not_at_it` pins the `reached` boundary the whole ladder turns on — a boundary nothing exercised before, since every earlier check ran at rungs where no token unlocks [V]. Withdrawing the clause narrows this row's reasoning and changes its ruling not at all. **Holding `0.2.2` and `0.2.3` back onto a single `0.2.1`** is closed twice: it withdraws a version bump D-18 requires at each count move, and it returns the distribution to two — here three — different surfaces under one version, the exact defect D-33 exists to end. *Surfaces moved:* **nothing mechanical.** `VERSION_SUBPHASE` already carries the three entries; the `tests/_milestones.py` docstring's account of why `0.2.2` and `0.2.3` share the rung is confirmed as **ruling rather than inference** — its inferred "a second rung would be a second claim about capability, and D-34 makes none" is replaced by this row — and §35.1's version-rung paragraph carries the citation. **Discharged in the closure remainder**, so the enumeration is not left open the way D-33's was: this row's citation is added at `packages/benchmark-registry-authority/README.md`'s `BR-2C-0` milestone row, at `src/ugence_benchmark_registry_authority/version.py`'s ladder docstring, and at that `_milestones.py` docstring. Landing with them, on the same D-30/D-31 precedent: `verify_benchmark_registry_authority_distribution.py`'s headline moves BR-2B → BR-2C-0 to match `adversarial_probes.py`, and its claim to run "the six controls §20 specifies" is **withdrawn** — §20 is the *Readiness integration boundary* and specifies no negative control [V]; the script's own scheme numbers six isolation properties and runs eight, because control 6 splits into 6a/6b and step 8's dependency-really-required control is unnumbered, and the docstring now states that relation instead of citing a section that rules nothing here. D-33's *Surfaces moved* clause is **not amended**: it enumerates what that row moved at `0.2.1`. Landing beside this row, under D-30 and D-31's standing precedent for shipped-text defects and not as decisions of their own, are the closure pass's five prose corrections: `README.md`'s BR-2 refusal counts seventeen → twenty-four and 34 → 41 members, matching the counts D-27 and D-28 moved and their gates already pin; the measured-results table **re-run and re-stated, never edited** — as that pass measured it, suite **2108**, probes **83**, gate inventory **72**, **67 KILLED / 5 SURVIVED / 0 errored**, distinct properties **503 : 38 = 13.24 : 1**, negative controls **8 run, 8 caught**, pyflakes clean; later passes on this rung moved the suite and the property ratio again, and `README.md` and the CHANGELOG's `[Unreleased]` carry the current figures rather than this row; the three headline milestone labels BR-2B → BR-2C-0 at `src/ugence_benchmark_registry_authority/api.py:1`, `src/ugence_benchmark_registry_authority/__init__.py:1` and `pyproject.toml`'s `description`, which ships as the distribution **Summary**; those same three curated-surface descriptions extended to name `BenchmarkTrustAnchorResolution` (D-34) and `BENCHMARK_VERIFICATION_REFUSAL_REASONS` (D-35); and the stale `93 → 106` inside `__init__.py`'s `0.2.3` bullet with the `BR-2B` label printed by `tests/test_adversarial_ratio.py`. No generator literal moved, so no manifest was regenerated and none was hand-edited. **What this row does not do:** it mints no rung and amends no ladder; it moves no count, no digest domain, no pinned vector, no refusal member, no capability token and no `package_version`; it amends no ruling of D-33, D-34 or D-35; it does not make `BR-2C-0` a subphase — D-01's five stand unamended and the rung still mints no closure audit, which BR-2C takes at `0.3.0` on D-32's terms and its external cryptographic audit; and it begins no BR-2C verifier engineering, which D-32(3) leaves standing in full. **No artifact of this package describes the verifier as audited, independently reviewed or production-ready, and none may until that audit is obtained and recorded.** |
| **D-37** | The capability-ban vocabulary is self-attested: `FORBIDDEN_CAPABILITY_UNLOCK` and `BR2A_FROZEN_CAPABILITY_TOKENS` are both literals in one test module, so an edit removing a token from each moves both sides of the "no weaker than BR-2A froze" comparison together. Is that accepted, or is a second authority built? | **RATIFIED 2026-08-22 — SELF-ATTESTATION IS THE STANDING POSTURE AT `BR-2C-0`, WITH THE MEASURED LIMIT RECORDED RATHER THAN A GUARD CLAIMED.** No second authority is built and no shape scan is added at this rung [R]. *The escape, measured* [V]: deleting `"keyparser"` and `"key_parser"` from both `FORBIDDEN_CAPABILITY_UNLOCK` and `BR2A_FROZEN_CAPABILITY_TOKENS` (`packages/benchmark-registry-authority/tests/packaging/test_milestone_boundary.py`) and shipping a `class KeyParser` with a working `parse()` under `src/` leaves the suite green at 2113. **CORRECTED 2026-08-22 BY D-38 — THE GROUND RECORDED HERE WAS FALSIFIED, AND THE STRUCK TEXT IS REPLACED BY THE MEASURED POSITION.** *Struck:* "the hole is a stub-sized one, and that is why this posture is survivable"; "only a parser that imports nothing outside the allow-list and computes nothing — a placeholder — survives, and §17 already bans shipping an executable placeholder"; and the clause crediting the list-free gates with the load and calling the token list "a naming guard layered over those, not the only thing standing between this rung and a shipped capability". *What replaces it, measured against the same tree with the tokens removed* [V]: a `KeyParser` that digests its input fails `test_no_module_outside_canonical_computes_a_digest` — **because of where the module sits, not because it digests**, since that same digesting parser written into `contracts/canonical.py` instead leaves the suite **green at 2113**; the digest gate is scoped to location. One that calls `base64.b64decode` fails the curated stdlib allow-list twice, at `tests/contract/test_timestamps.py::test_the_only_non_stdlib_import_anywhere_is_the_frozen_br1_layer` and `tests/packaging/test_dependency_boundary.py::test_the_only_non_stdlib_import_is_the_frozen_br1_layer`. **But a `KeyParser` whose `parse()` validates `"<key-id>:<base64>"` with `re` and decodes the base64 in pure Python — a bit accumulator, no import beyond `re` — ships green at 2113, and it is not a placeholder: it round-trips `base64.b64encode` at every length 1..39 and raises on malformed input** [V]. With the two spellings restored, **exactly one** gate catches it: `test_no_class_or_function_anywhere_carries_a_forbidden_capability_name`. The token list is therefore the **sole** control preventing a working key parser from entering the package, not a naming guard layered over gates that carry the load. **§17 bans no placeholder** [V]: it is *Registry and resolution semantics* — fourteen rules on lookup, registration, revocation and supersession, plus §17.1 on historical resolution — and the word appears nowhere else in this ADR. The real enforcement is `EXPORTED_IMPLEMENTATION_UNLOCK`'s permanent `{stub, fake, dummy, noop, null_}` in `tests/contract/test_confusable_and_ports.py` — a **name** ban, in the same self-attested one-file two-literal pattern this row rules, and one that stops none of the four variants above. *Ground:* a ban set states what must **not** exist, and every committed manifest here is generated from what **does** — `tools/generate_manifests.py` derives the four from `api.__all__`, the sealed registry and live dataclass fields, and `gate_mutation_sweep.py` writes `gate_inventory.json` from gates that exist. A prohibition derived from the tree is vacuous: it would pass by construction. So **no generated artifact in this tree carries this vocabulary** — verified against the five committed JSONs, none of which holds a prohibition [V] — and a hand-written block inside a generated file is dropped on the next regeneration [V]. **Narrowed 2026-08-22 by D-38:** this is a fact about these five files, not a modal truth about generators; one emitting a hand-authored list would carry a prohibition, as this row's own next sentence concedes. A **new** committed manifest reproduces the failure mode one directory away — a symmetric edit then spans two files instead of one — and buys attestation, not enforcement. *Two corrections to the assessment this row rests on, both measured* [V]: **the ADR does name tokens.** D-33's row names `signature_verifier`, `key_parser`, `trust_anchor_store` and `approval_verifier` "and their unseparated spellings" — **eight of the twenty-five**, and precisely the BR-2C-unlocking eight, inside a row under §35.2 change control. The assessment recorded that the ADR names none. That makes the ADR-row option **stronger than it was credited for**, though still partial, still needing a novel document-parsing check, and still against the standing precedent that only BR-1's `README.md` is parsed by any check. The second correction — "the residual escape is narrower than the demonstration reads" — is **struck by D-38 as falsified**: the escape is exactly as wide as the demonstration reads, and variant C above ships a working parser green. The ADR-naming correction stands and changes the ruling not at all; both are recorded so a later reconsideration starts from the measured position. *What this row does not do:* it does not weaken, restate or renumber any ban — the twenty-five capability tokens and the eighteen exported-implementation tokens are untouched, and no token is added; it moves no count, no digest domain, no pinned vector, no refusal member and no `package_version`; it adds no test, so the suite stays at **2113** and the property ratio at **508 : 38 = 13.37 : 1**, both re-stated from fresh runs and never edited. **It rules for `BR-2C-0` only.** The tokens are inert here because no capability exists to name: the rung ships contract surface and nothing else. At `0.3.0` a verifier makes them load-bearing, and this posture is **not carried forward** — BR-2C re-decides it on its own terms, alongside D-32(4)'s external cryptographic audit. *Covered by the same ruling:* the identical one-file two-literal pattern at `tests/contract/test_confusable_and_ports.py`, holding `EXPORTED_IMPLEMENTATION_UNLOCK` and `BR2A_FROZEN_EXPORT_TOKENS` [V] — a ruling naming only the first module would leave its twin unruled. *Where the warning lives:* the `BR2A_FROZEN_CAPABILITY_TOKENS` comment states the limit in place, and this row is the ratification behind it. The control is the reviewer of any diff touching either literal, and that control is **weaker than a gate** — recorded here plainly rather than left to be inferred [G]. Nothing in this row begins BR-2C verifier engineering, and no artifact of this package describes any verifier as audited, independently reviewed or production-ready. |
| **D-38** | D-37 ratified self-attestation of the capability-ban vocabulary at `BR-2C-0` on the ground that the residual escape is stub-sized. A fourth independent audit falsified that ground. Does the ruling stand, and on what? | **RATIFIED 2026-08-22 — D-37 STANDS FOR `BR-2C-0` ONLY, ON A REPLACED GROUND; THE ORIGINAL GROUND IS WITHDRAWN AS FALSIFIED.** The rung ships **no cryptographic capability**, and that inertness — not the size of the escape — is why self-attestation is survivable here [R]. The "stub-sized residual escape" ground D-37 rests on is **withdrawn**, and D-37's text is corrected in place rather than left standing beside this row. ***The measured position*** [V], reproduced independently against a scratch copy of this package at this head, with `"keyparser"` and `"key_parser"` deleted from both `FORBIDDEN_CAPABILITY_UNLOCK` and `BR2A_FROZEN_CAPABILITY_TOKENS` in `tests/packaging/test_milestone_boundary.py`, then a `class KeyParser` shipped under `src/` in four variants. **A.** A parser that digests its input, in `contracts/keys.py`, fails exactly one gate, `test_no_module_outside_canonical_computes_a_digest`. **D.** *That same digesting parser* written into `contracts/canonical.py` instead ships **green at 2113** — so variant A fails because of **where the module sits**, not because it digests; the digest gate is scoped to location, and stating otherwise was the error in D-37. **B.** A parser calling `base64.b64decode` fails the curated stdlib allow-list twice, at `tests/contract/test_timestamps.py::test_the_only_non_stdlib_import_anywhere_is_the_frozen_br1_layer` and `tests/packaging/test_dependency_boundary.py::test_the_only_non_stdlib_import_is_the_frozen_br1_layer`. **C.** A parser whose `parse()` validates `"<key-id>:<base64>"` with `re` and decodes the base64 in **pure Python** — a bit accumulator, no import beyond `re`, nothing digested — ships **green at 2113**. It is not a placeholder: it round-trips `base64.b64encode` at every payload length 1 through 39, and raises on malformed input. Restore the two spellings and re-run variant C: **exactly one** gate catches it, `test_no_class_or_function_anywhere_carries_a_forbidden_capability_name`. ***What that establishes*** [V]. **The naming-token guard is currently the SOLE control preventing a working key parser from entering this package.** D-37's claim that the list-free gates carry the load — no concrete class satisfies a port, nothing performs cryptography, no module outside `canonical` computes a digest, no cryptographic module is imported — is false of variant C, which trips none of them. The token list is not "a naming guard layered over those"; with the tokens gone there is nothing underneath it. **And §17 bans no placeholder** [V]: §17 is *Registry and resolution semantics*, fourteen rules on lookup, registration, revocation and supersession plus §17.1 on historical resolution, and the word appears nowhere else in this ADR. The real enforcement is `EXPORTED_IMPLEMENTATION_UNLOCK`'s permanent `{stub, fake, dummy, noop, null_}` in `tests/contract/test_confusable_and_ports.py` — a **name** ban, in the very same self-attested one-file two-literal pattern, which stops **none** of variants A through D. That miscitation is corrected at every site it ships: D-37's row, the CHANGELOG's D-37 entry, `banned_capability_tokens`'s docstring in `tests/_milestones.py`, and the `EXPORTED_IMPLEMENTATION_UNLOCK` comment and `test_a_placeholder_implementation_name_is_banned_at_every_subphase`'s docstring [G closed]. ***Why the ruling nonetheless stands here*** [R]. A ban set is load-bearing only against a capability that exists to be banned. At `BR-2C-0` none does: the rung ships BR-2C's contract surface and nothing else, no verifier, no key parser, no trust resolver, no store and no clock. A token naming a capability the rung cannot ship is inert, and a guard over inert tokens can be self-attested without anything being at risk. That is a **different** ground from D-37's, and it does not survive the arrival of a capability — which is what the precondition below is for. ***No shape scan is accepted as a substitute*** [R]. A classifier that tries to recognise a parser by shape — imports, arithmetic density, string handling — would be another gameable one, and variant C is the demonstration: it was written to pass exactly the gates that exist. Adding one would buy a second literal to edit and a new false sense that the boundary is mechanical. **This is why this pass adds no test, and that is a ruling, not an omission.** The suite therefore stays at **2113** and the property ratio at **508 : 38 = 13.37 : 1**, both re-stated from a fresh run and never edited. ***Precondition, binding before any capability-bearing BR-2C release, `0.3.0` included*** [R]. A **distinct independent authority** — not the owner, and not this or any prior audit of a contract-only rung — must review and adversarially audit the **exact release head**, and must confirm both: **(i)** that only ratified parsing, verification and trust-resolution capabilities ship; and **(ii)** that changing the prohibition list cannot self-authorize a new capability. **If that review is unavailable, `0.3.0` REMAINS BLOCKED.** This amends **D-32(1)** and **(2)**, whose distinct-reviewer waiver is withdrawn for any capability-bearing BR-2C release and survives only for `BR-2C-0`; D-32(3), (5) and (6) are unamended. It is **not** the same gate as **D-32(4)**: that is an external cryptographic audit of the verifier and a precondition to **production use**; this is a distinct independent reviewer of the release head and a precondition to **shipping at all**. Satisfying either leaves the other outstanding [G]. ***What this row does not do.*** **It adds no executable capability to `BR-2C-0`.** It weakens no ban, adds no token and removes none — the twenty-five capability tokens and the eighteen exported-implementation tokens are untouched; it moves no count, no digest domain, no pinned vector, no refusal member and no `package_version`; it adds no test; it mints no rung and amends no ladder; and it begins **no BR-2C verifier engineering**, which D-32(3) leaves standing in full. **No artifact of this package describes any verifier as audited, independently reviewed or production-ready, and none may until both preconditions above are obtained and recorded.** **Amended 2026-09-06 (D-44 — owner ruling, Option 2).** D-38(i)'s reviewer condition is amended from *a distinct independent authority — not the owner* to: **a reviewer distinct from the author of the commit under review, who may be owner-affiliated**. The owner ruled this explicitly, after being told twice that the names supplied (Rakesh Mohan; Ugence Labs Private Limited) fail the original condition and once that the repository identifies Ugence Labs as the platform's own company. The reviewer-authority ruling of 2026-09 (*an independent external cryptographic reviewer*) is superseded by this amendment to the same extent. **What this amendment costs, stated so it is not read later as free** [V]: it reverses the ground D-38 restored from D-32 — that ratification and review must not be the same hand — and it means a review under it is an **owner-affiliated review**. Every artifact of this distribution that describes a review conducted under this amendment must say *owner-reviewed* or *owner-affiliated review*, and **may never say independently reviewed**. The amendment reaches D-38(i) only: **D-32(4)'s external cryptographic audit is not amended and remains a separate, outstanding precondition** to any production use, and `0.3.0` still requires both D-38(i) as amended and D-32(4) as it stands to be recorded as satisfied. |
| **D-39** | May BR-2C's capability-bearing release contain private key/signature parsing needed to implement the D-24 verification seams, and does ratifying that here discharge D-38(i)? | **RATIFIED 2026-08-23 — PERMITTED AS A PRIVATE CAPABILITY; THIS RATIFICATION DOES NOT DISCHARGE D-38(i), WHICH REMAINS OUTSTANDING.** BR-2C's capability-bearing release **may** contain key and signature parsing needed to implement the publisher, approval and revocation verification seams D-24 (and D-25/D-26) ratify, as a capability internal to the package: it **must export no parser API** — no name is added to `api.__all__` or `public_api.json` by this row, and none is authorized by it. Before any such implementation, the accepted key-identifier grammar, the accepted public-key and signature encodings, their exact lengths, the canonical representation, and the rejection behavior for every malformed input must be **closed and pinned**; a malformed input is expressed only as a typed refusal, never raised, consistent with D-24's ruling that verification answers are returned as evidence-bound values. *What this ruling does and does not do* [R]. It **permits** the capability described above. It does **not** satisfy D-38(i). D-38(i) requires a **distinct independent authority** — not the owner — to review and adversarially audit the **exact capability-bearing release head** and confirm that only ratified parsing, verification and trust-resolution capabilities ship. A §35.2 row is an act of ratification by the same owner `.github/CODEOWNERS:113-118` names alone (D-32); it decides **what is permitted**, not **what shipped**, and no wording in this row — however exactly the grammar and encodings are pinned — performs the inspection D-38(i) requires. **Non-exported status narrows nothing D-38 reviews.** D-38(i) reviews the release head, not `api.__all__`: the gate D-37/D-38 found to be the sole surviving control, `test_no_class_or_function_anywhere_carries_a_forbidden_capability_name` (`packages/benchmark-registry-authority/tests/packaging/test_milestone_boundary.py:486-498`), already scans every file under `SRC.rglob("*.py")`, not the exported surface; so do the dependency and crypto-import gates cited in D-33 and D-40. A private parser is exactly as much within D-38's review scope as an exported one. *What this row does not do:* it begins no verifier engineering; it moves no port, test, capability token, unlock phase, refusal member or `package_version`; it amends no file under `packages/`; it does not claim D-38's review or D-32(4)'s audit has occurred, and neither has; and it authorizes no implementation to merge before D-38's precondition is met. **Amended 2026-08-24:** the precondition this row states — the accepted key-identifier grammar, the accepted public-key and signature encodings, their exact lengths, the canonical representation and the per-malformation typed refusals — is **closed by D-42**, which pins them. D-39's permission and its D-38(i) ruling stand unchanged and are what that row builds on. Closing the precondition begins no verifier engineering and discharges neither D-38(i) nor D-32(4); neither review has occurred. |
| **D-40** | Does a vetted third-party Ed25519 dependency for BR-2C's verifier require an exception to §23, and does BR-2's existing no-cryptography posture change now? | **RATIFIED 2026-08-23 — §23 DOES NOT REACH THIRD-PARTY DEPENDENCIES; BR-2's OWN NO-CRYPTOGRAPHY GATES REMAIN BINDING UNTIL A RATIFIED RELEASE TRANSITION DELIBERATELY REPLACES THEM.** §23's consumer/dependency matrix governs the direction of imports **between Ugence packages**: every row and "must never import" entry names an Ugence package, and its invariants ("no leaf imports an authority internal") speak only to that graph (this ADR, §23). It states no rule about third-party dependencies. A vetted, maintained third-party Ed25519 dependency for BR-2C's verifier therefore **requires no exception to §23** [V]. `packages/trusted-evidence-authority/pyproject.toml:33-52,80-83` records the same reading for TEV-2's own `cryptography`+`PyNaCl` posture, and records that an in-package RFC 8032 Ed25519 implementation was deleted after the TEV-2 closure audit found real vulnerabilities in it — evidence against reimplementing Ed25519 in pure Python, not a ruling that binds BR-2's separate posture: that pyproject comment is unratified prose about a different, structurally distinct package (no Ugence dependency at all, governed by E-1/E-2, not by D-01's subphase ladder) and carries no ratification weight of its own, on the same footing this ADR already applies to unratified explanatory prose elsewhere (§35.1, struck paragraph, 2026-08-22). **This interpretive reading changes nothing BR-2 currently enforces.** BR-2's own gates — the pyproject/dependency-string ban and frozen-BR1-only import allowlist (`packages/benchmark-registry-authority/tests/packaging/test_dependency_boundary.py:88-90,102-121`), "nothing in the package performs cryptography" (`packages/benchmark-registry-authority/tests/packaging/test_milestone_boundary.py:642-648`), and "no cryptographic module is imported" (`packages/benchmark-registry-authority/tests/contract/test_trust_contracts.py:419-424`) — are BR-2-specific milestone gates ratified under D-01/D-33, not §23 gates, and D-33 records that their load is carried by D-32(4)'s external cryptographic audit. **They remain binding exactly as ratified** until BR-2C's capability-bearing implementation deliberately replaces them under its own ratified release transition, at the version §35.1/D-33 name for that closure; this row authorizes no such replacement now. When that transition occurs, BR-2C must use a **maintained, vetted** Ed25519 implementation; it must not reimplement Ed25519 (or a primitive it depends on) in pure Python, and must not reuse the Ed25519 implementation belonging to Policy Authority, Risk Authority, or `ugence_trusted_evidence_authority` — the last is already BR-2's first forbidden import (`test_dependency_boundary.py:46`), and D-22(4) is the standing precedent that a shape is copied and the code never is, across exactly these boundaries. **Dependency selection is left OPEN** — no repository evidence ratifies a specific package for BR-2's own verifier. TEV's `cryptography`+`PyNaCl` pair is a precedent for a maintained, vetted, dual-backend pattern, not a binding selection for BR-2, which is a structurally distinct leaf with a narrower single-profile need (`ED25519_SHA512_V1`, D-29(1)). Recorded as a new open item, **DD-12** (§31), with its options and consequences; selection is an owner decision for the ratified release transition, not this row. *What this row does not do:* it moves no port, test, capability token, unlock phase, refusal member or `package_version`; it amends `FORBIDDEN_PACKAGES` in no file and no crypto gate under `packages/`; it does not claim D-32(4)'s audit has occurred, and it has not; and it does not itself authorize adding a dependency to any `pyproject.toml`. |
| **D-41** | Which vetted, maintained third-party Ed25519 implementation does BR-2C's capability-bearing release adopt, closing DD-12? | **RATIFIED 2026-08-24 — OPTION (i): THE `cryptography` + `PyNaCl` PAIR, ON A GROUND DD-12 DID NOT RECORD. THIS SELECTS A DEPENDENCY AND PERFORMS NO RELEASE TRANSITION.** BR-2C's capability-bearing release adopts `cryptography` for Ed25519 signature verification and `PyNaCl` (libsodium `crypto_core_ed25519_is_valid_point`) for strict public-key point validation at the moment key material enters a trust-anchor record — the same pair, the same division of labour and the same bounded-on-both-sides declaration style as `packages/trusted-evidence-authority/pyproject.toml:80-83`. It is adopted as a **selection of two third-party distributions**, never as a dependency on `ugence_trusted_evidence_authority`, which stays BR-2's first forbidden import (`packages/benchmark-registry-authority/tests/packaging/test_dependency_boundary.py:46`) on D-22(4)'s standing precedent that a shape is copied and the code never is. Option (iii) was closed by D-40 and is not reopened. ***Ground 1 — DD-12's recorded cost of (i) is refuted by measurement*** [V]. DD-12 recorded (i)'s cost as "carrying PyNaCl's stricter validation surface even if BR-2C's narrower profile does not need it". Measured differentially against `cryptography` 41.0.7 and `PyNaCl` 1.6.2 on the exact input classes BR-2C's verification seams would see: on a **malleable signature** (a genuine signature with `S + L` substituted) the two libraries **agree** — both refuse — so malleability buys nothing here; on an **all-zero key** both refuse. They diverge on precisely one axis, which is *which public keys are admissible*: for the canonical identity point `0100…00`, the non-canonical identity `0100…80`, an order-8 torsion point and a `y ≥ p` encoding, `cryptography` **accepts** a signature forged without any private key and reports verification as successful, while libsodium refuses the signature and `crypto_core_ed25519_is_valid_point` refuses the key outright. That is a universal forgery reachable through a maintained backend, not a defect of the deleted in-package implementation, and PyNaCl is what closes it. ***Ground 2 — the surface is BR-2C's, not only TEV-2's*** [V]. The TEV-2 closure audit's F-01 and F-03 are findings about **admitting a key**, and BR-2C has that seam: `BenchmarkTrustAnchorRecord.public_key_material` is validated as an **encoding** — exactly 64 lowercase hex characters, never decoded, no curve point checked, no key object constructed (`packages/benchmark-registry-authority/src/ugence_benchmark_registry_authority/contracts/_validation.py:223-251`, called at `contracts/trust.py:473-475` for the field declared at `:435`). That is correct and deliberate for a contract-only rung under D-04, and it means the string `01` followed by thirty-one `00` bytes satisfies the record today. D-25 hands that record to the verification seams D-24 ratifies. F-03's shape therefore already exists one milestone ahead in this package, and F-06 (secret-dependent signing) and F-02 (no second implementation to differ against) are the only two findings that do **not** generalize, F-06 because BR-2 declares four ports and no signer (DD-10b) and registry-event signing arrives at BR-2D under D-12. ***Ground 3 — the profile count is orthogonal*** [V]. D-29(1) fixes BR-2C at one profile, `ED25519_SHA512_V1`. Every divergence measured in Ground 1 is about which key is installed, not which profile it signs under, so a single profile removes none of it; DD-12's inference from "narrower single-profile need" to "may not need the stricter posture" does not hold. ***Ground 4 — the audit trail D-32(4) must cover*** [V]. Under (i), D-32(4)'s external cryptographic audit inherits corpora that exist and are standing tests: `packages/trusted-evidence-authority/tests/authority/test_backend_strict_corpus.py` (F-01's malformed corpus), `test_backend_differential.py` (F-02's cross-implementation check) and `test_backend_ed25519_conformance.py` (RFC 8032 §7.1 vectors, which reproduced byte-for-byte across the TEV-2 backend substitution). Those are a **precedent to re-run against BR-2C's own seams**, not evidence about BR-2C, and re-running them is the audit's work and not this row's. Under (ii) the same corpora would have to be constructed before the audit could begin. ***Why (ii) loses, including its strongest form*** [I]. (ii)'s serious variant is not "pick a different library" but **PyNaCl alone**: BR-2C verifies and never signs, and libsodium's own verification already refuses every Ground-1 input, so one dependency would cover BR-2C's whole need with the stricter default. It is rejected on two counts. It leaves no second implementation to differ against, which is F-02 restated as a design choice rather than a finding. And BR-2D's ratified registry-event signing (D-12) reintroduces a signing backend, so the saved dependency returns under a further ratification one milestone later; this is inference about a milestone not yet built, marked [I] and load-bearing for neither Ground 1 nor Ground 2, each of which is sufficient on its own. ***What this row does NOT do.*** **(a) It selects a dependency and replaces no gate.** BR-2's no-cryptography gates — the pyproject/dependency-string ban and the frozen-BR1-only import allowlist (`tests/packaging/test_dependency_boundary.py:88-90,102-121`), "nothing in the package performs cryptography" (`tests/packaging/test_milestone_boundary.py:642-648`) and "no cryptographic module is imported" (`tests/contract/test_trust_contracts.py:419-424`) — **remain binding exactly as ratified**. D-40 reserves their replacement for the ratified release transition at the version §35.1/D-33 name for that closure, and **this row authorizes no such transition**: it changes no prohibition list, amends `FORBIDDEN_PACKAGES` in no file, unlocks no phase, and adds no dependency to any `pyproject.toml` under `packages/`. A selection ratified in advance of the transition is what keeps the transition from being a moment at which both the gate and the library are decided at once. **(b) It does not discharge D-39's precondition** [G]: the accepted key-identifier grammar, the accepted public-key and signature encodings, their exact lengths, the canonical representation and the per-malformation typed refusals are **still unclosed**, and naming a library closes none of them — `require_public_key_material` is an encoding rule, and which bytes a backend would then be handed is exactly what D-39 leaves open. **(c) It does not discharge D-38(i)** [G]: no distinct independent authority has reviewed or adversarially audited a capability-bearing release head, because none exists; **(d) nor D-32(4)** [G]: no external cryptographic audit has occurred. Satisfying either would still leave the other outstanding, and this row satisfies neither. **(e) It does not satisfy the distinct-reviewer condition D-38 restored** [G]: `.github/CODEOWNERS:113-118` names `@rasaha` alone for BR-2's shipped surface, the author of this work, so this row is ratified by the same owner D-32 identifies and is an act of ratification, never of review. **(f) It moves nothing measured.** No port, test, capability token, unlock phase, refusal member, digest domain, pinned vector, manifest count or `package_version`; `__version__` stays **`0.2.3`** on rung **`BR-2C-0`** (`src/ugence_benchmark_registry_authority/version.py:32`) and the suite stays at **2113**. It begins **no BR-2C verifier engineering**, which D-32(3) leaves standing in full. **No artifact of this package describes any verifier as audited, independently reviewed or production-ready, and none may until D-38(i) and D-32(4) are both obtained and recorded.** |
| **D-42** | D-39 requires the accepted key-identifier grammar, the accepted public-key and signature encodings, their exact lengths, the canonical representation and the per-malformation typed refusals to be closed and pinned before any BR-2C implementation. What are they? | **RATIFIED 2026-08-24 — THE PRECONDITION IS CLOSED. THE TWO HEX ENCODINGS ARE PINNED AS THEY STAND; THE KEY-IDENTIFIER GRAMMAR IS NARROWED, BECAUSE TODAY THERE IS NONE.** ***What is already fixed, and is pinned here rather than left incidental*** [V]. **Public-key material:** exactly **64 lowercase hex characters — 32 bytes**, `_ED25519_PUBLIC_KEY_HEX_RE` (`packages/benchmark-registry-authority/src/ugence_benchmark_registry_authority/contracts/_validation.py:68`), applied at `require_public_key_material` (`:223-251`). **Detached signature:** exactly **128 lowercase hex characters — 64 bytes**, `_ED25519_HEX_RE` (`:74`), applied at `require_detached_signature` (`:199-221`). **Canonical representation for both:** a `str` and never `bytes` — the canonical encoder refuses `bytes` outright — exact type with `str` subclasses refused, unpadded, Unicode NFC, lowercase, one admissible spelling per value (`require_canonical_str`, `:114-156`). Both are validated as **encodings and never decoded**: no curve point is checked and no key object is constructed, which D-04 requires of this package and D-41 leaves to the selected backends at the ratified release transition. The two patterns are **kept as separate constants from `_SHA256_RE` (`:62`) even though all three accept the same or an overlapping language**, and that separation is ratified here rather than left as a comment, so a later change to the digest rule cannot silently re-specify what a public key may be. *Recorded as a limit of the encodings, not a defect* [G]: `_SHA256_RE` and `_ED25519_PUBLIC_KEY_HEX_RE` accept the **identical** language, so a 64-hex value is structurally admissible in both a digest field and a key field and only the field name distinguishes them; nothing here closes that, and closing it would need its own ratification. ***What is not fixed, and is closed here*** [G→closed]. `require_identifier` (`:159-169`) is `require_canonical_str` with `allow_empty=False` and **nothing else**: it fixes exact `str` type, non-empty, unpadded and NFC, and fixes **no alphabet, no length bound and no structure**. BR-1's own `require_identifier` is equally bare (`packages/benchmark-registry/src/ugence_benchmark_registry/contracts/_validation.py:199-203`); the extra grammar there lives in `require_exact_coordinate_token` (`:205-241`) and is applied to **benchmark coordinates only**, never to a key identifier. So a key identifier of ten thousand characters, of mixed case, or spelled with a Cyrillic `е` is admissible in a `BenchmarkTrustAnchorRecord` today. **The ratified grammar for a key identifier is `^[a-z0-9](?:[a-z0-9._-]{0,126}[a-z0-9])?$`** — 1 to 128 characters, lowercase ASCII alphanumerics with `.`, `_` and `-` admissible only in interior positions, on top of the `require_canonical_str` disciplines already listed. *Ground, element by element* [R]. **Lowercase ASCII only, because NFC is not a confusable check:** normalization leaves `kеy-1` (Cyrillic `е`) and `key-1` as two distinct records, which under D-25 — the anchor revision **is** the record's canonical digest — are two anchors, while a directory adapter folding case or script would resolve them to one. The vocabulary has no member for that at this seam: `CONFUSABLE_COORDINATE` is classified `COORDINATE_CONFLICT` (`contracts/reasons.py:305-307`), so **D-35 excludes it from the twelve a `REFUSED` verified result may carry** [V] — a confusable key identifier has no refusal that names it. Restricting the alphabet makes the condition **unrepresentable rather than detected**, which is §09's stated preference and the discipline `_validation.py:19-21` already declares. It also extends to the identifier the single-case rule the digest, public-key and signature fields already carry. **A length bound, because an unbounded identifier is unbounded canonical bytes** inside a record whose digest is its revision (D-25); 128 admits every identifier this package ships with margin, and the exact number is an owner choice ratified here rather than discovered under implementation. **No leading or trailing separator**, so one key has one spelling. *Deliberately not extended* [R]: the floating-token ban (`latest`, `current`, `head`, …) is **not** carried over from `require_exact_coordinate_token`, and `latest` is an admissible key identifier. B-8 and §17.2 make a floating *benchmark coordinate* unrepresentable because a resolver could select on it; D-25 and D-34 resolve an anchor by the exact `(role, identity, key_id)` triple and no seam selects among keys, so a key named `latest` names one key and floats nothing. Recorded so the omission is not read later as an oversight. *Scope* [G]: this grammar binds **key identifiers only**. D-39 names the key-identifier grammar and does not name `identity`, which keeps `require_identifier`'s bare rule — so the Unicode-confusable class stays representable in an identity, and closing that is a separate decision **not taken here**. A **separate validator** is therefore required rather than a tightening of `require_identifier`, which identities share; the in-package precedent is exactly the separation of `_ED25519_PUBLIC_KEY_HEX_RE` from `_SHA256_RE` at `:62-68`. ***The typed refusal for every malformation class*** [V]. Measured against the live validators, and against D-35's admissible twelve — the eleven `TRUST_AND_AUTHENTICITY` members plus `INDETERMINATE`. **Key identifier** — non-`str`, `str` subclass, empty, all-whitespace, padded, non-NFC, outside the ratified alphabet, or over the length bound → **`INDETERMINATE`**, the fail-closed catch-all whose own definition is "the condition could not be determined … never allow; never probably fine" (`contracts/reasons.py:186-188`). **Public-key material** — any non-conformance → **`INDETERMINATE`**, as `require_public_key_material` already attaches. **Detached signature** — any non-conformance → **`SIGNATURE_INVALID`**, as `require_detached_signature` already attaches, and this is not a conflation of shape with verification: the member defines itself as "a declared signature is **absent, malformed**, or did not verify" (`contracts/reasons.py:158-162`), so a malformed encoding is inside the ratified meaning rather than borrowed from it. **Every class has a member, all three carriers are within D-35's twelve, and this row therefore mints none: the BR-2 refusal count stays 24 and the composite stays 41, with §35.6's append-only guarantee untouched.** ***Where a raise becomes a returned refusal*** [R]. D-39 requires a malformed input to be "expressed only as a typed refusal, never raised". That is a rule about the **verification seam**, not about contract construction, and the two are separated exactly as D-34 separated them: a malformed contract raises, a runtime trust condition is returned. So **(a)** every `__post_init__` continues to raise `BenchmarkRegistryContractError`, and `errors.py:37-41` — the absence of a refusal proves structure and nothing else — is unamended; **(b)** the point at which an **external** artifact is admitted into a contract is where the raise must become a `REFUSED` verified result, carrying the reason the validator attached and never propagating the exception past the seam, which is what D-24's ruling that verification answers are returned as evidence-bound values requires; **(c)** the mechanism already exists and is ratified here rather than invented — `BenchmarkRegistryContractError.reason` is set as an instance attribute by the validators "so a caller can branch on the ratified vocabulary rather than on message text" (`contracts/errors.py:44-49`); and **(d)** `reason is None` maps to **`INDETERMINATE`**, never to success and never to an exception escaping the seam. (d) is load-bearing and not hypothetical: **45 raise sites across `contracts/*.py` construct an error class directly with no reason attached** [V], among them the anchor validity-interval rule at `contracts/trust.py:479-485`, so without this rule a malformed anchor would leave the seam as an exception — the one thing D-39 forbids. **No third state is minted on the resolution seam:** D-34's two reasons, `TRUST_ANCHOR_NOT_FOUND` and `TRUST_DIRECTORY_UNAVAILABLE`, remain the only ones a `BenchmarkTrustAnchorResolution` may carry, because a malformed key identifier is refused before the directory is asked and never reaches that seam. ***What this closure moves: nothing measured*** [V for each]. **`api.__all__` and `public_api.json` do not move.** The grammar is enforced in `contracts/_validation.py`, which declares itself a **private module, nothing part of the curated public API** (`:3-4`), and this row mints no exported symbol. Exporting the grammar as a specification constant on the model of D-35's `BENCHMARK_VERIFICATION_REFUSAL_REASONS` is **closed**: D-35 exported a constant derived from a total map that specifies what a *result* may carry, whereas a key-identifier grammar is an **input** rule that the refusal message already states in full, and §05 forbids byte space an artifact does not need. **The canonical domain inventory does not move.** Both inventory gates are keyed on the sealed root-canonicalizable registry (`tests/packaging/test_inventories.py:50-53`), no type is minted and none is sealed, so it stays at **22**, as does `public_contract_inventory.json`. **The pinned vectors do not move.** Canonical bytes are a function of field values, and every key identifier in the pinned corpus — `publisher-key-1`, `approval-key-1`, `revocation-key-1`, re-derived across all **22** vectors — conforms to the ratified grammar, so no vector re-pins. The six literal key identifiers in the test tree, routed through three constants in `tests/_builders.py:149,163,182`, conform as well, so the later tightening breaks no shipped fixture. ***What this row does NOT do.*** **(a) It closes D-39's precondition and authorizes no implementation to merge.** D-38(i)'s precondition still gates any capability-bearing release, and a closed grammar is a specification, not a shipped validator. **(b) It does not discharge D-38(i)** [G]: no distinct independent authority has reviewed or adversarially audited a capability-bearing release head, because none exists. **(c) It does not discharge D-32(4)** [G]: no external cryptographic audit has occurred. **Neither has occurred**, and satisfying either would still leave the other outstanding. **(d) It does not satisfy the distinct-reviewer condition D-38 restored** [G]: `.github/CODEOWNERS:113-118` names `@rasaha` alone for BR-2's shipped surface, the author of this work, so this row is an act of ratification and never of review. **(e) It performs no release transition and selects no dependency:** BR-2's no-cryptography gates remain binding exactly as D-40 and D-41 leave them, no prohibition list, unlock phase or `pyproject.toml` changes, and nothing here decodes a key or a signature. **(f) It begins no BR-2C verifier engineering**, which D-32(3) leaves standing in full; it amends no file under `packages/`; it moves no port, test, capability token, refusal member, digest domain, pinned vector, manifest count or `package_version`; `__version__` stays **`0.2.3`** on rung **`BR-2C-0`** (`src/ugence_benchmark_registry_authority/version.py:32`) and the suite stays at **2113**. **No artifact of this package describes any verifier as audited, independently reviewed or production-ready, and none may until D-38(i) and D-32(4) are both obtained and recorded.** |
| **D-43** | D-42 narrowed the key-identifier grammar and left `identity` on `require_identifier`'s bare rule. Do BR-2C's actor-identity fields need the same grammar, or is the bare rule correct for them? | **RATIFIED 2026-08-25 — YES, AND ON A STRONGER GROUND THAN D-42's: THE SAME GRAMMAR BINDS THE ACTOR-IDENTITY FIELDS, AND ONLY THOSE.** The grammar is D-42's, unchanged: `^[a-z0-9](?:[a-z0-9._-]{0,126}[a-z0-9])?$`. ***The hazard is inverted, and it is worse*** [V]. D-42's key-identifier hazard was that two spellings of one key become two anchor revisions — a bookkeeping failure. For identities the failure runs the other way and lands on D-02. `require_distinct_actors` (`packages/benchmark-registry-authority/src/ugence_benchmark_registry_authority/contracts/_validation.py:306-335`) refuses only on `first == second`, comparing **the exact declared strings**, and its own docstring records that separation is compared on those strings and is not an authenticity check. It is applied at four sites — `contracts/chain.py:219,378,691` and `contracts/envelopes.py:451` — each placed at the first contract where both identities are mechanically reachable. A homoglyph pair therefore **passes** that check while naming one actor: `publisher-alpha` and the same string with a Cyrillic `а` are unequal, so the four-party separation that stops self-approval is satisfied by one party wearing two spellings. NFC is a normalization, not a confusable check (D-42), so nothing upstream closes this. ***The legitimacy objection was weighed and does not hold for this field*** [V]. A lowercase-ASCII grammar on a field carrying a real publisher's name would make legitimate names unrepresentable, which is a different and heavier cost than constraining an internal key label — and that is the reason this row was expected to come out the other way. The tree refutes it. `BenchmarkTrustAnchorRecord.identity` is documented as *the anchor's declared identity **within its role namespace*** (`contracts/trust.py:419-420`), the package carries **no display-name field** on any contract, and every identity literal in the tree is already a role-scoped slug: `publisher-alpha`, `approval-authority-beta`, `registry-authority-gamma`, `revoker-delta` (`tests/_builders.py:82-87`), plus the deliberate near-miss `publisher-alphb` (`tests/contract/test_chain_integrity.py:179`). All conform to the grammar as it stands, so the narrowing breaks no fixture and the distinctness test keeps exercising a one-character-apart pair. These are **handles the composition root assigns** (D-04), not names a third party asserts; the row narrows what a registry issues, not who may be named. ***Scope: three kinds of field share one validator, and only one is narrowed*** [V]. `require_identifier` validates **(a)** actor identities — `identity` (`trust.py:466,627`), `signer_identity` (`trust.py:882`), `publisher_identity` (`envelopes.py:320`), `approval_authority_identity` (`envelopes.py:409`), `revoker_identity` (`envelopes.py:551`) and `declared_registry_authority_identity` (`chain.py:214`, `read_payloads.py:111,181`); **(b)** key identifiers, already ruled by D-42; and **(c)** neither — `applicable_policy_ref` (`envelopes.py:437`), whose pinned value `benchmark-approval-policy/v1` contains a `/` the grammar rejects, and `declared_revocation_reason` (`envelopes.py:572`), which is free text. **(c) is deliberately not narrowed**: a wholesale tightening of `require_identifier` would have refused an already-pinned policy reference, and a separate validator is therefore required rather than a change to the shared one. That a free-text reason and a path-like reference are validated as *identifiers* at all is recorded as a limit, not closed here [G]. ***Refusal member — none minted*** [V]. `require_distinct_actors` raises `UNAUTHORIZED_TRANSITION`, which is **not** among D-35's twelve (measured: the twelve are `APPROVAL_UNVERIFIED`, `INDETERMINATE`, `NO_TRUST_ANCHOR_CONFIGURED`, `PUBLISHER_UNTRUSTED`, `SIGNATURE_INVALID`, `STALE_TRUST_SNAPSHOT`, `TRUST_ANCHOR_DISABLED`, `TRUST_ANCHOR_EXPIRED`, `TRUST_ANCHOR_NOT_FOUND`, `TRUST_ANCHOR_NOT_YET_VALID`, `TRUST_ANCHOR_REVOKED`, `TRUST_DIRECTORY_UNAVAILABLE`). This is the same structural position D-42 met with `CONFUSABLE_COORDINATE`, and it takes the same answer: the condition is made **unrepresentable at construction rather than detected at the seam**. A malformed actor identity refuses as `INDETERMINATE`, which is admissible. **Counts stay 24 and 41**; no member is minted, renamed, re-valued or re-ordered. *What this row does not do:* it moves no port, test, capability token, unlock phase, refusal member or `package_version`; `__version__` stays `0.2.3` on rung `BR-2C-0`; it amends no file under `packages/`, selects no dependency and performs no release transition; it begins **no BR-2C verifier engineering**; it does **not** discharge D-38(i) or D-32(4), neither of which has occurred; it does **not** satisfy the distinct-reviewer condition D-38 restored, `.github/CODEOWNERS:113-118` naming `@rasaha` alone; and no artifact of this distribution describes any verifier as audited, independently reviewed or production-ready. |
| **D-44** | The BR-2C candidate head exists at `0.3.0rc1` (commit `2753404f`, branch `claude/br-2c-candidate-usrr61`). Who is the D-38 reviewer, and is the review commissioned? | **RECORDED 2026-09-06 — REVIEWER OF RECORD: UGENCE LABS PRIVATE LIMITED, REVIEW CONDUCTED BY RAKESH MOHAN, UNDER D-38 AS AMENDED (OPTION 2). STATUS: COMMISSIONED, OUTCOME PENDING. THIS IS AN OWNER-AFFILIATED REVIEW, AND THE CANDIDATE REMAINS NOT INDEPENDENTLY REVIEWED AND NOT AUDITED.** ***What was ruled*** [R]. The owner named **Ugence Labs Private Limited** as the reviewing firm and **Rakesh Mohan** as the reviewer for the exact candidate head at commit `2753404f`, engagement reference none yet, commissioned 2026-09-06, and chose **Option 2**: amend D-38 to accept an owner-affiliated review rather than name a reviewer outside Ugence Labs. The amendment is recorded on D-38 itself. ***Why it is owner-affiliated, on the record*** [V]. `.github/CODEOWNERS:113-118` names `@rasaha` alone for BR-2's shipped surface; `packages/benchmark-registry-authority/pyproject.toml:90` names Rakesh Mohan as the package author; `packages/runtime/agent-runtime/pyproject.toml` and `packages/providers/actiongate/pyproject.toml` name Ugence Labs as author, and the platform briefs describe the products as the Ugence Labs platform. The reviewer is the vendor of the code under review. D-38 as amended permits that; nothing permits calling it independent. **Distinct-author condition** [G]: the seven commits of the candidate head (`4e45f220`..`2753404f`) carry a machine author with the owner's session attribution; the amended D-38 requires the reviewer to be distinct from the *author of the commit under review*, and the owner is treated here as the commissioning principal of those commits rather than a separate reviewer of them. That reading is the owner's, ratified by this row, and is recorded as inference. ***Scope of the commissioned review*** [V]. At commit `2753404f` exactly: `packages/benchmark-registry-authority/src/ugence_benchmark_registry_authority/verifier.py` (the only module importing the D-41 pair; the three seams; the strict point check at anchor admission; RFC 8032 `S < L`; the refusal mapping under D-42(d)); the D-42/D-43 validators `require_key_identifier` and `require_actor_identity` in `contracts/_validation.py` and their application sites; the ladder and both unlock maps in `tests/_milestones.py`, `tests/packaging/test_milestone_boundary.py` and `tests/contract/test_confusable_and_ports.py` (the twelve BR-2C tokens lifted at `BR-2C-RC`, nothing else moved); the strict anchor corpus, key-less-forgery differential, frame reconstruction and D-28 order in `tests/contract/test_verifier.py`; and the three pinned public keys and three pinned signatures in `adversarial_probes.py`. The review is to confirm that only ratified parsing, verification and trust-resolution capability ships, that the D-40 transition moved no other prohibition, and that no artifact claims independent review or audit. ***What this row does not do*** [V]. It does **not** bump `package_version`: `__version__` stays `0.3.0rc1` on rung `BR-2C-RC`, and `0.3.0` stays reserved. It amends no file under `packages/`, moves no port, test, token, unlock phase, refusal member or dependency, and opens no PR. It records a commissioning, not a completion: the outcome is **pending**, and a separate completion row must state the findings and their disposition. It does **not** touch D-32(4): the external cryptographic audit remains outstanding and unamended, so even a completed review under this row leaves `0.3.0` blocked on that audit. Until a completion row exists, every artifact of this distribution continues to describe the verifier as a candidate that is not reviewed and not audited; after one exists, the strongest claim any artifact may make is *owner-reviewed*. **Completion recorded 2026-09-06 — COMPLETED — OWNER-REVIEWED.** *Reviewer* [V]: Rakesh Mohan for Ugence Labs Private Limited, an owner-affiliated review under D-38 as amended, assisted by a reviewing analyst distinct from the candidate implementation author. *Head under review*: commit `2753404f`; fixes at `3f47949f`, `b7fcdfb5` and `1f24806a`; re-verified head `1f24806a` on branch `claude/br-2c-candidate-usrr61`. *Findings and dispositions* [V]. **F-1** (trust-resolution implementation and tests; BLOCKING): the resolved record's role, identity and key identifier were not rechecked against the requested triple after resolution — FIX, fixed at `3f47949f`, re-verified at `1f24806a`. **F-2** (mutation-sweep coverage; NON-BLOCKING): ratified parsing, verification and trust-resolution enforcement branches lacked twelve explicit mutation gates — FIX, gates G-78 through G-89 and the missing adversarial tests added at `b7fcdfb5` and `1f24806a`; final sweep 84 gates, 78 killed, 6 survived, 0 errored; the in-package `S < L` survivor (G-79) is classified as shadowed by the signature backend and retained as defence in depth. **F-3** (naive-instant validation test; NON-BLOCKING): the test did not prove that invalid instant inputs were refused before the trust directory was consulted — FIX, the test now proves zero consultation for naive, string and absent instants and exactly one consultation for an aware instant, pinned by G-89, re-verified at `1f24806a`. **F-4** (maturity and review-language statements; NON-BLOCKING): eleven documentation and source locations misstated D-38(i) or failed to distinguish the amended owner-affiliated review from the outstanding D-32(4) external cryptographic audit — FIX, all eleven sites corrected at `1f24806a` and re-verified. *Sign-off, verbatim*: "I confirm that, at re-verified head 1f24806a, the BR-2C candidate ships only the parsing, verification and trust-resolution capability ratified under D-40, and that the D-40 transition moved no other prohibition." *Final evidence at `1f24806a`, from fresh runs* [V]: package suite **2261 passed and 1 pre-existing failure**; adversarial probes **93 passed**; property ratio **561:41, or 13.68:1**; mutation sweep **84 gates, 78 killed, 6 survived, 0 errored, exit 0**; distribution verifier **verified, with 8 of 8 negative controls caught**; pyflakes **clean**. *The one remaining package-suite failure, exactly* [V]: `tests/packaging/test_dependency_boundary.py::test_no_package_in_the_monorepo_imports_this_one`, message `AssertionError: ['packages/integration/ai-system-registry/tests/test_boundaries.py']` — `assert [...] == []`. The gate scans every other package's source for the string `ugence_benchmark_registry_authority`, and `packages/integration/ai-system-registry/tests/test_boundaries.py:28` names it inside that package's own forbidden-import list. That file was last changed at commit `973b7061`, an ancestor of `49ca9d67`, the default-branch head this candidate branch was cut from, so the failure predates every D-44 candidate change and is outside their scope; it is the BR-2A terminal-state gate reading a neighbour's ban list as a reverse import. **The package suite is not green**, and the failure is neither suppressed nor repaired here; either is a separate authorization. *Stated explicitly* [V]: `package_version` remains **`0.3.0rc1`** on rung **`BR-2C-RC`**; capability tokens, unlock phases, refusal members, digest domains and pinned vectors **did not move**; **D-32(4)'s external cryptographic audit remains outstanding**; and **release as `0.3.0` remains blocked on that audit**. This outcome is **owner-reviewed** and is never to be described as independently reviewed, externally reviewed, cryptographically audited, production-ready or released. |
| **D-45** | The twelve BR-2C capability tokens unlock at `BR-2C-RC`. At `0.3.0` on rung `BR-2C` that unlock is in the past, and `test_no_capability_beyond_br2c_is_scheduled_to_unlock_at_or_before_this_version` requires it to be exactly *at* the reached rung. Does the unlock phase stay at `BR-2C-RC`, or move to `BR-2C`? | **RATIFIED 2026-09-06 — THE UNLOCK PHASE STAYS AT `BR-2C-RC`; THE GATE RELAXES TO `<=` FOR THOSE TWELVE TOKENS AND FOR NOTHING ELSE.** ***What was ruled*** [R]. `FORBIDDEN_CAPABILITY_UNLOCK` and `EXPORTED_IMPLEMENTATION_UNLOCK` keep `BR-2C-RC` as the unlock phase of the twelve tokens D-33 records as BR-2C's. The BR-2C branch of the gate at `packages/benchmark-registry-authority/tests/packaging/test_milestone_boundary.py` changes from `SUBPHASE_LADDER.index(unlock) == reached` to `<= reached`; the `> reached` branch every other token takes is **unchanged**, and no token, no unlock phase and no ban set moves under this row. ***Ground*** [V]. The twelve tokens did unlock at `BR-2C-RC`: the candidate rung ships `BenchmarkEd25519Verifier` and `BenchmarkDenyAllVerifier` under exactly those names, and both unlock maps are pinned to `BR-2C-RC` in both directions at `test_milestone_boundary.py:205-227` and `tests/contract/test_confusable_and_ports.py:115-133`. Rewriting the phase to `BR-2C` would make the map state that the capability was banned at the rung that shipped it, which is a false statement about this repository's own history, and it would silently re-ban the twelve at `BR-2C-RC` for anyone re-running the ladder against the candidate. ***What the relaxation costs, stated so it is not read later as free*** [V]. `== reached` pinned two facts at once: that the twelve unlock at a named rung, and that no unlock sits below the reached rung. `<= reached` drops the second for those twelve only. The first is not lost: the set equality against `{token for token, unlock in ... if unlock == "BR-2C-RC"}` in both files already pins the rung **by name**, in both directions, so a token whose unlock drifted to any other rung fails there. ***Measured*** [V], on a scratch copy of the package at head `f8ea5573c` with `__version__` set to `0.3.0` and no other change: **exactly three of 2263 tests fail** — `test_happy_the_package_version_is_the_br2c_candidate_rung`, `test_the_manifest_version_equals_the_package_version` and this gate (`assert 3 == 4`). **No capability-ban assertion fails**, which is the measurable difference from D-33's five-red measurement at `0.2.x`, where three of the five were ban assertions reporting that the bump unlocked capability the slice did not ship. The transition already lifted the twelve; the version move no longer lifts anything. **Re-measured after the relaxation this row rules** [V], same scratch method: **two** of 2263 fail, and both are the version literals; the gate passes at `0.3.0rc1` and at `0.3.0` alike, and the package suite at the real head is **2263 passed, 0 failed**. ***What this row does not do*** [V]. It does **not** bump `package_version`: `__version__` stays `0.3.0rc1` on rung `BR-2C-RC`, and the two version literals the measurement names are **not** moved here. It adds no token, removes none, moves no refusal member, digest domain, pinned vector, port or manifest count, and discharges neither D-38(i) nor D-32(4). |
| **D-46** | D-44's completion row reviewed head `1f24806a`. The package has since moved to `f8ea5573c`. Does D-38(i) carry forward to the `0.3.0` head, or does the delta require its own review? | **RATIFIED 2026-09-06 — A DELTA RE-VERIFICATION IS REQUIRED, AND IT IS OUTSTANDING. THE COMPLETION ROW DOES NOT CARRY FORWARD TO A MOVED HEAD.** ***What was ruled*** [R]. D-38(i) reviews the **exact release head**. Any head offered as `0.3.0` that is not the head D-44 recorded requires a **delta re-verification row**, recording the reviewer, the commit range reviewed, and a sign-off in the same form as D-44's — confirming for the moved head that only ratified parsing, verification and trust-resolution capability ships and that the D-40 transition moved no other prohibition. A delta review under D-38 as amended is **owner-reviewed**, never independently reviewed. ***The delta, exactly*** [V]. `git diff 1f24806a..f8ea5573c -- packages/benchmark-registry-authority` is three files: `CHANGELOG.md`, `README.md` and `tests/packaging/test_dependency_boundary.py`, in one commit, `f8ea5573c`, which rewrites the reverse-import gate to read imports over the AST rather than by substring. **No module under `src/` is touched**, so no capability moved. The measurable effect is that the one failure D-44 recorded — `test_no_package_in_the_monorepo_imports_this_one`, on `ai-system-registry`'s forbidden-import list being read as a reverse import — is gone: the package suite is **2263 passed, 0 failed** at `f8ea5573c`, where D-44 recorded 2261 passed and 1 pre-existing failure [V]. **D-45 adds a fourth file to the delta**, `tests/packaging/test_milestone_boundary.py`, and this row is written knowing that. ***Ground*** [R]. The delta is small and touches no `src/` module, which is exactly the argument for carrying the completion row forward — and exactly why the ruling goes the other way. `f8ea5573c` changes a **gate**, and `test_dependency_boundary.py` is not in the scope D-44 enumerates. D-38 exists because a fourth independent audit falsified the claim that the list-free gates carry the load; a rung that lets gate changes accumulate against a review of an earlier head reproduces that failure with the review as its cover. Carrying a review forward across gate edits would also make the first such carry-forward the precedent for every later one, with no rung at which the accumulated delta is ever inspected. ***What this row does not do*** [V]. It does **not** perform the delta review, commission one, or name a reviewer — the review is **outstanding**, and a separate completion row must record it. It casts no doubt on D-44's completion at `1f24806a`, which stands as recorded for that head. It moves no `package_version`, token, port, refusal member or test, and it does not touch D-32(4). |
| **D-47** | With D-38(i) recorded complete at `1f24806a`, is `0.3.0` held until D-32(4)'s external cryptographic audit is recorded, or is a further candidate rung minted to carry work in the meantime? | **RATIFIED 2026-09-06 — `0.3.0` IS HELD UNTIL D-32(4) IS RECORDED SATISFIED. NO FURTHER RUNG IS MINTED.** ***What was ruled*** [R]. The ladder is unchanged: `BR-2C-RC` carries `0.3.0rc1` and remains the head at which BR-2C candidate work lands. `0.3.0` and rung `BR-2C` stay reserved for the release that has **both** D-38(i) as amended, at its own head under D-46, **and** D-32(4) recorded as satisfied. ***Ground*** [V]. D-32(4) is an **external cryptographic audit of the verifier**, unamended by D-38, D-44 and this row, and **no test in this repository can assert it** — D-33 records that the version rung protects the twelve tokens, not the blocker, and that the blocker's load is D-32(4)'s. It is therefore the one precondition that is **not satisfiable from inside this repository**, in contrast to D-38(i) as amended, which is satisfiable by an owner-affiliated reviewer and was satisfied at `1f24806a`. §35.1 defines `0.3.0` as the **audited verifier**; releasing it before the audit would make the ladder's own definition false at the moment it is used. A second candidate rung is closed on D-36's ground: it would add a `VERSION_SUBPHASE` entry, a `SUBPHASE_LADDER` index shift and a version literal at every pinned site, and rule nothing that `BR-2C-RC` does not already carry. ***What this costs*** [V]. BR-2C stays open, and with it BR-2D, which §35.9 records as awaiting BR-2C and DD-10 — DD-10b, the HSM/KMS posture, is itself still deferred. That cost is accepted here rather than discovered later. ***What this row does not do*** [V]. It commissions no audit and names no auditor; D-32(4) remains outstanding and unamended. It moves no `package_version`, rung, token, port, refusal member or manifest, and it does not weaken D-46. Until both preconditions are recorded, every artifact of this distribution continues to describe the verifier as a **candidate**, at most **owner-reviewed**, and never as audited, independently reviewed, released or production-ready. |
| **D-48** | D-46 requires a delta re-verification for any head offered as `0.3.0` that is not D-44's reviewed head `1f24806a`. Who is the reviewer, what exactly is the delta, and is the review commissioned? | **RECORDED 2026-09-06 — REVIEWER OF RECORD: RAKESH MOHAN FOR UGENCE LABS PRIVATE LIMITED, UNDER D-38 AS AMENDED, ASSISTED BY A REVIEWING ANALYST DISTINCT FROM THE AUTHOR OF THE COMMITS UNDER REVIEW. STATUS: COMMISSIONED, OUTCOME PENDING. THIS IS AN OWNER-AFFILIATED REVIEW, AND THE CANDIDATE REMAINS NOT INDEPENDENTLY REVIEWED AND NOT AUDITED.** ***What was ruled*** [R]. The owner named the same reviewing arrangement D-44 records, applied to the moved head, and commissioned the review D-46 requires. This row records a commissioning, not a completion: **the review has not been performed**, no finding is dispositioned here, and a separate completion row must state the findings, their dispositions and the reviewer's sign-off before any head is offered as `0.3.0`. ***The delta, exactly*** [V]. From `1f24806a` to `26da5f3cf`, `packages/benchmark-registry-authority` moved by **three commits and four files, none under `src/`**: `f8ea5573c` rewrote the reverse-import gate to read imports over the AST rather than by substring, with a planted-import test over six spellings; `070d56189` made that gate fail closed on a file it cannot parse, with a second planted-file test; `f85246b31` relaxed the BR-2C branch of `test_no_capability_beyond_br2c_is_scheduled_to_unlock_at_or_before_this_version` to `<=` under D-45. The four files are `CHANGELOG.md`, `README.md`, `tests/packaging/test_dependency_boundary.py` and `tests/packaging/test_milestone_boundary.py`. **Two gates and the documentation that follows them; no shipped module moved.** *Corrected here rather than left to be repeated* [G]: the pull requests that introduced `070d56189` and its predecessor described the delta as *five* files. It is four — the third commit touches files the first already did — and a delta-review row that miscounts its own delta is the defect class D-44 finding F-4 corrects. ***Scope of the commissioned review*** [V]. At `26da5f3cf` exactly, under D-46's `DELTA-SCOPE = PACKAGE_FILES_ONLY`: that the delta ships no parsing, verification or trust-resolution capability; that the D-40 transition still moved no other prohibition; that neither gate change weakens what it measures — specifically that reading the AST did not narrow the reverse-import claim beyond the unparseable-file case already closed at `070d56189`, and that `<=` did not admit an unlock below the reached rung for any token outside the twelve; and that no artifact claims independent review, audit or production readiness. ***Figures at this head, carried forward rather than re-measured, and why that is sound*** [V]: `git diff 070d56189..26da5f3cf -- packages/benchmark-registry-authority` is **empty**, so the package tree is byte-identical to the head measured on the pull request that introduced `070d56189`: package suite **2264 passed, 0 failed**; adversarial probes **93 passed**; distinct properties **563 : 41 = 13.73 : 1**; mutation sweep **84 gates, 78 killed, 6 survived, 0 errored**, `gate_inventory.json` byte-identical; distribution verifier **verified, 8 of 8 negative controls caught**; pyflakes clean. ***What this row does not do*** [V]. It does **not** perform the review, disposition any finding, or record a sign-off. It does **not** bump `package_version`: `__version__` stays `0.3.0rc1` on rung `BR-2C-RC`. It moves no token, unlock phase, port, refusal member, digest domain, pinned vector or manifest count, amends no earlier row, and does not touch **D-32(4)**, whose external cryptographic audit remains outstanding and unamended — so even a completed delta review leaves `0.3.0` blocked on it under D-47. Until a completion row exists, every artifact of this distribution continues to describe the verifier as a candidate that is not reviewed at this head and not audited; the strongest claim any artifact may ever make under D-38 as amended is *owner-reviewed*. *Everything above this point is the record as it stood when the review was commissioned, preserved unchanged; the completion follows, on D-44's own two-step form.* **Completion recorded 2026-09-06 — COMPLETED — OWNER-REVIEWED.** *Reviewer* [V]: Rakesh Mohan for Ugence Labs Private Limited, an owner-affiliated review under D-38 as amended. **Recorded on the owner's instruction, and one difference from D-44 is stated here rather than left to be inferred** [G]: D-44's completion names a reviewing analyst distinct from the candidate implementation author. **No such analyst assisted at this head.** The author of `f8ea5573c`, `f85246b31` and `070d56189` is the same agent that prepared this record, so the technical examination behind this completion is the owner's own, over the four-file delta, and is not corroborated by a second party. D-38 as amended permits an owner-affiliated review; nothing permits calling this one independently reviewed, and no artifact does. *Head under review*: `26da5f3cf`; the delta is exactly the three commits and four files stated above, unchanged by this completion — no fix was applied, so the head under review did not move and no new delta is opened under D-46. *Findings and dispositions* [R]: **NONE.** No blocking or non-blocking finding is recorded at this head. What was examined, stated so the absence of findings is legible rather than bare: the AST reverse-import walk and its documented limit for names computed at runtime (`tests/packaging/test_dependency_boundary.py:168`); the `node.level == 0` exclusion of relative imports (`:201`); the six planted import spellings (`:246`); the substring fallback that reports a file the gate cannot parse when it names this package, and its deliberate cost that an unparseable *mentioner* is reported too (`:195`, pinned at `:279`); the `<= reached` branch scoped to `BR2C_CAPABILITY_TOKENS` with the `> reached` branch unchanged for every other token (`tests/packaging/test_milestone_boundary.py:290-292`); and that the rung stays pinned **by name** through the set equality on `unlock == "BR-2C-RC"` in both unlock maps, which is what `<=` no longer pins. *Sign-off* [R]: "I confirm that, at head `26da5f3cf`, the delta from `1f24806a` ships no parsing, verification or trust-resolution capability, that the D-40 release transition moved no other prohibition, that neither gate change weakens what it measures, and that no artifact of this distribution claims independent review, audit or production readiness." *Figures*: as recorded above; the package tree is byte-identical to the measured head, so nothing is re-measured for this completion and nothing is edited. *Stated explicitly* [V]: `package_version` remains **`0.3.0rc1`** on rung **`BR-2C-RC`**; no token, unlock phase, refusal member, digest domain or pinned vector moved; **D-32(4)'s external cryptographic audit remains outstanding and unamended**; and release as `0.3.0` remains blocked on it under D-47. This outcome is **owner-reviewed** and is never to be described as independently reviewed, externally reviewed, cryptographically audited, production-ready or released. |

**Note on D-39 and D-40 (ADR-only, 2026-08-23).** Ratification defines what is **permitted**; it does not independently **verify what shipped** — D-38(i)'s inspection of the exact release head is a separate, still-outstanding act. Non-exported code remains fully within D-38's review scope; privacy from `api.__all__` narrows nothing the distinct authority must inspect. D-38's review of the exact capability-bearing release head **blocks shipping `0.3.0`** until obtained and confirmed positive; D-32(4)'s external cryptographic audit **separately blocks production use** of the verifier once shipped — satisfying one leaves the other outstanding (D-38). Any change made to remediate a finding from either review produces a **new candidate release head**, which requires its own applicable re-review; neither review transfers to a head it did not inspect. **Neither D-38's review nor D-32(4)'s audit has occurred**, and no artifact may describe either as complete, in progress on this row's authority, or waived. This commit is **ADR-only**: it authorizes no change to any prohibition list, unlock phase, cryptographic gate, or dependency declaration anywhere under `packages/`, and begins no BR-2C verifier engineering.
