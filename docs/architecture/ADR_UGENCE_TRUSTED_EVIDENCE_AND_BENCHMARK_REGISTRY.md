...
Forty-four owner decisions, **D-01 through D-44**, all ratified — nineteen at
the original ratification (D-01–D-19) and twenty-five added by later amendment.
...
D-44, without exception**, and the rows are not swept when code moves.
...
| **D-44** | D-38 requires a distinct independent authority to review the exact capability-bearing release head, and D-32(4) requires an external cryptographic audit of the verifier before production use. Neither is achievable in a solo-maintainer model with no external cryptographer engaged. Do they stand? | **RATIFIED 2026-08-25 — BOTH ARE WITHDRAWN AND ONE IS REPLACED. D-41's QUALIFICATION GATE BECOMES A BLOCKING RELEASE GATE; D-38's MEASURED FINDING STANDS UNREMEDIED AND IS ACCEPTED.** The owner has ruled that no external cryptographer will be engaged and no second maintainer exists. This row records that as a decision with its consequences stated, rather than leaving two preconditions standing that will not be met [R]. ***(1) D-32(4)'s external cryptographic audit is withdrawn, and replaced.*** The ground is that **BR-2C links a maintained third-party implementation rather than writing one** [V]: D-40 bars reimplementing Ed25519 or any primitive it depends on in pure Python, and bars reusing the Policy Authority, Risk Authority or `ugence_trusted_evidence_authority` implementations; D-41 rules that signature verification goes through `nacl.signing.VerifyKey.verify` and public-key point validation through `nacl.bindings.crypto_core_ed25519_is_valid_point`. The risk that warranted an external cryptographer is **hand-written cryptography**, and the repository's own evidence for that risk is TEV-2's: an in-package RFC 8032 Ed25519 implementation was deleted after the TEV-2 closure audit found real vulnerabilities in it (`packages/trusted-evidence-authority/pyproject.toml`, cited at D-40) [V]. That class of code is already prohibited here, so what BR-2C ships is **composition, not cryptography**: encoding validation, exception-to-refusal mapping, anchor lookup and fail-closed defaults. **Replaced by:** D-41's release-qualification conditions, which become a **blocking release gate** rather than a stated condition — a wheelhouse pinning exact wheel hashes, and an admission gate exercising identity, non-canonical identity, all eight small-order points and mixed-order prime-order-plus-torsion points, failing the build on any acceptance, qualified per platform wheel. **No gate, no release** [R]. ***(2) D-38's distinct-reviewer requirement is withdrawn as unachievable.*** D-38(i) and (ii) required a distinct independent authority — not the owner — to review and adversarially audit the exact release head. `.github/CODEOWNERS:113-118` names `@rasaha` alone and no second maintainer exists [V]; D-21 has recorded the solo-maintainer model as the operating context since 2026-08-20, and its own question asks what independent review means in that model [V]. The requirement is **withdrawn, not waived and not deemed satisfied**: no review has occurred, none is scheduled, and no artifact may state otherwise. D-32(1) and (2), whose waiver D-38 withdrew for capability-bearing releases, are moot under this row rather than revived. ***(3) What is accepted rather than closed*** [G]. D-38's measured finding is unaffected by withdrawing its remedy and is **accepted as residual risk**: with `"keyparser"` and `"key_parser"` deleted from both `FORBIDDEN_CAPABILITY_UNLOCK` and `BR2A_FROZEN_CAPABILITY_TOKENS`, a working key parser that decodes base64 in pure Python ships **green at 2113**, and exactly one gate catches it — `test_no_class_or_function_anywhere_carries_a_forbidden_capability_name`, a self-attested naming guard in the same module as the list it checks. Nothing in this row closes that. **And the substitution is a real reduction in assurance, recorded here rather than glossed** [G]: a known-answer corpus catches the failure modes its author enumerated, while TEV-2's F-01 and F-03 were **composition** failures — mixed-order keys and identity points — found **by an audit**, not by a suite. An auditor finds what the author did not think to test; the replacement gate cannot. That difference is the accepted cost of this row. ***(4) Disclosure, binding*** [R]. No artifact of this distribution, and no description given to any client or third party, may describe a BR-2C verifier as audited, independently reviewed, externally validated or production-ready. The accurate description is **self-qualified against a ratified adversarial corpus**. This row removes two review gates; it does not license a claim that either occurred. *What this row does not do:* it authorizes no implementation and begins no BR-2C verifier engineering; it moves no port, test, capability token, unlock phase, refusal member or `package_version`, and `__version__` stays `0.2.3` on rung `BR-2C-0`; it amends no file under `packages/`; it changes no prohibition list, cryptographic gate or dependency declaration, which remain binding until the release transition D-33/D-40 name replaces them; it does not reopen DD-12, which D-41 closed; and it does not claim that any review, audit or qualification gate has been performed — the gate it mandates does not yet exist. |
