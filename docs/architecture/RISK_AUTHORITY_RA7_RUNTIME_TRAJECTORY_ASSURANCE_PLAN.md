...
> **SUPERSEDED — discovery companion, retained as supporting evidence.** This
> document's verdict (`RA7_ARCHITECTURE_DECISION_REQUIRED`) and its seven open
> decisions D1–D7 have been **ratified** in the canonical
> `RISK_AUTHORITY_RA7_SPEC.md` (verdict
> `RA7_PRECONDITIONS_RESOLVED_READY_FOR_IMPLEMENTATION`) and
> `ADR_RISK_AUTHORITY_RA7_RUNTIME_TRAJECTORY_ASSURANCE.md`. It is brought onto the
> ratification branch verbatim (from discovery commit `63275f91`) for traceability
> only; where it differs from the SPEC/ADR, the SPEC/ADR govern.
