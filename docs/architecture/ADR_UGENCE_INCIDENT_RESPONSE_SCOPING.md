...
| D-2 | How containment reaches RA-6 | **It produces a signal; it never delivers one.** The package builds the neutral `AuthorityReassessmentSignal` payload and records that it did. A composition root hands it to RA-6's reassessor. The package holds no writer, no client and no reference to `AuthorityLifecycleService`, so it cannot mutate lifecycle state even by mistake. |
