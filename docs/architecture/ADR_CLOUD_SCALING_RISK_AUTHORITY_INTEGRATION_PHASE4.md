...
### (4a) Pre-merge audit corrections F-1, F-2 and F-3

Findings from the independent audits of the Phase 4C implementation PR, corrected before merge. Neither changes a contract, a digest, a D-4 identifier or the security *claim* — F-1 closes a gap between the claim and the implementation, and F-2 corrects an over-stated verification claim.

**F-1 — in-process admission was `isinstance`, admitting subclasses.** Every value the adapter reads from a recommendation (`digest()`, `to_canonical_dict()`, `_digest_payload()`, the embedded serializers) is reached through dynamic dispatch, so a subclass overriding `digest()` had its attacker-chosen value adopted as the authenticated `recommendation_digest` — defeating the boundary at exactly the point it exists to defend. Admission is now **exact type** (`type(source) is CapacityActionRecommendation`), evaluated **before any attribute of the object is touched**. Recomputing with the unbound base method was considered and rejected as insufficient: `CapacityActionRecommendation.digest(source)` still routes through `self._digest_payload()` → `self.to_canonical_dict()`, so an override further down the chain is reached anyway and *silently* yields a digest over attacker-supplied content. The same correction is applied to the sibling `RecommendationAbstention` in-process path, which carried the identical flaw. Legitimate subclasses retain a route: serialize and submit the canonical document, which reconstructs an exact base instance and re-digests from content.

**F-2 — the "offline isolated install" was not offline.** The verifier upgraded pip from the index and installed without `--no-index`, so numpy was fetched from PyPI during the supposedly isolated phase; the installation was offline only in the sense that wheels might happen to be cached. The verifier now collects the full dependency closure into a local wheelhouse in an explicitly-labelled online phase, then installs from that wheelhouse alone with the index structurally disabled, and proves the guarantee by negative control (a missing wheel fails; a bogus index cannot rescue it; a failed install leaves nothing importable). Success is reported only after every step records completion. **The scope of the claim is stated precisely rather than broadly:** the verifier run as a whole is *not* offline — **phase A is an online build-and-collect phase** and must reach an index, because that is what collecting a dependency closure means; **phase B is the genuinely offline isolated-installation phase**, and it alone is the guarantee. Within phase B, `--no-index` and `PIP_NO_INDEX=1` are the actual index prohibition; the unroutable sentinel index is **defense in depth** behind them, protecting against a future edit dropping a flag rather than supplying the prohibition itself. The closing banner names exactly the stage verified: `OFFLINE ISOLATED INSTALLATION STAGE VERIFIED`.

**F-3 — the authenticated token did not enforce its own digest invariant.** `AuthenticatedRecommendation` is the *token* every downstream consumer trusts: holding one is what entitles a caller to project a recommendation into a v2 request. It could nevertheless be hand-constructed with an **exact** canonical `CapacityActionRecommendation` and a **syntactically valid but incorrect** `recommendation_digest`, because nothing checked that the digest described the recommendation. A consumer would then accept a token whose name claimed a reconciliation that had never happened, and would emit a request bound to a digest of the constructor's choosing. The invariant is now enforced:

```
token.recommendation_digest == token.recommendation.digest()
```

checked in `__post_init__`, so no supported construction can mint a mismatched token, and **re-established at the point of consumption**, because a frozen dataclass is not a security boundary: `object.__new__` skips `__post_init__` entirely, `object.__setattr__` rewrites a frozen field afterwards, and a token subclass can replace a field with a property that returns a different object on each read. Consumers therefore require the **exact** token type — not `isinstance` — and, having established the exact embedded `CapacityActionRecommendation` type, recompute the digest without subclass-dispatch risk. Every rejection is a controlled typed failure occurring before any clock read, context, binding, request or seam call. `AuthenticatedAbstention` receives the symmetric exact-type treatment; no digest semantics are invented for abstentions, but a digest that *is* carried must describe the carried abstention. The one authoritative validation routine is deliberately **private** — the public API is unchanged.

**Which boundary is attacker-reachable.** The three call sites are deliberately not described as equivalent. `project_recommendation` accepts an `AuthenticatedRecommendation` **token** from its caller and is therefore the attacker-reachable boundary: a hand-minted, constructor-bypassed, mutated or subclassed token arrives there directly, and `_validate_authenticated_recommendation` is what refuses it. `CloudScalingRiskAdapter.project` and `CloudScalingRiskAdapter.evaluate` accept **raw controller artifacts or their canonical mappings** — never a caller-supplied token — so their `_validate_authenticated_output` calls re-validate a token the adapter itself just minted through `authenticate_controller_output` and cannot fail today. A forged token passed directly to `.project` or `.evaluate` is rejected as an **unsupported input source** before any token validation runs: a correct rejection, but by a different gate. Those two revalidations are retained as deliberate defense in depth — the invariant is re-established at the boundary that relies on it rather than inherited from a caller's good behavior — and are not to be read as a claim that all three boundaries carry equal attacker reachability.

**The validated-pair discipline is now guarded by a regression test.** `project_recommendation` consumes only the `(recommendation, digest)` pair the validator returned and never re-reads those fields from the token, closing the check-then-use window that `object.__setattr__` from a concurrent caller would otherwise open. `test_the_projection_uses_the_validated_pair_and_never_re_reads_the_token` installs hostile class-level properties on the exact token type *after* a legitimate token exists — a subclass would be refused by the exact-type gate before this path is reached — has each protected field answer honestly on its first read and substitute a different, internally self-consistent recommendation on every later read, and asserts both that the projection carries the honest digest, subject and context facts and that each field was consulted exactly once.

**F-3 is content integrity, not signed producer authenticity**, and narrows nothing below. A fully self-consistent forgery carrying its own genuine digest satisfies this invariant and remains structurally admitted, asserted as a passing test rather than left implicit.

**What did not change.** The serialized-document path's independent digest comparison is untouched and remains the substance of §7. The three-way distinction of Amendment 4 (4) stands exactly as written: F-1 was a **subclass-dispatch defect**, and correcting it does **not** address the third case — a fully self-consistent fabrication with an attacker-selected matching digest is still structurally admitted, because Phase 4C establishes **content-integrity binding, not signed producer authenticity**. Nothing in this correction should be read as narrowing that limit.

### (5) Scope of this amendment

No schema, field, digest or version changes. `SubjectContext`, `SubjectBinding` and `risk-subject-evaluation-request-2` are untouched; `subject_digest` is not expanded; v1 is unchanged. The new package is **additive** — a new distribution, a new CI workflow, and this amendment. Every Phase 4C outcome terminates at a non-executable `SubjectRiskDecision` or a typed non-evaluation, with every authorization/execution flag fixed `False` and a forged `True` **rejected rather than normalized**. No envelope issuance, ActionGate invocation, credential issuance, provider execution, effect verification or learning is **implemented in this package, imported by any module of this package, publicly exported by this package, or called by any Phase 4C path**. Stated at that precision deliberately: Risk Authority may transitively load envelope- or ActionGate-related modules through its own package initialization, so a blanket "not reachable" claim about the process would be false. What Phase 4C asserts, and what its structural tests check, is that Phase 4C itself neither imports, exports nor calls any of them.
...
Phase 4 performs **no** envelope issuance, **no** ActionGate authorization, **no** provider/cloud execution or actuation (Phase 5), and **no** effect verification or recommendation learning (Phase 6). These remain fail-closed: not implemented by this design, not imported by any module it adds, not publicly exported by it, and not called by any Phase 4 path. (A dependency may still load such modules through its own package initialization; the claim is about what Phase 4 implements, imports, exports and calls, not about what exists in the process.)
