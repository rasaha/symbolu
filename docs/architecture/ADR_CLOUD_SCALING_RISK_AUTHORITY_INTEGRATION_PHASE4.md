...
A design-only projection type, **`CapacityRiskSubjectProjection`** (proposed, schema `cloud-scaling-risk-subject-projection-1`), owned by the adapter package. It performs **exactly one** authoritative digest-binding chain in which every risk-relevant fact is hashed **exactly once** — never simultaneously as loose fact-dictionary entries and through a nested `context_digest`. The full normative chain, pseudocode and a worked canonical example are in **§5.3**; the summary is:
...
2. Canonicalize it with the named **Risk Authority canonicalizer** (`risk_authority.crypto.canonical.to_canonical_obj` / `canonical_bytes`) and compute a domain-separated **`context_digest`** (§5.3, §5.4).
3. Build a **fixed `SubjectBinding` object** (schema `risk-subject-binding-1`) containing only binding anchors — schema/domain id, subject identity, recommendation digest, context digest, tenant binding — and compute **`subject_digest = digest(SubjectBinding)`**.
...
The chain is **canonical, deterministic, domain-separated, digest-bound, versioned, tenant- and scope-bound, strict about missing fields, non-authoritative**, and **structurally incapable** of embedding policy, control results, keys, decisions, envelopes, or execution instructions. Each risk-relevant fact appears in **one** canonical object only; **duplicate representations are prohibited** (§5.5).
...
| 2 | subject / workload id | `.subject.workload_id` | outer `subject_id` + `SubjectBinding.subject_id` | `request_digest` + `subject_digest` |
...
Note the deliberate split (F2): **tenant** and **evidence references** live on the **outer request**, which is authoritative for tenant identity and admitted evidence; `SubjectContext` holds only *neutral subject facts* (#4–#11) and **not** tenant or evidence references. `SubjectBinding.tenant_id` is a binding anchor (not a second source of truth) and carries a mandatory equality invariant with the outer `tenant_id` (§5.3). Emitted request: `subject_type="cloud_scaling.capacity_action"`, `requested_scope=Scope(purposes=("cloud_scaling.capacity_action",))` (**minimal — never overloaded** with topology), all executable flags fixed `False` by the `SubjectRiskDecision` contract.
...
`SubjectContext` carries **only neutral subject facts**. It **does not** carry `tenant_id` or `evidence_references` — those are authoritative on the **outer** request (F2). The `schema_version` string is the object's fixed **domain identifier** for domain separation (§5.4).

| Field | Type | Neutral meaning | Populated from (scaling) | Missing-vs-named |
|---|---|---|---|---|
| `schema_version` | `str` (mandatory, fixed `= "risk-subject-context-1"`) | contract + domain identifier | constant | — |
| `subject_id` | `str` (required, non-empty) | evaluated subject identity | `subject.workload_id` | required |
...
- deterministic domain-separated **`context_digest = digest(to_canonical_obj(context))`** (`sha256:`) — the context's **only** digest; its fields are hashed **once**, here, and are **not** re-hashed as loose entries anywhere else (F1);
...
Three canonical objects, three distinct digest domains, **every fact hashed exactly once**. No fact is hashed both as a loose fact-dict entry and via `context_digest`.
...
# (1) curated, closed neutral context — neutral subject facts ONLY.
#     Built field-by-field by the adapter; the controller's raw to_canonical_dict()
#     is NEVER passed in (§5.4, §14).
context = SubjectContext(
    schema_version   = "risk-subject-context-1",     # fixed domain id
    subject_id       = rec.subject.workload_id,
...
context_digest = digest(context)                      # domain: risk-subject-context-1

# (2) fixed subject-binding object — binding anchors ONLY (no loose facts).
binding = SubjectBinding(
    schema_version        = "risk-subject-binding-1", # fixed domain id
    tenant_id             = outer_tenant_id,          # equality-invariant with the outer request (§ below)
    subject_id            = rec.subject.workload_id,
...
subject_digest = digest(binding)                      # domain: risk-subject-binding-1

# (3) versioned request — outer authority for tenant, evidence refs, purpose/domain.
request = SubjectRiskEvaluationRequest_v2(
    schema_version      = "risk-subject-evaluation-request-2",   # fixed domain id
    ...
    subject_context     = context,                    # inspectable by the resolver (§5.6)
    correlation_id      = rec.forecast_evidence.forecast.correlation_id,
    idempotency_key     = digest({"tenant_id": outer_tenant_id, "subject_id": rec.subject.workload_id,
...
request_digest = digest(request)                      # domain: risk-subject-evaluation-request-2
```

**Mandatory equality invariant (F2).** `binding.tenant_id` and `context`-vs-request `subject_id` are binding anchors, **not** independent sources of truth. `binding.tenant_id == request.tenant_id` and `context.subject_id == binding.subject_id == request.subject_id` are checked before evaluation; **any mismatch is a fail-closed non-decision** (`NOT_EVALUATED`, never PASS/ALLOW). Reconciliation is explicit, never implicit.

**Worked canonical example** (compact JSON, sorted keys, from the RA canonicalizer; reproducible byte-for-byte):

`SubjectContext` canonical form →
```json
{"action_type":"scale_up","compute_group":"cluster-7","environment":"prod","magnitude_after":9,"magnitude_before":6,"region":"eu-west-1","resource_class":"web","schema_version":"risk-subject-context-1","subject_asserted_at":"2026-08-13T04:00:00.000000Z","subject_id":"wl-checkout-api","subject_valid_from":"2026-08-13T04:00:00.000000Z","subject_valid_until":"2026-08-13T04:15:00.000000Z","zone":null}
```
`context_digest = sha256:0f0171ed3ce14a6b5e0f4908fe9853794439e047d84ba726838a4abc9cfe5831`

`SubjectBinding` canonical form (with an illustrative `recommendation_digest = sha256:1111…1111`) →
```json
{"context_digest":"sha256:0f0171ed3ce14a6b5e0f4908fe9853794439e047d84ba726838a4abc9cfe5831","recommendation_digest":"sha256:1111111111111111111111111111111111111111111111111111111111111111","schema_version":"risk-subject-binding-1","subject_id":"wl-checkout-api","subject_type":"cloud_scaling.capacity_action","tenant_id":"tnt-acme"}
```
`subject_digest = sha256:9b42092be328da3d78f3abca69e838e608eda23231351ef05afd285afebfeafe`

Two independent implementers using the RA canonicalizer over the same curated inputs produce these exact digests.

### 5.4 Authoritative canonicalizer and domain separation (F4, F5)
...
- **Explicit domain separation.** Each canonical object carries a fixed `schema_version`/domain identifier — `risk-subject-context-1`, `risk-subject-binding-1`, `risk-subject-evaluation-request-2` — inside its own canonical form (consistent with existing RA primitives, which embed `schema_version` per contract). Each digest therefore belongs to a **distinct domain**; **no digest from one domain may be substituted for another** (a `context_digest` can never satisfy a `subject_digest` slot, etc.). Different `schema_version` values yield different canonical bytes and different digests.
...
**Required (fail closed if missing):** outer `tenant_id`; `subject_id` (`workload_id`); `recommendation_id`; `recommendation_time`; `validity_seconds`; the selected plan's `action_kind`; at least one `ResourceChange` with `current_capacity` / `proposed_capacity`; and `forecast_evidence_digest` (outer `evidence_references`). **Where they live:** neutral subject facts (`environment`, `region`, `zone`, `compute_group`, `resource_class`, `action_type`, `magnitude_before/after`, validity window) in `SubjectContext`; tenant identity and evidence references on the **outer request**. **Missing is distinct from named** — an absent optional (e.g. `environment`) is the explicit sentinel `null` inside `SubjectContext` (never omitted, never coerced to `""`), so `context_digest` (and thus `subject_digest`) differs between "no environment" and "environment = ''".

## 7. Recommendation and evidence digest binding

- `subject_digest = digest(SubjectBinding)`, and `SubjectBinding` carries `recommendation_digest = rec.digest()` and `context_digest`. The evidence digests (forecast / cost / topology / state) live in the **outer** `evidence_references`, bound by `request_digest`. **Any change to the recommendation digest, any neutral subject fact, or any evidence reference changes `subject_digest` or `request_digest`** (acceptance invariant); because each fact is hashed once, a change is reflected in exactly one digest, deterministically.
- The adapter independently recomputes `rec.digest()` from the reconstructed recommendation and requires equality with the value placed in `SubjectBinding` before projecting → a **recommendation-digest mismatch fails closed**, no seam call. A **`context_digest` mismatch** (recomputed context ≠ carried `context_digest`) is likewise a fail-closed non-decision (§12).
...
- The future v2 trusted production path **must reject or ignore any caller-supplied evaluation time** — a caller-supplied evaluation time on a trusted production path is a fail-closed non-decision (§12), never a source of authority.
- **Reference/test-mode** time injection must remain **explicitly separated** from production authority (the labelled reference seam only), exactly as the merged v1 seam already separates reference from production. **No v1 code is changed by this PR.**
...
| **`context_digest` mismatch** (recomputed context ≠ carried digest) | fail-closed non-decision, no authority |
| **outer-request/context inconsistency** (equality invariant violated: `binding.tenant_id ≠ outer tenant_id`, or `subject_id` disagreement) | fail-closed non-decision, no authority |
...
| **caller-supplied evaluation time on a trusted production path** | rejected/ignored; if asserted as authority → fail-closed non-decision (§10) |
...
For `SubjectContext`, `SubjectBinding`, `SubjectRiskEvaluationRequest_v2` and any adapter outcome type: strict constructor / `from_dict` parity; unknown fields rejected; mandatory `schema_version`; canonical JSON; deterministic `sha256:` digest; **no floats** and no exponent/representation-dependent decimal strings; canonical RFC3339 UTC timestamps; nested digest re-validation on reconstruction; exact tenant/subject/scope binding; frozen/immutable records; round-trip digest stability; explicit authority flags fixed `False`. A reconstructed request recomputes `context_digest` and `subject_digest` and rejects any mismatch.
...
- **v2 subject-context migration (D-2), RA-owned, additive, versioned, backward-compatible:**
  - New `SubjectContext` (schema `risk-subject-context-1`), new `SubjectBinding` (schema `risk-subject-binding-1`), and new request schema `risk-subject-evaluation-request-2` with an **optional** `subject_context` field — three distinct digest domains (§5.4).
...
**Negative/adversarial tests ≥ 2× happy-path.** Independent tests for: caller-supplied `PASS` has no path; forged / duck-typed recommendation; recommendation-digest mismatch; **`context_digest` mismatch**; **outer-request/context equality-invariant violation** (`binding.tenant_id ≠ outer tenant_id`, `subject_id` disagreement); **domain-digest substitution** (a `context_digest` placed in a `subject_digest` slot, or a cross-schema digest reuse) is rejected; missing tenant / subject; cross-tenant & cross-scope reuse; missing-vs-named optional (`None` ≠ `""`/`0` yields distinct digests); **non-canonical value** rejection (float, exponent/representation-dependent decimal string, mis-formatted/non-UTC timestamp, `bool`-for-int); stale / future / expired recommendation; validity beyond forecast horizon; unsupported schema; unknown fields (each of the three canonical objects); **caller-supplied `evaluation_time` on a trusted production path rejected/ignored**; contradictory / missing / untrusted / stale evidence (→ deny/escalate, never pass); no authoritative / ambiguous policy; unavailable Risk Authority; reconstruction failure; direct-construction-vs-`from_dict`; **worked-example digest reproducibility** (§5.3 fixtures); installed-wheel behavior; and **sentinel tests** proving envelope issuance, ActionGate, and provider execution are never invoked and every executable flag stays `False`. Happy path: a valid recommendation over admitted trusted evidence yields a non-executable `RISK_PASSED` and nothing more.
...
- **One authoritative digest chain**: every risk-relevant fact is hashed **exactly once** (§5.3); no field is hashed both as a loose entry and via `context_digest`. Each of `risk-subject-context-1` / `risk-subject-binding-1` / `risk-subject-evaluation-request-2` is a **distinct digest domain**; no cross-domain digest substitution.
- An input change **alters exactly one** of `context_digest` / `subject_digest` / `request_digest`, deterministically.
- **Equality invariants enforced fail-closed**: `binding.tenant_id == outer tenant_id`; `context.subject_id == binding.subject_id == outer subject_id`. Any mismatch is a non-decision, never PASS.
