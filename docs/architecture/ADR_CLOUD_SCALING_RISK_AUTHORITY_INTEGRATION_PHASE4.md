...
### (4a) Pre-merge audit corrections F-1 and F-2

Two findings from the independent audit of the Phase 4C implementation PR, corrected before merge. Neither changes a contract, a digest, a D-4 identifier or the security *claim* — F-1 closes a gap between the claim and the implementation, and F-2 corrects an over-stated verification claim.
