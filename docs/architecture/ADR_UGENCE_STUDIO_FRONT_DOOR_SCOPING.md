...
| 7 | Simulation laboratory | evidence, failures, readiness | `agent-runtime` over fixture providers, `agent-runtime-governance` hook; readiness: `agent-value-readiness` — **named here and wired nowhere**: no package under `packages/integration/`, `apps/` or `platform/` imports `ugence_agent_value_readiness`, and `console-api` does not either (verified 2026-09-09), so this row states an intended consumer, not an existing one. **RULED 2026-09-09: wiring is deferred until UVI ADR §26.10 is resolved *in implementation* and the complete verifier dependency chain is releasable** — trusted evidence verification, benchmark resolution and governed threshold evaluation (UVI ADR §25 M-GTE.1 / M-GVR.1), which requires `benchmark-registry-authority` at `0.3.0` with its reviews complete. Wiring sooner would expose a capability that can only return `NOT_EVALUATED`, misleading users into treating a deliberately incomplete path as a functioning simulation; the honest BLOCK shown today is the better state. **The future consumer is this Studio Simulate / front-door workstream**, which owns the wiring and the screen but **not** readiness semantics and **not** verification — both stay with their own owners. `agent-value-readiness` is an **intended** consumer-reachable capability and is expressly **not** a terminal leaf (it reaches users through UVI, the one customer-facing capability; the package itself remains an internal engine per UVI ADR §4 and D-18); `agent-assurance-evidence` records what an exercise found | Simulate screen; `governance_hook`, `provider_registry` not handed in P3E, so it BLOCKs by design | `[V]` packages, `[G]` the readiness wiring — unowned |
