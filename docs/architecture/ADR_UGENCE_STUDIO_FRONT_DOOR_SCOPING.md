...
| 7 | Simulation laboratory | evidence, failures, readiness | `agent-runtime` over fixture providers, `agent-runtime-governance` hook; readiness: `agent-value-readiness` — **named here and wired nowhere**: no package under `packages/integration/`, `apps/` or `platform/` imports `ugence_agent_value_readiness`, and `console-api` does not either (verified 2026-09-09), so this row states an intended consumer, not an existing one. **Wiring is deferred until UVI ADR §26.10 resolves** (recorded 2026-09-09): until a conforming `GateResultVerifier` can exist, the shipped deny-all refuses every gate result, so a wired Simulate screen could only ever render `NOT_EVALUATED` — a worse outcome than the honest BLOCK it shows now. This defers; it assigns no workstream, and naming one remains an owner ruling; `agent-assurance-evidence` records what an exercise found | Simulate screen; `governance_hook`, `provider_registry` not handed in P3E, so it BLOCKs by design | `[V]` packages, `[G]` the readiness wiring — unowned |
