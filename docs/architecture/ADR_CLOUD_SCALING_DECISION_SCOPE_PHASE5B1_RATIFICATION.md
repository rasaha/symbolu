...
| R-12 **CLOSED (Phase 5A)** `[V]` | **Audited pairwise, then closed at the builder under owner ruling.** The audit took all fifteen pairs among the six instants to their actual producers rather than their names. Nine are already enforced upstream `[V]`: `valid_from <= asserted_at <= valid_until` by `SubjectContext.__post_init__` (`risk_authority/integrations/evaluation_contracts.py:880`), with the adapter setting `valid_from` *equal* to `asserted_at` (`ugence_cloud_scaling_risk_integration/projection.py:330`) over a strictly positive `validity_seconds` (`ugence_cloud_scaling_controller/planning/recommendation.py:211`); `evaluated_at` inside that window by the v2 seam *before* it stamps the decision it returns (`risk_authority/api/evaluation_seam.py:328`, `:330`, `:334`, threaded to `evaluated_at` at `:264` and `:521`); and `expires_at = evaluated_at + ttl` over a positive default TTL by Decision Authority (`risk_authority/services/decision_authority.py:41`, `:146`). One is a genuine `[G]` and was **not** invented: nothing relates `expires_at` to `valid_until`, and the package's own reference composition legitimately has a decision outliving the recommendation (`scripts/verify_isolated_install.py:142`, `:180`). The remaining five all involve `attestation.issued_at`, the one instant no upstream contract relates to any other — 5B-0A §11 carries it forward deliberately unjudged. **Owner ruled** `issued_at >= subject_asserted_at` and `issued_at <= decision_evaluated_at`, both inclusive, and **declined** `issued_at <= subject_valid_until` as the freshness window 5B-0A §11 refused to invent and as already subsumed by the evaluation bound. Closed by two builder guards under `ATTESTATION_INSTANT_INCOHERENT` (inventory 52 → 54), each carrying a mutation-backed admission and misattribution proof. Reads no clock: both sides of each comparison are facts the builder already holds. Original framing kept below for the record:  **Found by the 5B-2 part 2 independent review.** Gate 13 compares each of the candidate's six timestamps against `as_of` and **never against each other**, so a candidate whose attestation predates its subject assertion by a year verifies `[V]`. That is internal *incoherence* rather than staleness — the pair is consistent with the instant and inconsistent with itself — and it belongs upstream at construction, where the builder holds all six. Out of scope for R-2, which is about reconciling the instant | Phase 5A |
