> **SUPERSEDED — supporting discovery input.** This is the RA-8 architecture
> *discovery* document (verdict `RA8_ARCHITECTURE_DECISION_REQUIRED`, discovery
> commit `19b0dc33`, brought forward verbatim below the banner). It is retained
> as the code-grounded evidence base. It is **superseded** by the ratified
> canonical set:
> - `RISK_AUTHORITY_RA8_SPEC.md` (canonical spec; verdict
>   `RA8_PRECONDITIONS_RESOLVED_READY_FOR_IMPLEMENTATION`)
> - `ADR_RISK_AUTHORITY_RA8_EXECUTION_EFFECT_RECONCILIATION.md` (ACCEPTED)
> - `RISK_AUTHORITY_RA8_IMPLEMENTATION_PLAN.md`
>
> Its five open decisions D-A–D-E are ratified in the spec (§4–§8). Read it for
> the discovery evidence; read the spec/ADR for the decisions.
