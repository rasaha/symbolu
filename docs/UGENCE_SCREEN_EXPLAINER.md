...
**Status:** reference, 2026-09-06, against `a22c9c79`. Every entry is taken from the
...
clear, execute) are performed on no screen anywhere, by ruling SD-2, and the authority
plane's four writes wait on a verified identity provider (AP-3).
...
- **Never:** loads or revokes a grant. That write waits on the identity gate.
...
- **Issuing, activating, revoking, granting:** the authority plane's four writes, gated by AP-3 on a verified identity provider that no one has provisioned.
