...
consistently (e.g. `symbolu_robotics/autonomous_control_plane/__init__.py`, `acp/ACP_ARCHITECTURE.md:1`,
`acp/ACP_EXECUTIVE_SUMMARY.md:58,64`). "Clearance" is a separate vocabulary word — the CLEAR/HOLD verdict of
...
| **ACP** / AI Control Plane (product umbrella) | The 3-layer enterprise stack (Context Minimization + ActionGate + ACP); unified console plan | `ACP/UGENCE_UNIFIED_CONSOLE_PLAN.md`, `ai_control_plane_v3/`, `AI_CONTROL_PLANE_VC_BRIEF.md` | Ugence platform | **Documentation only** |
...
| **clearance** / CLEAR–HOLD | Shared vocabulary for the operational-safety verdict ("safe right now?") | `models.py:7`, `operational_safety.py`, `ACP/PHASE1_GOVERNED_LOOP_DTO_CONTRACT.md:208` | shared | vocabulary, not a module |
...
- **Agrees:** the **cloud/console** framing — `ugence_console_api/…/operational_safety.py:11-12` (*"It never
  ...
  `acp/RESPONSIBILITY_MATRIX.md:12-13`; `symbolu_robotics/autonomous_control_plane/cloud/composition.py`.
- **Contradicts:** the **robotics V1** framing — `acp/ACP_ARCHITECTURE.md:20` (*"deterministic
