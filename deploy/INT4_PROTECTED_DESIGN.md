...
> throughput.** int4_protected is **decode-throughput-negative** (~0.13–0.67× bf16;
...
|---|---|
| **Decode throughput-negative (0.13–0.67×)** | kernel-bound (gather + dequant); recoverable ceiling ~0.27–0.30×, **not parity**. Size the gather-fusion win first: `bench_decode_gather_fusion_headroom.py`. |
