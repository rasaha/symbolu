"""
VENDORED CTM+ KV-Cache Eviction Reference — DO NOT EDIT FOR BEHAVIOR.

Provenance
----------
- Upstream path:   CTM_plus/KVPolicy/kv_policy/attention_evictor.py
...
Role
----
This file is the **specification reference** for the PCAM KV-cache
scoring policy. It is NOT the runtime implementation. The runtime lives
at simulator/pcam/kv_policy.py and is a faithful bit-parity port of
this file per ADR-0001.

This vendored copy exists so the PCAM parity harness has a stable,
in-tree oracle and does not depend on sys.path tricks against an
ambient CTM_plus package location. When the upstream CTM+ reference
changes behavior, update this file by replaying the update ritual
above, re-running the parity harness, and bumping the PCAM version.

Editing rules
-------------
- Do NOT edit the algorithmic behavior of this file. Any change that
  alters increment, estimate, halve, score_block, select_victims, or
  the PHASE_WEIGHTS table must first land upstream in CTM+ and must
  be re-vendored here.
- Minor type-hint or import adjustments required solely for the
  vendored location (e.g. removing circular imports, adjusting
  `from __future__ import annotations`) are permitted only when
  strictly necessary and must be documented inline.
- The module docstring below this header is preserved verbatim from
  upstream.

================================================================
Original upstream docstring follows:
================================================================
