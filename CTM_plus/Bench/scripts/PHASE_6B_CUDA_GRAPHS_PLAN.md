...
| **Phase 6D — Kernel profile** | ✅ **CLOSED, MEASURED** — torch.profiler apples-to-apples (int4_eager vs bf16_eager). int4 = 1133 ms vs bf16 = 467 ms (2.43×). Model linear layers + RMSNorm/rotary identical in both cells (sanity-check OK). **The entire 666 ms gap is in the writer's per-decode-step op chain (5944 `aten::index` + 4732 `index_put_` + 11k `to`/`copy_` calls = ~30 small ops per layer per step) — not the int4 kernel itself.** | `PHASE_6D_PROFILING_RUNBOOK.md` + `bench_phase6_d_profile_gpu.py` + `analyze_phase6d_profile.py` |
| **Phase 6E — Writer op fusion (CUDA)** | ✅ **CLOSED, SHIPPED** behind `PHASE6E_FUSED_WRITER=1` opt-in. Two custom CUDA kernels (`fused_decode_write_v`, `fused_decode_write_k`) collapse the writer's ~30-op-per-layer-per-decode-step Python chain into 2 launches. **Byte-eq verified** (15/15 GREEN across B=1..32, multi-block boundaries, inactive masks, non-contig QKV views). **Throughput**: captured B=1 39.2 → 46.9 (+20%); captured B=8 174 → 192.6 (+10.7%, short of the 25% target); cap/bf16 at B=32 0.19× → 0.20× (short of the 0.25× gate). Bench self-reports GREEN. Three bring-up bugs (kv_cache stride, scale-divide codegen, non-contig views) all documented in the findings doc. | `PHASE_6E_WRITER_FUSION_FINDINGS.md` + `verify_phase6e_fused_byte_eq.py` + `CUDA_int4_protected/` |
| **Phase 6 long-context HBM crossover bench** | ✅ **MEASURED, INFORMS NEXT PRIORITIES** — at `max_model_len ∈ {8K, 16K, 32K}`, vLLM reports int4 `max_concurrency` = 2× bf16 at every length, BUT measured HBM after init is +4.66 to +5.09 GB HIGHER for int4 (sidecars allocated outside vLLM's budget). Throughput ratio improves with context: 0.66× at 8K → 0.78× at 32K (B=8). Quality sanity check 0/3 for BOTH cells (greedy decode at max_tokens=32 doesn't reach the embedded answer — measurement artifact, not a quality regression). **Verdict per the stated criterion: NOT_JUSTIFIED for Phase 6F kernel surgery yet.** Two follow-on workstreams scoped instead: 6G (sidecar diet) + 6H (high-load capacity bench). | `PHASE_6_LONG_CONTEXT_RUNBOOK.md` + `bench_phase6_long_context_gpu.py` |
| **Phase 6G — Sidecar diet (Step 1 audit complete)** | ✅ **AUDIT MEASURED, IMPLEMENTATION DECISION DEFERRED to post-6H.** Tensor-by-tensor measurement at mml ∈ {8K, 16K, 32K}: sidecars total **3.44-3.98 GB** (16.4% of KV cache, invariant across mmls). Top 5 contributors all within 1.25× of each other (no single-tensor win). **62% per_token / 38% per_block** scaling. Per-cached-token marginal cost: 104 bytes/token/layer = 10.2% overhead on top of vLLM cache. **Realistic diet ceiling: ~2.5 GB** (C+D+A stacked with overlap correction) against the ~5 GB observed delta — diet alone cannot close the HBM crossover. Recommendation in the findings doc: run Phase 6H FIRST to test whether the 2× max_concurrency is honest; if YES, do diet option C (fp8 sidecars, 1.72 GB measured savings) to make the capacity advantage cleaner; if NO, diet doesn't rescue the project. | `PHASE_6G_SIDECAR_DIET_FINDINGS.md` + `audit_phase6g_sidecar_overhead.py` |
| **Phase 6H — High-load capacity bench** | ✅ **MEASURED, INCONCLUSIVE for saturation; clear NEGATIVE for throughput.** Raw verdict was NOT_JUSTIFIED but the verdict tree over-fired on `ratio==1.0` (both cells completed all requests = INCONCLUSIVE, not loses). The chosen B values (8K: 64/96; 16K: 32/48; 32K: 16/20) did NOT push either cell to OOM — vLLM's gpu_memory_utilization is a soft hint not a hard cap. Real data: **bf16 was 1.4-1.9× faster than int4 at every (mml, B); int4 used 2.2-2.85 GB more HBM at every (mml, B); int4's reported 2× max_concurrency advantage unmeasurable at tested B values.** Verdict tree fixed; three options forward documented in findings doc: (α) push bench to extreme B, (β) pivot to int4_protected-vs-int4_naive quality story, (γ) close the line. **User chose β → Phase 6J scoped.** | `PHASE_6H_HIGH_LOAD_FINDINGS.md` + `bench_phase6_h_high_load_gpu.py` |
| **Phase 6J — int4_protected vs int4_naive quality A/B** | SCOPED. Three cells: A (bf16, ceiling), B (int4_naive, all-zeros protect mask), C (int4_protected, calibrated mask). Five metrics: needle-in-haystack retrieval at 8K/16K/32K, token agreement with bf16, decode collapse/repetition, perplexity, optional lm-eval slice. **Acceptance: protected ≥ naive + 0.20 on needle, ≥ naive + 0.10 on token agreement** at mml ∈ {16K, 32K}. If JUSTIFIED → project ships as long-context quality-preserving int4 backend; if NOT_JUSTIFIED → close the int4_protected line as research artifact. **The right comparison for the protect-mask claim**: Phases 6E-6H compared int4_protected to bf16 (apples-to-oranges); 6J compares int4_protected to int4_naive (the actual A/B for the design). | `PHASE_6J_QUALITY_COMPARISON_DESIGN.md` |
| **Phase 6J — int4_protected vs int4_naive quality A/B** | **BLOCKED on Phase 6K fix.** Path 1 implementation landed (PHASE6J_NAIVE_FORCE_ZERO toggle, all-zeros mask generator, bench scaffold, 16/16 CPU tests pass). Stage 3 smoke ran but revealed the **Phase 6K bug**: int4_protected captured produces `pérdida pérdida...` collapse on short prompts (<32 tokens of prefill). Bisection in N showed non-monotonic failure (N=8 garbage, N=17 coherent, N=30 degraded, N=44 coherent). Root cause traced to a vllm-flash-attn-dev kernel bug; patch landed at `PHASE_6K_FLASH_ATTN_OOB_FIX.patch`. Resume 6J after 6K verification. | `PHASE_6J_QUALITY_COMPARISON_DESIGN.md` + `bench_phase6j_quality_gpu.py` |
| **Phase 6K — int4_packed_load OOB mask fix** | PATCH WRITTEN + AWAITING GPU VERIFICATION. Four kernel call sites in `vllm-flash-attn-dev/csrc/flash_attn/src/flash_fwd_kernel.h` passed `params.seqlen_k` (buffer S=8192) as `s_curr` instead of per-batch `binfo.actual_seqlen_k`. Result: OOB smem-zeroing for short cache_seqlens never fires; positions ≥ actual_seqlen_k contribute uninitialized/spliced-noise K/V to attention. **The bug masked all of Phases 6E/6H/long-context findings on output coherence** (throughput/HBM numbers still valid since they don't depend on output correctness, but the protect-mask claim was never measurable until 6K is verified). Fix: 4 one-line changes; rebuild + run bisection script. | `PHASE_6K_FLASH_ATTN_OOB_FIX_FINDINGS.md` + `PHASE_6K_FLASH_ATTN_OOB_FIX.patch` + `apply_phase6k_flash_attn_oob_fix.sh` |
| Phase 6F — Kernel-internal optimization | **HALTED** pending 6G + 6H outcomes. Phase 6E delivered correctness + partial perf; remaining gap to bf16 is structural (Python writer alongside C++ attention). Restart criterion: 6G achieves the HBM gate AND 6H achieves the capacity gate — both conditions point to a memory + capacity backend where closing the throughput gap via kernel surgery becomes worth the multi-week cost. | `PHASE_6F_KERNEL_OPT_DESIGN.md` |
