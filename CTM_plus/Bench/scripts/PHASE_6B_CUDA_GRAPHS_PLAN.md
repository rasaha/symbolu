...
| **Phase 6E — Writer op fusion (CUDA)** | DAY 1 DONE (Python ref + integration scaffold + 8-test CPU verifier + CUDA skeletons + setup.py). Days 2-5 = CUDA kernel implementation on GPU pod. Projected: cap/bf16 0.30× → ~0.45-0.50× at B=8. | `PHASE_6E_WRITER_FUSION_DESIGN.md` + `PHASE_6E_NEXT_SESSION_PROMPT.md` |
| Phase 6F — Kernel-internal optimization | SCOPED (contingent on 6E outcome) — profile the int4 kernel with ncu, fix the dominant H1-H4 bottleneck. Skip if 6E gets us to cap/bf16 ≥ 0.5×. | `PHASE_6F_KERNEL_OPT_DESIGN.md` |
