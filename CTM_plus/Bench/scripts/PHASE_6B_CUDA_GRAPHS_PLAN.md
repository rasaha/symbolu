...
| **Phase 6G — Sidecar diet** | SCOPED. Goal: shrink the ~5 GB sidecar overhead so measured int4 HBM ≤ bf16 at 16K/32K. Step 1 (mandatory): tensor-by-tensor sidecar audit. Then evaluate stack of options: `v_n_groups=2` (~1.5 GB), `n_protect=3` (~0.5 GB), fp8 sidecars (~2 GB). Acceptance: HBM ≤ bf16 OR ≥40% sidecar shrink with quality intact. **Halted until audit lands measured data.** | `PHASE_6G_SIDECAR_DIET_DESIGN.md` |
| **Phase 6H — High-load capacity bench** | SCOPED. Bench at B values near/above bf16's max_concurrency (8K: B=64,96; 16K: B=32,48; 32K: B=16,20). Goal: prove or disprove that int4's reported 2× `max_concurrency` translates to 2× completed-load capacity, vs being bookkeeping eaten by sidecar OOMs. Acceptance: int4 completes ≥1.5× the requests bf16 completes at high B at one or more max_model_lens, with quality intact. Run pre-6G AND post-6G to measure both baseline and diet impact. | `PHASE_6H_HIGH_LOAD_CAPACITY_DESIGN.md` |
