...
| Phase 6F — Kernel-internal optimization | NOT YET STARTED. Phase 6E delivered correctness + partial perf; remaining gap to bf16 (cap/bf16 0.20× at B=32 vs the 0.25× gate) reflects structural overhead from the Python writer. Candidates: (1) fuse the read path's splice + view ops too, (2) move int4 logic into the flash_attn kernel itself, (3) investigate the pre-existing eager-mode garble in the int4_protected backend. | `PHASE_6F_KERNEL_OPT_DESIGN.md` |
