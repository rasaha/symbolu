...
The algorithm ops (quantize/dequantize/pack/unpack) stay **bit-identical** to route-B. The **numerical output may differ** from route-B's measured §19.4 quality because per-group scale boundaries depend on block layout: route-B applies group=32 scales across the entire prefill as one tensor; route-A applies them per vLLM block. **Bit-identical quality is only guaranteed when `block_size == group_size == 32`** (see open question 3 — this is the recommended alignment). For block_size != group_size, quality re-validation is required; see day 5 below.
...
* **`vllm/attention/backends/flash_attn.py`** — `FlashAttentionImpl.forward`, the `cache_kv` invocation block.
  ...
  Patch shape: wrap the call. On entry, take the (key, value) tensors from local scope (`key`, `value` of shapes `(num_tokens, num_kv_heads, head_dim)`), pass through `quantize_per_channel_int4` + `quantize_per_token_int4` + `pack_int4`, store the packed bytes in an alternate paged buffer sized at ~1/4 of FP16's footprint (4 INT4 elements pack into the same 2 bytes that hold 1 FP16 element; the actual ratio is closer to 1/3.2 once group-32 scale + asymmetric offset overhead is included — see `kv_policy/int4_fused_attention_sketch.py:hbm_bytes_for_attention` for the precise math).
...
* **Quality numbers — conditional on `block_size == group_size == 32`.** Perplexity 1.024×, MMLU −0.9pt @ 1000q, teacher-forced 96.4% next-token agreement. The algorithm is the same and the input tensors are the same, but the per-group scale boundaries depend on how K/V is sliced into blocks. With `block_size = group_size = 32` (the recommended alignment per open question 3), each vLLM block IS exactly one KIVI group, and the per-block scales coincide with route-B's per-group scales — quality is bit-identical. With `block_size = 16` (vLLM default) and `group_size = 32`, scales would span two blocks; the implementation either has to gather across blocks (matches route-B) or accept per-block scales (different boundaries → different quality, re-validation needed). Day 5 explicitly re-runs MMLU 200q to detect this drift if it occurs.
...
5. **How are scales updated when a block fills incrementally during decode?** Each decode step appends one token to the current block. With per-group scales, options are: (a) recompute the block's scale by dequantizing prior tokens, mixing in the new one, and re-quantizing the whole block — expensive; (b) keep the scale fixed at the first token's value and clip later tokens that exceed range — quality cost; (c) **buffer FP16 tokens in a staging slot until the block fills, then quantize the whole block at once — recommended default**, costs ~32 × head_dim × 2 bytes per active sequence = ~1 KB extra FP16 per sequence per layer (trivial). Verify on day 1; pick (c) unless there's a vLLM scheduler reason not to.
6. **Does `vllm/engine/llm_engine.py`'s `LLMEngine.__init__` expose the discovery hook timing we assume?** The plan references `model_executor.init_model_executor_environment` as the install anchor (after model load, before first request). The exact symbol name has moved across vLLM minor versions — verify on day 1 against the pod's actual vLLM 0.7.x source. If the symbol moved, the install-timing pattern is the same (between model load and first request); just walk to the new symbol.
