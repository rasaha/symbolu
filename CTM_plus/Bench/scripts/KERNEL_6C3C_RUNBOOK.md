...
| 1.3 | Dispatch: add `_int4kv` arm in `run_mha_fwd_splitkv_dispatch` (in `flash_fwd_launch_template.h` — at SHA 720c948 the splitkv dispatch lives in the main launch template, not a separate `_splitkv_` file). INITIALLY routes to the same template instantiation as fp16/bf16 | new dispatch arm picked at runtime when `mha_fwd_kvcache_int4` is called |
