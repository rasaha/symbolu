#!/usr/bin/env python3
"""ASG Reflection Renderer v2 — deterministic acoustic trajectory → authored reflection.

Implements `ASG_RENDERER_V2_DESIGN.md`. This is a RENDERING layer only: it reads the frozen engine's output
...
Pipeline:  Deterministic Engine → Acoustic Trajectory (deterministic) → Authoring (prompt + deterministic
...
# ----- Layer 2: the Acoustic Trajectory (deterministic) ---------------------------------------------
...
def render_prompt(traj, mode):
    ...
    return f"""You are the authoring voice of ASG (Acoustic Symbol Generation). You render a DETERMINISTIC
acoustic trajectory into prose. You are a rendering layer, not an interpreter: narrate the MOVEMENT given to
...
- Word / form: {traj['word']}
- Acoustic Trajectory (roles, in order): {' → '.join(traj['trajectory'])}
...
def format_text(res):
    ...
    L = [f"# ASG — \"{res['word']}\"  ·  mode={res['mode']}  ·  read={res['by']}", "",
...
def main(argv=None):
    ap = argparse.ArgumentParser(description="ASG Reflection Renderer v2 (deterministic trajectory → authored reflection).")
