...
- **Word acoustic profile — three pre-registered composition variants**, each run identically through both nulls (§9–10) and both rubrics:
  - **(a) Consonant-only (PRIMARY / CONFIRMATORY):** sum of `worldly_vritti` one-hot **count vectors** over the word's consonantal varṇas (from frozen `lexicon_wordformation_consonants.json`). **This variant alone determines the §14 verdict.**
