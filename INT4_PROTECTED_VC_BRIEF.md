...
| **Read-skip decode throughput-positive at long context — GRADUATED: projected → MEASURED** | ✅ **MEASURED** (`Llama-3.1-8B-Instruct`, 128K-native, int4 read-skip, no rope hacking): retention vs full-int4 decode = **+25.0 % @32K, +46.4 % @44K, +58.8 % @52K, +72.2 % @60K**, needle **1.0/1.0** at every ctx (monotonic — grows with length). `off` slopes down (linear KV read), retention stays flat (bounded ~1.9K retained, ~95 % skip) → gap **grows** with length. Crossover lands **≤32K on KV-heavy GQA** (8-KV-head Llama); ~42K on 4-KV-head Qwen. **Caveat:** int4-KV long-context *quality* is **model-dependent** — held on Llama (standard rope), broke on Qwen2.5-7B-1M (extreme `rope_theta`). |
