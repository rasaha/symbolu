...
| AWQ + AWQ-Marlin | These quantize *weights*, not KV-cache — orthogonal budgets, **complementary** to int4_protected. **Composition status (Phase 6O, measured + fixed):** the stack initially crashed on a dtype mismatch (AWQ fp16 activations vs int4 bf16-dequant K); a one-commit dtype bridge (e06dd26) fixed it. **AWQ weights + int4_protected KV now load and run together with quality preserved — MMLU 56% (stacked) vs 55% (each alone), within noise.** The *integration and quality* compose. The combined *memory* saving was not cleanly measured (the stack bench's HBM proxy is unreliable); confirming the additive weight+KV footprint via live introspection is the small remaining step. |
